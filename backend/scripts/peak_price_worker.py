...
Modes:
  --active   : update signal status IN ('open','tp1','tp2','tp3') — cron tiap 5 menit
               Always refresh (peak still moving)
  --closed   : update closed signals — cron tiap 1 jam
               Refresh if: peak_price IS NULL OR signal created within --recent-days (default 14)
               Closed signals <14 days old may still see price movement post-close
  --all      : backfill all signals (one-shot or scheduled)
               Process: peak_price IS NULL OR signal created within --recent-days
  --signal-id <id> : update satu signal specific (debug)
  --dry-run  : compute tapi gak update DB
  --limit N  : max signal per run (default 500)
  --recent-days N : for --closed and --all, re-compute signals within N days even if
                    peak already exists (default 14). Set to 0 to only process NULL peaks.

Usage:
  python peak_price_worker.py --active --limit 2000
  python peak_price_worker.py --closed --limit 5000
  python peak_price_worker.py --closed --recent-days 7
  python peak_price_worker.py --all --limit 5000
...
# ─── DB helpers ───────────────────────────────────────────────────────────
def get_signals_to_update(session, mode: str, limit: int, signal_id: Optional[str], recent_days: int) -> List[dict]:
    if signal_id:
        sql = text("""
        ...
        # Active signals: always refresh — peak still moving
        sql = text("""
            SELECT signal_id, pair, entry, status, created_at
            FROM signals
        ...
        # Closed signals: NULL peak OR recent (within --recent-days)
        # Recent closed signals may still see price movement post-close
        if recent_days > 0:
            sql = text("""
                SELECT signal_id, pair, entry, status, created_at
                FROM signals
                WHERE status IN ('closed_win', 'closed_loss')
                  ...
                  AND (
                      peak_price IS NULL
                      OR created_at::timestamptz >= NOW() - INTERVAL '1 day' * :recent_days
                  )
                ORDER BY peak_updated_at NULLS FIRST, created_at DESC
                LIMIT :lim
            """)
            rows = session.execute(sql, {"lim": limit, "recent_days": recent_days}).fetchall()
        else:
            # recent_days=0: only process NULL peaks (old behavior)
            sql = text("""
                SELECT signal_id, pair, entry, status, created_at
                FROM signals
                WHERE status IN ('closed_win', 'closed_loss')
        ...
        # All signals: NULL peak OR recent
        if recent_days > 0:
            sql = text("""
                SELECT signal_id, pair, entry, status, created_at
                FROM signals
...
def run(mode: str, limit: int, signal_id: Optional[str], dry_run: bool, recent_days: int):
    ...
    try:
        signals = get_signals_to_update(session, mode, limit, signal_id, recent_days)
        log.info(f"Mode={mode} limit={limit} recent_days={recent_days} dry_run={dry_run} → {len(signals)} signals to process")
...
def main():
    parser = argparse.ArgumentParser(description="Peak Price Worker for LuxQuant signals")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--active", action="store_true", help="Update active signals (open/tp1-3)")
    group.add_argument("--closed", action="store_true", help="Update closed signals (NULL peak + recent)")
    group.add_argument("--all", action="store_true", help="Backfill all signals (NULL peak + recent)")
    group.add_argument("--signal-id", type=str, help="Update single signal")
    parser.add_argument("--limit", type=int, default=500, help="Max signals per run (default 500)")
    ...
    parser.add_argument("--recent-days", type=int, default=14,
                        help="For --closed/--all: also re-compute signals created within N days "
                             "even if peak already exists. Set 0 for NULL-only. Default 14.")
    ...
    try:
        run(mode, args.limit, args.signal_id, args.dry_run, args.recent_days)
    finally:
        release_lock()
