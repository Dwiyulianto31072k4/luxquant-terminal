...
const PageLoader = () => (
  <div className="flex items-center justify-center min-h-[60vh]">
    <div className="flex flex-col items-center gap-3">
      ...
      <span className="font-mono text-[10px] uppercase tracking-wider text-text-muted">Loading...</span>
    </div>
  </div>
);
      ...
      <>
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <div className="w-14 h-14 rounded-md bg-[#0a0805] border border-gold-primary/25 flex items-center justify-center mb-5 relative overflow-hidden">
            ...
            <svg className="w-7 h-7 text-gold-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
          </div>
          <h2 className="text-xl font-normal text-white mb-2 tracking-tight">Premium Feature</h2>
          <p className="font-mono text-[11px] uppercase tracking-wider text-text-muted mb-6 max-w-md normal-case">
            Fitur ini hanya tersedia untuk pengguna premium. Upgrade sekarang untuk akses penuh ke semua fitur LuxQuant.
          </p>
...
function AppShell({ children }) {
  ...
  const moreMenuItems = [
    ...
    { path: "/daily-performance", label: "Daily Performance", icon: "📅", description: "Per-day breakdown by hit date with BTC context" },
            ...
            <div className="flex items-center gap-2 lg:gap-6">
              ...
              <div className="flex items-center gap-2 cursor-pointer group" onClick={() => handleNav("/home")}>
                <div className="w-8 h-8 lg:w-9 lg:h-9 relative rounded-sm overflow-hidden border border-white/[0.06]">
                  <img src="/logo.png" alt="LuxQuant" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                </div>
              ...
              return (
                ...
                >
                  ...
                  <span className={`transition-colors ${
                    active ? 'text-gold-primary' : 'text-text-muted group-hover:text-text-secondary'
                  }`}>
                    {item.icon}
                  </span>
                  <span className={`font-mono text-[9px] uppercase tracking-wider transition-colors ${
                    active ? 'text-gold-primary' : 'text-text-muted group-hover:text-text-secondary'
                  }`}>
                    {item.label}
                  </span>
    ...
    <BrowserRouter>
      <AuthProvider>
        ...
        <Routes>
          ...
          <Route path="/daily-performance" element={<AppShell><DailyPerformancePage /></AppShell>} />
