...
const dirPillClass = (dir) => {
 ...
 if (v.includes('BEAR') || v === 'SHORT') return 'bg-red-500/15 text-loss border border-red-500/30';
...
const Tag = ({ name, important = false }) => {
 ...
 if (tag.includes('BEARISH') || tag.includes('BELOW_EMA200')) return 'bg-red-500/15 text-loss border-red-500/30';
 ...
 if (tag.includes('FNG_EXTREME') || tag.includes('LIQ_VERY_LOW') || tag.includes('RISK_OFF')) return 'bg-red-500/10 text-loss border-red-500/20';
...
const HeroSummary = ({ facts, tagsAnnotated, direction }) => {
 ...
 const btcColor = btcChange == null ? 'text-text-primary' : Number(btcChange) >= 0 ? 'text-green-400' : 'text-loss';
 ...
 <span className="text-[9px] uppercase tracking-wider text-text-muted font-semibold">Bias</span>
...
const DeepAnalysis = ({ signalId, enrichment: legacyEnrichment, isOpen, onClose, pair }) => {
 ...
 !showLive ? 'bg-accent/20 text-accent' : 'text-text-muted hover:text-text-primary'
 }`}
 >
 Entry snapshot
 </button>
