...
const Tag = ({ name, important = false }) => {
  ...
  const getColor = (tag) => {
    ...
    if (
      ...
      tag.includes("PARABOLIC") ||
      tag.includes("EXHAUSTION") ||
...
const FactRow = ({ label, value, subtle = false }) => (
  <div className="flex items-center justify-between py-1 border-b border-ink/5 last:border-b-0">
    ...
    <span
      className={`text-[11px] font-mono ${subtle ? "text-text-primary/60" : "text-text-primary"}`}
    >
      {value}
    </span>
  </div>
...
const Section = ({ title, children }) => (
  <div className="bg-surface-raised rounded-lg border border-ink/5 p-3 space-y-1 h-full">
    <p className="text-[9px] text-text-muted uppercase tracking-wider font-semibold mb-2">
      {title}
    </p>
    {children}
  </div>
...
const HeroSummary = ({ facts, tagsAnnotated, direction }) => {
  ...
  const stats = [
    {
      label: "H1 RSI",
      value: formatNum(h1.momentum?.rsi, 1),
      sub: h1.momentum?.rsi_state,
      color: "text-text-primary",
    },
    {
      label: "H1 ADX",
      value: formatNum(h1.trend?.adx, 1),
      sub: h1.trend?.trend_strength,
      color: "text-text-primary",
    },
    {
      label: "BTC 24h",
      value: formatPct(btcChange),
      sub: `dom ${formatNum(ctx.btc?.dominance, 1)}%`,
      color: btcColor,
    },
    {
      label: "Fear & Greed",
      value: ctx.fng?.value ?? "—",
      sub: ctx.fng?.classification,
      color: fngColor(ctx.fng?.value),
    },
    {
      label: "Volatility",
      value: ctx.environment?.volatility_regime || "?",
      sub: `ATR P${formatNum(ctx.environment?.atr_percentile_h4, 0)}`,
      color: "text-text-primary",
    },
  ];

  return (
    <div className="bg-gradient-to-br from-accent/15 to-accent/5 rounded-xl border border-ink/12 p-3 sm:p-4">
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        ...
        <Section title="Momentum H1">
          <FactRow
            label="RSI"
            value={`${formatNum(h1.momentum?.rsi, 1)} (${h1.momentum?.rsi_state || "?"})`}
          />
          <FactRow
            label="MACD"
            value={`${formatNum(h1.momentum?.macd_hist, 6)} (${h1.momentum?.macd_direction || "?"})`}
          />
          <FactRow
            label="Volume"
            value={`${formatNum(h1.volume?.ratio, 2)}x avg (${h1.volume?.state || "?"})`}
          />
        </Section>
        ...
        <Section title="Structure">
          ...
          />
          {structure.smc?.golden_setup && <FactRow label="Special" value="SMC Golden Setup" />}
        </Section>
      ...
      {detailTags.length > 0 && (
        <div>
          <button
            ...
            className="text-[10px] text-accent hover:text-accent transition"
          >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <Section title={`${tf.toUpperCase()} Trend`}>
        ...
        <FactRow
          label="Strength"
          value={`${trend.trend_strength || "?"} (ADX ${formatNum(trend.adx, 1)})`}
        />
        ...
        {trend.ema200_available && <FactRow label="EMA200" value={formatNum(trend.ema200, 6)} />}
      ...
      <Section title={`${tf.toUpperCase()} Momentum`}>
        <FactRow
          label="RSI"
          value={`${formatNum(momentum.rsi, 2)} (${momentum.rsi_state || "?"})`}
        />
        <FactRow
          label="MACD hist"
          value={`${formatNum(momentum.macd_hist, 8)} (${momentum.macd_direction || "?"})`}
        />
      ...
      <Section title={`${tf.toUpperCase()} Volume`}>
        ...
        {volume.falling_with_trend && (
          <FactRow label="Trend" value="Volume falling with price" subtle />
        )}
      </Section>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
      <Section title="BTC Context">
        ...
        <FactRow
          label="Dom delta"
          value={btc.dominance_delta !== null ? formatPct(btc.dominance_delta, 2) : "— (warm-up)"}
        />
      ...
      <Section title="Sentiment & Derivatives">
        <FactRow
          label="Fear & Greed"
          value={`${fng.value ?? "—"} (${fng.classification || "—"})`}
        />
        <FactRow
          label="Funding rate"
          value={context.funding_rate !== null ? formatNum(context.funding_rate, 5) : "—"}
        />
      </Section>
      ...
      <Section title="SMC Structure">
        ...
        <FactRow
          ...
          value={smc.sweep_recent ? `✓ (${smc.sweep_count})` : "—"}
        />
      ...
      <Section title="Key Levels">
        ...
        {levels.broke_resistance_recent && (
          <FactRow label="Recent break" value="Broke H4 resistance" />
        )}
...
const DeepAnalysis = ({ signalId, enrichment: legacyEnrichment, isOpen, onClose, pair }) => {
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <h2 className="text-text-primary font-display text-sm font-semibold truncate">
                        {pair}
                      </h2>
                      <span
                        className={`px-1.5 py-0.5 rounded text-[9px] font-bold flex-shrink-0 ${dirPillClass(signalDir)}`}
                      >
                        {signalDir}
                      </span>
                    </div>
                  ...
                  {v3Data && (
                    <div className="relative">
                      <button
                        ...
                        className="flex items-center gap-1 px-2.5 py-1.5 bg-accent/12 hover:bg-accent/20 border border-ink/12 hover:border-ink/20 rounded-lg text-accent text-[10px] sm:text-[11px] font-semibold transition-all"
                      >
                        📋 Copy ▼
                      </button>
                      {showCopyMenu && (
                        <div className="absolute right-0 top-full mt-1 bg-surface-raised border border-ink/12 rounded-lg shadow-xl overflow-hidden z-20 min-w-[180px]">
                          {COPY_OPTIONS.map((opt) => (
                            <button
                              key={opt.id}
                              onClick={() => handleCopy(opt.id)}
                              className="block w-full text-left px-3 py-2 text-[11px] text-text-primary hover:bg-accent/12 transition"
                            >
                              {opt.label}
                            </button>
                          ))}
                      ...
                      {showHistory && history && (
                        <div className="mt-3 space-y-2 max-h-64 overflow-y-auto custom-scrollbar">
                          ...
                          {history.map((entry, i) => (
                            <div
                              key={i}
                              className="bg-surface-raised rounded border border-ink/5 p-2"
                            >
                              ...
                              <div className="flex flex-wrap gap-1">
                                {(entry.snapshot?.tags || [])
                                  .filter((t) =>
                                    (entry.snapshot?.tags_annotated || []).find(
                                      (ta) => ta.name === t && ta.important
                                    )
                                  )
                                  .slice(0, 10)
                                  .map((t) => (
 ...
 .custom-scrollbar::-webkit-scrollbar-thumb { background: rgb(var(--accent) / .3); border-radius: 4px; }
 .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgb(var(--accent) / .5); }
 `}</style>
    </>
  );
