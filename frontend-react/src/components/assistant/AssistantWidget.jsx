    ...
    <>
      ...
      {open && (
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 z-[9999] flex h-[85vh] w-full flex-col overflow-hidden rounded-t-2xl border border-line/30 bg-surface-raised shadow-[0_-8px_40px_rgb(var(--scrim) / 0.35)] sm:inset-x-auto sm:bottom-6 sm:left-1/2 sm:h-[600px] sm:w-[760px] sm:max-w-[92vw] sm:-translate-x-1/2 sm:rounded-2xl sm:shadow-[0_25px_60px_rgb(var(--scrim) / 0.35)] lg:w-[900px]">
            {/* Header */}
                  ...
                  <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3">
                    {suggestions.map((s, i) => (
                      <button
                        key={i}
                        onClick={() => send(s)}
                        className="text-left rounded-xl border border-ink/10 bg-ink/[0.03] px-3.5 py-2.5 text-[13px] text-text-primary/80 hover:border-line/40 hover:bg-gold-primary/[0.06] hover:text-text-primary transition-all"
                      >
                        {s}
                      </button>
                    ))}
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed sm:text-[14px] ${
                    m.role === 'user'
                      ? 'whitespace-pre-wrap bg-gold-primary text-accent-fg font-medium'
            ...
            <div className="border-t border-ink/10 bg-surface-raised p-3 pb-24 sm:px-6 sm:py-4 sm:pb-4">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="flex-1 resize-none rounded-xl border border-ink/10 bg-surface-raised px-3.5 py-2.5 text-[13px] text-text-primary placeholder:text-text-muted/60 focus:outline-none focus:border-line/40 max-h-24 sm:text-[14px]"
                />
                <button
                  onClick={() => send()}
