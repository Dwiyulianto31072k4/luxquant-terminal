...
export default function AssistantWidget({ pageId = "signals", contextHint = null }) {
  ...
  useEffect(() => {
    ...
    getAssistantStatus().then((s) => {
      if (alive) setEnabled(s.enabled !== false);
    });
    return () => {
      alive = false;
    };
    ...
    <>
      ...
      {open && (
        <>
          ...
          <div
            className="hidden sm:block fixed inset-0 z-[9998] bg-scrim/40 backdrop-blur-[2px]"
          ...
          <div className="fixed inset-x-0 bottom-0 z-[9999] flex h-[85vh] w-full flex-col overflow-hidden rounded-t-2xl border border-ink/12 bg-surface-raised shadow-[0_-8px_40px_rgb(var(--scrim) / 0.35)] sm:inset-x-auto sm:bottom-6 sm:left-1/2 sm:h-[600px] sm:w-[760px] sm:max-w-[92vw] sm:-translate-x-1/2 sm:rounded-2xl sm:shadow-[0_25px_60px_rgb(var(--scrim) / 0.35)] lg:w-[900px]">
            ...
            <div className="flex items-center justify-between border-b border-ink/10 bg-surface-raised px-4 py-3 sm:px-5">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="font-display text-sm font-semibold text-text-primary leading-none sm:text-base">
                    LuxQuant Assistant
                  </p>
                  {label && (
              ...
              <button
                onClick={() => {
                  setOpen(false);
                  navigate(`/assistant?page=${encodeURIComponent(pageId)}`);
                }}
              ...
              <button
                onClick={() => setOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-ink/10 text-text-muted hover:bg-ink/5 hover:text-text-primary transition-all"
                aria-label="Close"
              >
                <svg
                  className="w-4 h-4"
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 sm:px-6 sm:py-6 space-y-3"
            >
              {!hasChat && (
                <div className="space-y-4">
                  <p className="text-sm text-text-primary/70 leading-relaxed sm:text-[15px]">
                    Ask anything about how to use the{" "}
                    <span className="text-accent font-semibold">{label || "this"}</span> page —
                    column meanings, filtering, and more.
                  </p>
              ...
              {messages.map((m, i) => (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed sm:text-[14px] ${
                      m.role === "user"
            ...
            <div className="border-t border-ink/10 bg-surface-raised p-3 pb-24 sm:px-6 sm:py-4 sm:pb-4">
              ...
              <p className="mt-2 text-center text-[9px] text-text-muted/50">
