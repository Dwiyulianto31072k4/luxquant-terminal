...
export default function AssistantWidget({ pageId = 'signals', contextHint = null }) {
 ...
 className="lq-cta fixed bottom-24 right-4 z-[9998] h-12 pl-3.5 pr-4 text-sm sm:bottom-5 sm:right-5"
 aria-label="Open LuxQuant Assistant"
 >
 <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M21 12a8 8 0 01-11.6 7.1L4 20l1-4.4A8 8 0 1121 12z" /></svg>
 Ask AI
 </button>
 )}
 ...
 <div className="hidden sm:block fixed inset-0 z-[9998] bg-scrim/40 backdrop-blur-[2px]" onClick={() => setOpen(false)} />
 ...
 {label && <p className="mt-0.5 truncate font-mono text-[10px] uppercase tracking-wider text-text-muted">{label}</p>}
 </div>
 </div>
 <button
 onClick={() => { setOpen(false); navigate(`/assistant?page=${encodeURIComponent(pageId)}`); }}
 className="mr-1 hidden sm:inline-flex h-8 items-center gap-1.5 rounded-lg border border-ink/10 px-2.5 text-text-muted hover:bg-ink/5 hover:text-text-primary transition-all font-mono text-[10px] uppercase tracking-wider"
 ...
 <button onClick={() => setOpen(false)} className="flex h-8 w-8 items-center justify-center rounded-lg border border-ink/10 text-text-muted hover:bg-ink/5 hover:text-text-primary transition-all" aria-label="Close">
 ...
 Ask anything about how to use the <span className="text-accent font-semibold">{label || 'this'}</span> page — column meanings, filtering, and more.
 </p>
 ...
 key={i}
 onClick={() => send(s)}
 className="text-left rounded-xl border border-ink/10 bg-ink/[0.03] px-3.5 py-2.5 text-[13px] text-text-primary/80 hover:border-ink/15 hover:bg-surface-secondary hover:text-text-primary transition-all"
 >
 {s}
 </button>
 ))}
 ...
 <div className="rounded-2xl bg-ink/[0.05] border border-ink/5 px-3.5 py-2.5">
 <span className="flex gap-1">
 <span className="h-1.5 w-1.5 rounded-full bg-accent/70 animate-bounce" style={{ animationDelay: '0ms' }} />
 <span className="h-1.5 w-1.5 rounded-full bg-accent/70 animate-bounce" style={{ animationDelay: '150ms' }} />
 <span className="h-1.5 w-1.5 rounded-full bg-accent/70 animate-bounce" style={{ animationDelay: '300ms' }} />
 </span>
 </div>
 </div>
 )}
 ...
 {/* Input */}
 <div className="border-t border-ink/10 bg-surface-raised p-3 pb-24 sm:px-6 sm:py-4 sm:pb-4">
