...
export function renderInline(text, onLink) {
 ...
 if (m[2] !== undefined) nodes.push(<strong key={`b${i++}`} className="font-semibold text-accent">{linkify(m[2], onLink, `bi${i}`)}</strong>);
 else if (m[3] !== undefined) nodes.push(<code key={`c${i++}`} className="rounded bg-ink/10 px-1 py-0.5 font-mono text-[12px] text-accent">{m[3]}</code>);
 last = re.lastIndex;
 }
