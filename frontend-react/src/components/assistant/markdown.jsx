  ...
  while ((m = PAGE_RE.exec(text))) {
    ...
    nodes.push(
      <a
        ...
        onClick={(e) => {
          e.preventDefault();
          if (onLink) onLink(path);
        }}
        className="text-accent underline underline-offset-2 hover:text-accent cursor-pointer"
      >
        {matched}
      </a>
    );
...
export function renderInline(text, onLink) {
  ...
  while ((m = re.exec(text))) {
    ...
    if (m[2] !== undefined)
      nodes.push(
        <strong key={`b${i++}`} className="font-semibold text-accent">
          {linkify(m[2], onLink, `bi${i}`)}
        </strong>
      );
      ...
      nodes.push(
        <code
          key={`c${i++}`}
          className="rounded bg-ink/10 px-1 py-0.5 font-mono text-[12px] text-accent"
        >
          {m[3]}
        </code>
      );
...
export function renderMarkdown(text, onLink) {
  ...
  const flushList = () => {
    if (list) {
      ...
      blocks.push(
        <Tag
          key={k++}
          className={`space-y-1 pl-1 ${list.ordered ? "list-decimal list-inside" : ""}`}
        >
          {list.items.map((it, j) => (
            <li key={j} className="leading-relaxed">
              {!list.ordered && <span className="mr-2 text-text-muted">•</span>}
  ...
  for (let raw of lines) {
    ...
    if (/^(-{3,}|\*{3,}|_{3,})$/.test(t)) {
      flushPara();
      flushList();
      blocks.push(<div key={k++} className="my-1 h-px bg-ink/10" />);
      continue;
    }
    ...
    if (h) {
      ...
      flushList();
      blocks.push(
        <p key={k++} className="mt-1 font-semibold text-text-primary">
