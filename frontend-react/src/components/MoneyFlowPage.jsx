...
const Th = ({ label, sortKey, sort, onSort, align = "right", sortable = true, className = "" }) => {
  ...
  if (!sortable) {
    return (
      <th
        className={`py-2.5 px-2 sm:px-3 ${alignCls} font-mono text-[9px] uppercase tracking-[0.14em] text-text-primary/35 ${className}`}
      >
        {label}
      </th>
    );
      ...
      <tr key={i} className="border-b border-ink/[0.05]">
        {[...Array(cols)].map((__, k) => (
          <td key={k} className="py-3 px-3">
            <div
              className={`h-3 rounded bg-ink/[0.05] animate-pulse ${k === 0 ? "w-2/3" : "w-12 ml-auto"}`}
            />
          </td>
        ))}
      </tr>
...
const StatCell = ({ label, value, color = "text-text-primary", align = "left" }) => (
  <div className={`flex flex-col ${align === "right" ? "items-end" : ""}`}>
    <span className="font-mono text-[8px] uppercase tracking-[0.15em] text-text-muted/50">
      {label}
    </span>
...
const Gauge = ({ value, label, sub, subColor, accent = "gold" }) => {
  ...
  const bar = accent === "gold" ? "bg-accent" : accent === "emerald" ? "bg-profit" : "bg-ink/60";
  return (
    <Card className="p-4">
      ...
      <div className="flex items-baseline gap-2 mb-3">
        ...
        {sub && (
          <span
            className={`font-mono text-[10px] tabular-nums ${subColor || "text-text-muted/60"}`}
          >
            {sub}
          </span>
        )}
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <Gauge
        ...
        sub={
          macro.btc_dominance_change_7d != null
            ? `${macro.btc_dominance_change_7d > 0 ? "+" : ""}${macro.btc_dominance_change_7d}pp 7d`
            : null
        }
      ...
      <Gauge
        ...
        sub={
          macro.stablecoin_dominance_change_7d != null
            ? `${macro.stablecoin_dominance_change_7d > 0 ? "+" : ""}${macro.stablecoin_dominance_change_7d}pp 7d`
            : null
        }
        subColor={
          macro.stablecoin_dominance_change_7d != null
            ? pctColor(-macro.stablecoin_dominance_change_7d)
            : ""
        }
        accent="white"
      />
    ...
    <>
      <div className="space-y-6">
        ...
        <Card glow>
          ...
          <table className="hidden sm:table w-full border-collapse table-fixed">
            <thead>
              <tr className="border-b border-ink/[0.08]">
                <th className="py-2.5 px-2 sm:px-3 text-left font-mono text-[9px] uppercase tracking-[0.14em] text-text-primary/35 w-10">
                  #
                </th>
                    ...
                    >
                      ...
                      <td className="py-3 px-2 sm:px-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="flex -space-x-1.5 shrink-0">
                            {(s.top_3_coins || []).slice(0, 3).map((url, k) => (
                              <img
                                ...
                                src={url}
                                alt=""
                                className="w-5 h-5 rounded-full border border-surface-raised bg-ink/5"
                          ...
                          <svg
                            className="w-3.5 h-3.5 ml-auto flex-shrink-0 text-text-primary/20 group-hover:text-text-primary transition-colors"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
                      ...
                      <td
                        className={`hidden md:table-cell py-3 px-3 text-right font-mono text-xs tabular-nums ${pctColor(s.mcap_change_7d)}`}
                      >
                        {s.mcap_change_7d != null ? (
                          fmtPct(s.mcap_change_7d)
                        ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <svg
                          className="w-3.5 h-3.5 shrink-0 text-text-primary/25"
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2"
                      ...
                      <div className="mt-2.5 grid grid-cols-3 gap-2">
                        ...
                        <StatCell
                          ...
                          color={
                            s.mcap_change_7d != null
                              ? pctColor(s.mcap_change_7d)
                              : "text-text-muted/30"
                          }
                        />
                        <StatCell
                          label="Mcap"
                          value={fmtUSD(s.market_cap)}
          ...
          {err && !loading && (
            <div className="p-8 text-center text-loss text-sm font-mono">{err}</div>
          )}
          {!loading && !err && rows.length === 0 && (
        ...
        <p className="font-mono text-[9px] uppercase tracking-[0.15em] text-text-muted/40 px-1">
          Leaders = top-3 by 24h move · tap a row to see every coin in the narrative · Data:
          CoinGecko
        </p>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-text-primary text-sm font-medium truncate">
            {p.base_symbol || p.name}
          </span>
          {p.flow_tag && (
            <span
              className={`shrink-0 font-mono text-[8px] uppercase tracking-wider px-1.5 py-0.5 rounded border ${
                p.flow_tag === "net_buying"
                  ? "text-profit border-profit/25 bg-profit/10"
                  : p.flow_tag === "net_selling"
                    ? "text-loss border-loss/25 bg-loss/10"
  ...
  const rows = useMemo(() => {
    ...
    const val = (x, k) => {
      switch (k) {
        ...
        case "flow_intensity":
          return x.flow_intensity ?? -Infinity;
        default:
          return 0;
      }
    };
    ...
    return [
      ...sorted.filter((c) => c.is_luxquant_signal),
      ...sorted.filter((c) => !c.is_luxquant_signal),
    ];
  }, [coins, filter, q, sort]);
                ...
                {shownCoins.map((c) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          {clickable &&
                            (isLoading ? (
                              ...
                              <svg
                                className="w-3.5 h-3.5 flex-shrink-0 text-text-muted/50 group-hover/row:text-accent transition-colors"
                                viewBox="0 0 24 24"
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="2"
                      ...
                      {/* 24h */}
                      <td
                        className={`py-3 px-2 sm:px-3 text-right font-mono text-sm tabular-nums font-semibold ${pctColor(c.price_change_24h)}`}
                      >
                        ...
                        {c.turnover_tag && (
                          <span
                            className={`font-mono text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded border ${
                              hi
                                ? "text-accent border-ink/12 bg-accent/12"
                                : "text-text-muted border-ink/[0.08] bg-ink/[0.03]"
                            }`}
                          >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        {clickable &&
                          (isLoading ? (
                            ...
                            <svg
                              className="w-3.5 h-3.5 ml-auto flex-shrink-0 text-text-muted"
                              viewBox="0 0 24 24"
                              fill="none"
                              stroke="currentColor"
                              strokeWidth="2"
                      ...
                      <div className="mt-2.5 grid grid-cols-3 gap-2 items-start">
                        <StatCell
                          label="24h"
                          value={fmtPct(c.price_change_24h)}
                          color={pctColor(c.price_change_24h)}
                        />
      ...
      <div className="space-y-3 mt-8">
          ...
          right={
            <span className="hidden sm:flex items-center gap-3 font-mono text-[9px] uppercase tracking-[0.15em] text-text-muted/60">
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-sm bg-profit/80" />
                Buy
              </span>
              <span className="flex items-center gap-1.5">
        ...
        <Card glow>
          ...
          {!dexLoading && dexPools.length > DEX_LIMIT && (
            <ShowMore
              expanded={dexShowAll}
              total={dexPools.length}
              onClick={() => setDexShowAll((v) => !v)}
            />
          )}
        </Card>
        <p className="font-mono text-[9px] uppercase tracking-[0.15em] text-text-muted/40 px-1">
    ...
    <div className="w-full px-4 lg:px-8 py-6">
      ...
      <div className="mb-5 max-w-3xl">
        ...
        <p className="text-[12px] text-text-primary/50 leading-relaxed mt-2">
          ...
          buy/sell pressure and large on-chain moves in one place.
        </p>
      </div>
      ...
      <div className="flex items-end justify-between gap-3 border-b border-ink/[0.07] mb-6">
        <div
          role="tablist"
          aria-label="Money Flow sections"
            ...
            return (
              <button
                ...
                className={`group whitespace-nowrap pb-3 pt-1 text-[14px] font-medium border-b-2 -mb-px transition-colors ${
                  active
                    ? "text-text-primary border-accent/25"
        ...
        {searchable && (
          <SearchBox
            value={q}
            onChange={setQ}
            placeholder={tab === "coins" ? "Search coin…" : "Search sector…"}
          />
        )}
      </div>
