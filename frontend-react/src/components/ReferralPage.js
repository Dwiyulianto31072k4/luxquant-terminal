...
function generateSimpleQR(text) {
  // This creates a decorative QR-like pattern. For production, use a real QR lib.
...
export default function ReferralPage() {
  ...
  const [activeSection, setActiveSection] = useState('overview'); // overview | payout | history
            ...
            {stats && (
              <div className="flex gap-3 sm:gap-4">
                <div className="text-center px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] min-w-[90px]">
                  <p className="font-display text-xl sm:text-2xl font-bold text-white">{stats.total_referrals}</p>
                  <p className="text-[10px] text-text-muted uppercase tracking-wider mt-0.5">{T('total_referrals')}</p>
                </div>
                <div className="text-center px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] min-w-[90px]">
                  <p className="font-display text-xl sm:text-2xl font-bold text-emerald-400">${stats.total_commission?.toFixed(2)}</p>
                ...
                <div className="text-center px-4 py-3 rounded-xl bg-gold-primary/[0.06] border border-gold-primary/15 min-w-[90px]">
                  <p className="font-display text-xl sm:text-2xl font-bold text-gold-primary">${stats.available_balance?.toFixed(2)}</p>
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {codeData?.code ? (
                <div className="space-y-5">
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute -top-px left-6 right-6 h-px bg-gradient-to-r from-transparent via-gold-primary/40 to-transparent" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <CopyButton
                      text={codeData.code}
                      label={T('copy_code')}
                      className="justify-center py-2.5 px-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-text-secondary hover:text-white hover:bg-white/[0.07] hover:border-white/[0.12]"
                    />
                    <CopyButton
                      text={referralLink}
                      label={T('copy_link')}
                      className="justify-center py-2.5 px-3 rounded-xl bg-gold-primary/10 border border-gold-primary/20 text-gold-primary hover:bg-gold-primary/15"
                    />
                  </div>
                ...
                <div className="text-center py-8">
                  <div className="w-16 h-16 rounded-2xl bg-gold-primary/10 border border-gold-primary/15 flex items-center justify-center mx-auto mb-4">
                    <svg className="w-8 h-8 text-gold-primary/60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4.5v15m7.5-7.5h-15" /></svg>
                  </div>
          ...
          {codeData?.code && (
            <div className="rounded-2xl border border-gold-primary/10 overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)' }}>
              <div className="px-5 py-4 border-b border-white/[0.05]">
                <h2 className="text-sm font-semibold text-white flex items-center gap-2">
                  <svg className="w-4 h-4 text-gold-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.75 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 013.75 9.375v-4.5zm0 9.75c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5a1.125 1.125 0 01-1.125-1.125v-4.5zm9.75-9.75c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0113.5 9.375v-4.5z" /></svg>
                  {T('qr_code')}
                </h2>
              </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {stats && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: T('total_referrals'), value: stats.total_referrals, color: 'text-white', icon: '👥' },
                { label: T('confirmed'), value: stats.confirmed_referrals, color: 'text-emerald-400', icon: '✓' },
                { label: T('pending'), value: stats.pending_referrals, color: 'text-amber-400', icon: '⏳' },
                { label: T('total_earned'), value: `$${stats.total_commission?.toFixed(2)}`, color: 'text-emerald-400', icon: '💰' },
                { label: T('available'), value: `$${stats.available_balance?.toFixed(2)}`, color: 'text-gold-primary', icon: '💎' },
                { label: T('paid_out'), value: `$${stats.total_paid_out?.toFixed(2)}`, color: 'text-blue-400', icon: '📤' },
              ].map((s, i) => (
                <div key={i} className="rounded-xl border border-white/[0.06] p-4 hover:border-white/[0.1] transition-colors" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  {stats?.recent_referrals?.length > 0 ? (
                    <div className="space-y-2">
                      {stats.recent_referrals.map((r, i) => (
                        <div key={r.id || i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-colors">
              ...
              <div className="p-6 space-y-5">
                ...
                {payoutSuccess && (
                  <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                    ...
                    <p className="text-sm text-emerald-400">Payout request submitted successfully! Admin will review shortly.</p>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-text-muted mt-1.5">{T('payout_min')}</p>
                  </div>
                        ...
                        <button
                          ...
                          className={`py-2.5 rounded-xl text-xs font-semibold transition-all border ${
                            payoutNetwork === net
                              ...
                              : 'bg-white/[0.02] border-white/[0.06] text-text-secondary hover:text-white hover:border-white/[0.1]'
                          }`}
                        >
                          {net}
                        </button>
                  ...
                  <div className="space-y-2">
                    {payouts.map((p, i) => (
                      <div key={p.id || i} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                        <div>
                          <p className="text-sm font-semibold text-white">${parseFloat(p.amount_usdt).toFixed(2)}</p>
