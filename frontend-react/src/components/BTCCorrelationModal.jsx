...
const IC = {
  ruler: (
    <>
      ...
      <path d="m7 11 1.5 1.5M11 7l1.5 1.5M11 15l1 1M15 11l1 1" />
    </>
  ),
  ...
  flame: (
    <path d="M12 2c1.2 3 4 4.5 4 8.5A4 4 0 0 1 8 11c0-1.6.7-2.6 1.3-3.3.2 1.4.9 2.3 1.7 2.6-.4-3 .4-5.3 1-8.3z" />
  ),
  clock: (
    <>
...
export default function BTCCorrelationModal({
  signalId,
  pair,
  isOpen,
  onClose,
    ...
    fetch(`/api/v1/signals/${signalId}/btc-correlation`, { headers })
      .then((r) => {
        if (r.status === 404) {
          if (!cancelled)
            setError(
              "Correlation analysis is still being computed by the worker. Try again in a moment."
            );
          return null;
        }
    ...
    <div className="flex items-center gap-2.5 min-w-0">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="flex-shrink-0 rounded border border-ink/12 bg-accent/12 px-1.5 py-0.5 text-[9px] font-bold text-accent">
            vs BTC
          </span>
        </div>
  ...
  return (
    <div className="space-y-3 animate-pulse">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-16 rounded-lg bg-ink/[0.03]" />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="h-32 rounded-lg bg-ink/[0.03]" />
        <div className="h-32 rounded-lg bg-ink/[0.03]" />
      </div>
    </div>
  );
...
function ErrorState({ message }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <span className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-accent/12 text-accent ring-1 ring-accent/25">
  ...
  return (
    <div className="space-y-4">
      ...
      {interpretation?.summary && (
        <div className="rounded-lg border border-ink/5 bg-surface-raised p-3">
          <p className="mb-1.5 text-[9px] font-medium uppercase tracking-wider text-text-muted">
            Summary
          </p>
    ...
    <div className="rounded-xl border border-ink/12 bg-gradient-to-br from-accent/15 to-accent/5 p-4">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="mb-1 text-[9px] font-semibold uppercase tracking-wider text-text-muted">
            BTC Alignment
          </p>
        ...
        <div className="shrink-0 text-right">
          <div className={`font-mono text-3xl font-bold sm:text-4xl ${scoreColor}`}>
            {score ?? "—"}
          </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        <span
          className={`inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[9px] font-bold ${riskStyles[risk]}`}
        >
          <span className="h-1 w-1 rounded-full bg-current" />
          {risk.toUpperCase()} RISK
        </span>
        {is_decoupled && (
          <span className="inline-flex items-center gap-1 rounded border border-ink/12 bg-accent/12 px-2 py-0.5 text-[9px] font-bold text-accent">
        ...
        <span className="inline-flex items-center gap-1 rounded border border-ink/10 bg-ink/[0.03] px-2 py-0.5 text-[9px] font-medium text-text-primary/60">
          {regimeLabel}
        </span>
        {confidence && confidence !== "insufficient_data" && (
          <span className="inline-flex items-center gap-1 rounded border border-ink/10 bg-ink/[0.03] px-2 py-0.5 text-[9px] font-medium text-text-primary/40">
            {confidence} confidence
          </span>
        )}
      </div>
...
function CoreMetricsGrid({ metrics }) {
  return (
    <div>
      <p className="mb-2 text-[9px] font-medium uppercase tracking-wider text-text-muted">
        Core Metrics
      </p>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6">
        <MetricCard
          label="Tail ρ (BTC ↓)"
          value={metrics.tail_corr_btc_down}
          hint="BTC-down corr"
          digits={2}
          signed
        />
        <MetricCard
          label="Tail ρ (BTC ↑)"
          value={metrics.tail_corr_btc_up}
          hint="BTC-up corr"
          digits={2}
          signed
        />
        <MetricCard
          label="Downside β"
          value={metrics.downside_beta}
          hint="vs BTC-down"
          digits={2}
        />
        <MetricCard
          label="Lead/Lag"
          value={metrics.lead_lag_hours}
          hint="Hours"
          digits={0}
          signed
          suffix="h"
        ...
        <MetricCard
          label="Vol Ratio"
          value={metrics.volatility_ratio}
          hint="× BTC"
          digits={2}
          suffix="×"
        />
        <MetricCard
          label="7d Δ vs BTC"
          value={metrics.momentum_divergence_7d}
          hint="Outperformance"
          digits={1}
          signed
          suffix="%"
      ...
      {metrics.coin_volatility_pct != null && (
        <p className="mt-1.5 text-center text-[10px] text-text-muted">
          ...
          <span className="font-mono font-medium text-text-primary">
            {metrics.coin_volatility_pct.toFixed(0)}%
          </span>
        </p>
      )}
    </div>
    ...
    <div>
      ...
      <div className="space-y-1.5">
        {observations.map((obs, i) => (
          <div key={i} className="rounded-lg border border-ink/5 bg-surface-raised px-3 py-2">
    ...
    <div>
      ...
      <div className="space-y-1.5">
        {items.map((item, i) => (
          <div
            key={i}
            className="flex items-start gap-2.5 rounded-lg border border-ink/5 bg-surface-raised px-3 py-2"
          >
            <span className="mt-0.5">
              <IconBadge d={item.d} color={item.color} size={22} />
            </span>
            <p
              className={`text-[11px] leading-relaxed ${item.textClass || "text-text-primary/80"}`}
            >
...
function BtcContextBlock({ btc_context }) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 rounded-lg border border-ink/5 bg-surface-raised p-3 sm:grid-cols-4">
        <SnapshotItem label="Price" value={fmtPrice(btc_context.price)} />
        <SnapshotItem label="RSI 14" value={fmt(btc_context.rsi_14, 1)} />
        <SnapshotItem
          label="24h Δ"
          value={fmtPct(btc_context.change_24h_pct)}
          valueClass={changeColor}
        />
        <SnapshotItem
          label="Dominance"
          value={btc_context.dominance ? `${btc_context.dominance.toFixed(1)}%` : "—"}
        />
      </div>
    </div>
  );
...
function MetaFooter({ sample_size, confidence, data_source, snapshot_at }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 border-t border-ink/5 pt-2 text-[10px] sm:grid-cols-4">
      <div>
...
function MetricCard({ label, value, hint, digits = 2, signed = false, suffix = "" }) {
  ...
  return (
    <div className="rounded-lg border border-ink/5 bg-surface-raised p-2">
