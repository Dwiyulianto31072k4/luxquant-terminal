...
function LoadingSkeleton() {
 ...
 {[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-16 rounded-lg bg-ink/[0.03]" />)}
 </div>
 <div className="grid grid-cols-2 gap-2 sm:grid-cols-6">
 {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="h-16 rounded-lg bg-ink/[0.03]" />)}
 </div>
...
function HeadlineBlock({ interpretation, confidence, is_decoupled, is_extended, btc_context }) {
 ...
 <p className="mb-1 text-[9px] font-semibold uppercase tracking-wider text-text-muted">BTC Alignment</p>
...
function ActionableHints({ interpretation }) {
 ...
 <div className="space-y-1.5">
 {items.map((item, i) => (
 <div key={i} className="flex items-start gap-2.5 rounded-lg border border-ink/5 bg-surface-raised px-3 py-2">
