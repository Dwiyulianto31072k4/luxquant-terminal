    ...
    <span className={`relative inline-flex ${className}`} ref={ref}>
      <button
        ...
        className={`inline-flex items-center justify-center w-3.5 h-3.5 rounded-full border text-[9px] font-mono leading-none transition-colors ${
          open
            ? "border-ink/20 text-accent bg-accent/12"
            : "border-text-muted/40 text-text-muted/70 hover:border-ink/18 hover:text-text-primary"
        }`}
      >
        ?
      </button>
      {open && (
        ...
        >
          <span className="relative block bg-surface-raised border border-ink/15 rounded-lg shadow-2xl p-3 overflow-hidden">
            <span className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-ink/15 to-transparent" />
            {title && (
              <span className="block font-mono text-[10px] uppercase tracking-wider text-accent mb-1">
                {title}
              </span>
            )}
            <span className="block font-mono text-[11px] leading-relaxed text-text-muted">
              {text}
            </span>
          </span>
        </span>
...
const Row = ({ title, desc, warn }) => (
  <div className="py-3.5 border-b border-ink/[0.06] last:border-0">
    {title && (
      <p className="font-mono text-[11px] uppercase tracking-wider text-text-primary mb-1.5">
        {title}
      </p>
    )}
    <p className="font-mono text-[12px] leading-relaxed text-text-muted normal-case tracking-normal">
      {desc}
    </p>
    {warn && (
...
export const GuideModal = ({ onClose }) => {
        ...
        return (
          <>
            <Row
              ...
              desc={t("guide.pattern_d")}
              warn={t("guide.pattern_warn")}
            />
  ...
  const content = (
    ...
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="lq-sheet relative w-full max-w-3xl max-h-[min(var(--lq-modal-maxh),720px)] h-[min(var(--lq-modal-maxh),720px)] sm:h-auto sm:max-h-[min(var(--lq-modal-maxh),640px)] bg-surface-raised border-t border-ink/12 sm:border rounded-t-3xl sm:rounded-xl shadow-[0_-20px_60px_rgb(var(--scrim) / 0.35)] sm:shadow-2xl overflow-hidden flex flex-col"
      >
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="w-48 shrink-0 border-r border-ink/[0.06] bg-ink/[0.015] py-5 px-3 flex flex-col">
            <div className="px-2 mb-4">
              <div className="flex items-center gap-1.5 mb-1">
                <span className="inline-flex items-center justify-center w-4 h-4 rounded-full border border-ink/18 text-accent text-[9px] font-mono">
                  ?
                </span>
                ...
                return (
                  <button
                    ...
                    className={`text-left px-3 py-2.5 rounded-md font-mono text-[11px] tracking-wide transition-all ${
                      on
                        ? "bg-accent/12 border border-ink/12 text-accent"
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-6 py-2">{renderContent()}</div>
          </div>
        </div>
      </div>
    </div>
