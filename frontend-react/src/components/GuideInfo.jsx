...
export const InfoTip = ({ text, title, side = 'top', className = '' }) => {
 ...
 open ? 'border-ink/20 text-accent bg-accent/12' : 'border-text-muted/40 text-text-muted/70 hover:border-ink/18 hover:text-text-primary'
 }`}
 >
 ?
 </button>
 ...
 {title && <span className="block font-mono text-[10px] uppercase tracking-wider text-accent mb-1">{title}</span>}
...
export const GuideModal = ({ onClose }) => {
 ...
 <span className="inline-flex items-center justify-center w-4 h-4 rounded-full border border-ink/18 text-accent text-[9px] font-mono">?</span>
 <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-text-muted">{t('guide.button')}</span>
 </div>
 </div>
