...
const ChartProof = ({ entryChartUrl, latestChartUrl, pair, status, variant = "card" }) => {
  ...
  if (variant === "thumbnail") {
    ...
    return (
      <>
        <div
          className="relative w-16 h-10 rounded-md overflow-hidden cursor-pointer group border border-ink/10 hover:border-ink/18 transition-all flex-shrink-0"
          onClick={(e) => {
            e.stopPropagation();
        ...
        >
          ...
          <div className="absolute inset-0 bg-scrim/0 group-hover:bg-scrim/30 transition-colors flex items-center justify-center">
            <svg
              className="w-3 h-3 text-text-primary opacity-0 group-hover:opacity-100 transition-opacity"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        {/* Lightbox */}
        {lightboxOpen && (
          <Lightbox src={lightboxSrc} pair={pair} onClose={() => setLightboxOpen(false)} />
        )}
      </>
    );
  }
      ...
      <>
        <div className="mt-2.5">
          ...
          {currentUrl && !imgError[currentUrl] && (
            <div
              className="relative rounded-lg overflow-hidden border border-ink/08 cursor-pointer group"
              onClick={(e) => {
                e.stopPropagation();
  ...
  if (variant === "modal") {
    return (
      <div className="space-y-3">
        ...
        {currentUrl && !imgError[currentUrl] ? (
          <div
            className="relative rounded-xl overflow-hidden border border-ink/08 cursor-pointer group"
          ...
          <div className="glass-card rounded-xl p-12 border border-ink/08 text-center">
            ...
            <p className="text-text-muted/50 text-xs mt-1">
              Will be captured when a new signal comes in
            </p>
          </div>
        )}
...
const Lightbox = ({ src, pair, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {/* Pair name */}
      <div className="absolute top-4 left-4 text-text-primary/60 text-sm font-mono">{pair}</div>
