...
// Contains no per-message identifiers.`,
  ...
  },
  {
    ...
    fields: [
      ...
      ["coin.outcome_dist", "Outcome counts by bucket: { tp1, tp2, tp3, tp4, sl }."],
      [
      ...
      ["coin.monthly_trend", "Per-month win-rate history: { month, wr, closed, wins }."],
      [
 ...
 "computed_at": "2026-06-16T09:49:12+00:00"
  ...
  },
  {
    ...
    summary: "Win-rate trend over time (for charts): per-period totals plus an overall summary.",
    params: [
      [
        "time_range",
        "string",
 ...
 "time_range": "7d"
  ...
  },
  {
    ...
    params: [["date", "string", "no", "Target day, YYYY-MM-DD (UTC). Defaults to today (UTC)."]],
    fields: [
      ...
      ["trend_14d[]", "Trailing 14 days: { date, total, wins, losses, win_rate, regime }."],
    ],
    example: `{
...
const DocSection = ({ id, title, children }) => (
  <section id={id} className="scroll-mt-24">
    ...
    <div className="text-text-secondary text-[13px] leading-relaxed space-y-2 pl-3">{children}</div>
  </section>
);
        ...
        {rows.map(([name, type, req, desc]) => (
          <tr key={name} className="border-t border-ink/[0.05] align-top">
            <td className="py-2 pr-3">
              <code className="font-mono text-[11px] text-accent whitespace-nowrap">{name}</code>
            </td>
  ...
  >
    ...
    >
      <div className="flex items-center gap-2 flex-wrap min-w-0">
        ...
        <code className="font-mono text-[13px] text-accent break-all">{ep.path}</code>
      </div>
      <svg
    ...
    <div className="max-w-6xl mx-auto px-1 sm:px-2 lg:px-0 space-y-6">
      ...
      {!hasAccess && (
        <div
          ...
          style={{
            background:
              "linear-gradient(160deg, rgb(var(--accent) / 0.08), rgb(var(--ink) / 0.01))",
          }}
        >
      ...
      {justCreated && (
        <div
          ...
          style={{
            background:
              "linear-gradient(160deg, rgb(var(--accent) / 0.10), rgb(var(--ink) / 0.01))",
          }}
        >
          ...
          <p className="text-accent/90 text-[12px] mb-3">⚠ {t("apiKeys.created_warn")}</p>
      ...
      <section className="rounded-2xl border border-ink/5 bg-ink/[0.02] overflow-hidden">
        ...
        {hasAccess && showCreate && (
          <div className="px-5 py-4 border-b border-ink/5 bg-ink/[0.01]">
            ...
            {atLimit && (
              <p className="text-accent/80 text-[11px] mt-2">{t("apiKeys.limit_warn")}</p>
            )}
          </div>
        )}
      ...
      <div className="rounded-2xl p-5 sm:p-6 border border-ink/5 bg-ink/[0.02]">
        ...
        <div className="pb-3 mb-4 border-b border-ink/5">
          ...
          <p className="text-text-muted text-[13px] mt-1.5 leading-relaxed">
            Comprehensive documentation to integrate LuxQuant’s proprietary AI-powered trading
            algorithm and advanced quantitative market intelligence into your applications, trading
            bots, dashboards, and quantitative systems. Core endpoints deliver precise, real-time
            data with stable schemas, while our enriched analytics layers provide deep insights
            generated by our 24/7 market-scanning algorithm — engineered for seamless integration
            into professional trading infrastructure.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[190px_minmax(0,1fr)] gap-6">
          ...
          <div className="min-w-0 space-y-7">
            ...
            <DocSection id="doc-auth" title="Authentication">
              ...
              />
              <p>
                Alternatively you may send it as <Mono>X-API-Key: lq_live_YOUR_KEY</Mono>. The key
                is shown only once at creation — store it like a password. If it leaks, revoke it
                from this page and generate a new one.
              </p>
              <p className="text-text-muted text-[12px]">
                Access requires an active subscription. If your subscription lapses, the key stops
                working automatically until it is renewed.
              </p>
            </DocSection>
            ...
            <DocSection id="doc-base" title="Base URL">
              ...
              />
              <p>
                All endpoints below are relative to this base. All responses are JSON (except{" "}
                <Mono>/export/prompt</Mono>, which is plain text).
              </p>
            </DocSection>
            ...
            <DocSection id="doc-rate" title="Rate limits">
              <p>
                Requests are limited to <Mono>{RATE_LIMIT}/min per account</Mono> (a sliding
                60-second window), shared across all of your keys. Each response includes:
              </p>
              <ul className="list-none space-y-1 pl-1">
                ...
                <li>
                  · <Mono>X-RateLimit-Remaining</Mono> — requests left in the current window.
                </li>
              </ul>
              <p>
                When the limit is exceeded you get HTTP <Mono>429</Mono> with a{" "}
                <Mono>Retry-After</Mono> header. Polling every 10–15 seconds keeps you comfortably
                within the limit.
              </p>
            </DocSection>
            ...
            <DocSection id="doc-format" title="Response format & forward pagination">
              ...
              <p>
                To follow new data over time, save <Mono>cursor</Mono> from each response and pass
                it back as the <Mono>since</Mono> parameter on the next call. With{" "}
                <Mono>since</Mono>, results come oldest-first so nothing is skipped; without it, you
                get the newest items first.
              </p>
              <p className="text-text-muted text-[12px]">
            ...
            <DocSection id="doc-codes" title="Status & error codes">
              <ul className="list-none space-y-1.5">
                <li>
                  <span className="text-profit font-mono text-[12px]">200</span> — OK. Body contains
                  the requested data.
                </li>
                <li>
                  <span className="text-accent font-mono text-[12px]">400</span> — Bad request, e.g.
                  an invalid <Mono>status</Mono> value. The body lists what's valid.
                </li>
                <li>
                  <span className="text-loss font-mono text-[12px]">401</span> — Missing / invalid /
                  revoked key.
                </li>
                <li>
                  <span className="text-loss font-mono text-[12px]">403</span> — Key valid but
                  subscription inactive.
                </li>
                <li>
                  <span className="text-loss font-mono text-[12px]">404</span> — Resource not found
                  (or outside the public data window).
                </li>
                <li>
            ...
            <DocSection id="doc-ids" title="Working with signal_id">
              <p>
                ...
                <Mono>{`/enrichment/{id}`}</Mono>, and <Mono>{`/btc-correlation/{id}`}</Mono> expect
                a <Mono>signal_id</Mono> — <span className="text-text-primary">not</span> a pair
                name like <Mono>BTCUSDT</Mono>.
              </p>
              ...
              <ol className="list-decimal pl-5 space-y-1 text-[12px]">
                ...
                <li>
                  Take <Mono>signal_id</Mono> from any item in the response (a UUID like{" "}
                  <Mono>cbc5315b-3910-…</Mono>).
                </li>
                <li>
            ...
            <DocSection id="doc-endpoints" title="Endpoints">
              <p className="text-text-muted text-[12px] -mt-1">
                Tap a card to expand parameters, response fields, and an example. Signals endpoints
                have exact schemas; analytics endpoints show exact field names with illustrative
                values.
              </p>
            ...
            <DocSection id="doc-status" title="Signal status values">
              <p>
                The <Mono>status</Mono> field on a signal — and the values accepted by{" "}
                <Mono>?status=</Mono> on <Mono>/signals</Mono>:
              </p>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <tbody>
                    {STATUS_VALUES.map(([val, desc]) => (
                      <tr key={val} className="border-t border-ink/[0.05] align-top">
            ...
            <DocSection id="doc-examples" title="Code examples">
              <p className="text-text-muted text-[12px]">
                Same task in three languages — list open signals, then drill into one. Switch tabs
                to copy your preferred language.
              </p>
              <CodeTabs
                tabs={[
              ...
              <p className="text-text-muted text-[12px] mt-1">
                ...
                <Mono>/signals/updates</Mono> with a cursor on a 15s interval rather than
            ...
            <DocSection id="doc-best" title="Best practices & FAQ">
              <ul className="list-none space-y-2">
                <li className="flex items-start gap-2">
                  <span className="text-text-muted mt-0.5">·</span>
                  <span>
                    <span className="text-text-primary">Poll, don't hammer.</span> 10–15s intervals
                    are plenty and stay within the 60/min limit.
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-text-muted mt-0.5">·</span>
                  <span>
                    <span className="text-text-primary">Use the cursor.</span> Re-fetching
                    everything wastes your rate budget; <Mono>since</Mono> only returns what's new.
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-text-muted mt-0.5">·</span>
                  <span>
                    <span className="text-text-primary">Handle "not ready" states.</span> Analytics
                    endpoints can return <Mono>{`{ available: false }`}</Mono> /{" "}
                    <Mono>{`{ status: "not_enriched" }`}</Mono> for very new signals — treat that as
                    a normal, non-error response.
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-text-muted mt-0.5">·</span>
                  <span>
                    <span className="text-text-primary">Keep the key server-side.</span> Don't embed
                    it in a browser/client app where others can read it.
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-text-muted mt-0.5">·</span>
                  <span>
                    <span className="text-text-primary">One identity per key.</span> Sharing or
                    reselling access can get the key flagged and revoked.
                  </span>
                </li>
              </ul>
            </DocSection>
