...
export const humanizeTrigger = (reason) => {
  ...
  const parts = String(reason)
    ...
    .map((raw) => {
      ...
      if (r === "funding_flip_neg_to_pos")
        return "funding flipped negative → positive (longs now pay)";
      if (r === "funding_flip_pos_to_neg")
        return "funding flipped positive → negative (shorts now pay)";
      if ((m = r.match(/^funding_spike_([+-][\d.]+)%$/))) return `funding jumped to ${m[1]}%`;
...
export const dirMeta = (direction) => {
  ...
  if (k === "up")
    return {
      ...
      text: "text-positive",
      hex: COLOR.profit,
      tag: "bg-positive/10 text-positive border-positive/20",
      bar: "bg-positive",
    };
  if (k === "down")
    return {
      ...
      text: "text-negative",
      hex: COLOR.loss,
      tag: "bg-negative/10 text-negative border-negative/20",
      bar: "bg-negative",
    };
  return {
    k,
    ...
    hex: COLOR.flat,
    tag: "bg-ink/[0.05] text-text-secondary border-ink/10",
    bar: "bg-ink/30",
  };
};
...
export const StatCard = ({ label, value, detail, tone = "neutral", big = false }) => {
  ...
  return (
    <div className={`rounded-lg border p-3.5 md:p-4 ${tones[tone] || tones.neutral}`}>
      ...
      <div
        className={`mt-1.5 font-mono ${big ? "text-3xl" : "text-[22px] md:text-2xl"} font-semibold tabular-nums tracking-tight leading-none ${valueTone}`}
      >
        {value}
      </div>
    ...
    <div>
      <div className="relative h-2.5 overflow-hidden rounded-full border border-ink/[0.06] bg-ink/[0.03]">
        ...
        <div
          ...
          style={{
            width: `${pct}%`,
            background: `linear-gradient(90deg, ${meta.hex}66, ${meta.hex})`,
          }}
        />
      </div>
...
export const StanceGauge = ({ value = 0, dir = "up", size = 168 }) => {
  ...
  const arc = (from, to) => `M ${px(from)} ${py(from)} A ${r} ${r} 0 0 1 ${px(to)} ${py(to)}`;
  return (
            ...
            style={
              meta.k === "up"
                ...
                : {
                    right: "50%",
                    width: `${half}%`,
                    background: `linear-gradient(270deg, ${meta.hex}55, ${meta.hex})`,
                  }
            }
          />
        )}
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
        <span className="font-display text-[26px] font-bold leading-none text-text-primary">
          {centerValue}
        </span>
        {centerLabel && (
