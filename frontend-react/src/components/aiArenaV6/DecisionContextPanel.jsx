  ...
  return (
    <div className="min-w-0 rounded-xl border border-ink/[0.07] bg-ink/[0.02] p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className={`text-[9px] font-mono uppercase tracking-wider ${supportTone[support] || supportTone.unavailable}`}>
          {support}
        </span>
      </div>
...
function ChangeText({ change }) {
  ...
  Object.entries(change.horizons || {}).forEach(([horizon, detail]) => {
    if (
      ...
      parts.push(
        `${horizon} ${detail.direction_from} to ${detail.direction_to}`,
      );
    } else if (
      detail.strength_delta != null
      && Math.abs(Number(detail.strength_delta)) >= 0.1
    ) {
...
export default function DecisionContextPanel({ data }) {
  ...
  return (
    <section className="min-w-0 rounded-2xl border border-line/15 bg-surface-secondary/90 p-4 md:p-6">
        ...
        <div className="min-w-0 rounded-xl border border-ink/[0.06] bg-scrim/10 p-4">
          ...
          {issues.length ? (
            ...
            <div className="mt-3 text-xs text-emerald-300/70">
              No active data-quality warning.
            </div>
          )}
        </div>
