  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Segmented
          options={[
            { value: "7d", label: "Swing · 7d" },
            { value: "30d", label: "Holder · 30d" },
          ]}
          value={view}
          onChange={setView}
        />
          ...
          <div className="min-w-0 space-y-4 xl:col-span-8">
            ...
            <Card className="p-5 md:p-6" accent="gold">
              ...
              <p className="mb-4 max-w-[70ch] text-[13.5px] leading-relaxed text-text-muted">
                Valuation and macro gauges describing where this cycle sits. They move slowly — read
                them as context for the 24h and 7d calls, not as triggers.
              </p>
            ...
            <div className="space-y-4 xl:sticky xl:top-[64px]">
              <Card className="p-5">
                <SectionHeader
                  ...
                  right={
                    data?.cycle?.phase ? <Tag tone="gold">{readable(data.cycle.phase)}</Tag> : null
                  }
                />
                <div className="grid grid-cols-2 gap-2.5">
                  <Tile label="Cycle score">
                    <Num className="text-[20px] text-text-primary">
                      {data?.cycle?.score != null ? data.cycle.score : "—"}
                    </Num>
                  </Tile>
                  <Tile label="Phase">
                    <span className="font-display text-[14px] font-semibold text-text-primary">
                      {readable(data?.cycle?.phase)}
                    </span>
                  </Tile>
                  {outlook.slice(0, 2).map((r) => {
                    ...
                    return (
                      <Tile key={r.key} label={r.label}>
                        <span className={`font-display text-[14px] font-semibold ${m.text}`}>
                          {m.arrow} {m.label}
                        </span>
                      </Tile>
                    );
