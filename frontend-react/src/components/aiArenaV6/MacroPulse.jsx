...
const AssetCard = ({ config, asset }) => {
  ...
  return (
    <div className="bg-bg-card/50 rounded-xl p-4 border border-ink/5 hover:border-ink/10 transition-colors">
      ...
      <div className={`text-[11px] font-mono font-bold mt-1 ${changeColor(change1d)}`}>
        {fmtPct(change1d)} <span className="text-text-muted font-normal">1D</span>
      </div>
...
export default function MacroPulse() {
  ...
  const narrative = (() => {
    ...
    if (spxCorr != null && Math.abs(spxCorr) >= 0.7) {
      ...
      parts.push(
        `BTC shows moderate ${spxCorr >= 0 ? "positive" : "negative"} correlation with SPX (${fmtCorr(spxCorr)})`
      );
    }
