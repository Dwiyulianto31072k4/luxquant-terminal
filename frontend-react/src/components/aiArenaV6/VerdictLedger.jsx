 ...
 * ======================================================
 * THE killer feature of v6. Shows:
 *
 *   Top: 4 stat cards per horizon (24h / 72h / 7d / 30d)
 *        - hit rate %
 *        - sample size (hits/total)
 *        - directional accuracy bar
 *
 *   Bottom: scrollable history table
 *        - timestamp, BTC price, primary/secondary/tactical verdicts
 *        - outcome badges per horizon (hit/miss/pending/expired)
 *        - color-coded for quick scan
 *
 * Props:
 *   trackRecord — from /v6/track-record { window_days, by_horizon: {...}, total_evaluated }
 *   ledger — from /v6/ledger { reports: [{ ...report, outcomes: [...] }] }
 *
 * Both can be null while loading; component handles gracefully.
 */
...
function OutcomeBadge({ outcome }) {
  ...
  return (
      ...
      content={
        <div className="space-y-1 text-xs">
          <div>
            ...
            <span className="text-white">{outcome.direction}</span>
          </div>
          <div>
            <span className="text-white/50">Called at:</span>{" "}
            <span className="text-white font-mono">
              ${formatPrice(outcome.price_at_call)}
            </span>
          </div>
          {outcome.price_at_horizon != null && (
            <div>
              <span className="text-white/50">Horizon close:</span>{" "}
              <span className="text-white font-mono">
                ${formatPrice(outcome.price_at_horizon)}
              </span>
            </div>
          )}
    ...
    >
      <span
        className="inline-flex items-center justify-center w-7 h-5 rounded text-[10px] font-mono uppercase font-bold cursor-help"
        style={{
...
function HistoryRow({ report }) {
  ...
  const primaryDir = directionStyle(report.primary_direction_30d);
  const secondaryDir = directionStyle(report.secondary_direction_7d);
  const tacticalDir = directionStyle(report.tactical_direction_24h);
  ...
  return (
    <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
      ...
      <td className="py-2.5 px-3 whitespace-nowrap">
        ...
        >
          {primaryDir.arrow} {report.primary_confidence_30d ?? "—"}%
        </span>
      </td>

      {/* 7d secondary verdict */}
      <td className="py-2.5 px-3 whitespace-nowrap">
        <span
          className="inline-flex items-center gap-1 text-xs font-mono px-1.5 py-0.5 rounded"
          style={{ backgroundColor: secondaryDir.bg, color: secondaryDir.fg }}
        >
          {secondaryDir.arrow} {report.secondary_confidence_7d ?? "—"}%
        </span>
      </td>
      ...
      <td className="py-2.5 px-3 whitespace-nowrap">
        ...
        >
          {tacticalDir.arrow} {report.tactical_confidence_24h ?? "—"}%
        </span>
      </td>
...
export default function VerdictLedger({ trackRecord, ledger }) {
  ...
  const filtered = useMemo(() => {
    ...
    return reports.filter((r) => {
      if (!r.outcomes) return false;
      return r.outcomes.some((o) => o.outcome === filter);
    });
  }, [reports, filter]);

  const hasAnyEvaluated = trackRecord?.total_evaluated > 0;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {HORIZON_ORDER.map((h) => (
          <HorizonStatCard
            key={h}
            horizon={h}
            stats={trackRecord?.by_horizon?.[h]}
          />
        ))}
      </div>
        ...
        {filtered.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {filtered.map((report) => (
                  <HistoryRow key={report.id || report.report_id} report={report} />
                ))}
              </tbody>
            </table>
          </div>
