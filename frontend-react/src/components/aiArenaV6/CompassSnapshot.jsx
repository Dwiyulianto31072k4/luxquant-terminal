        ...
        {[
          ...
          >
            <span
              ...
              style={{ color: `${m.hex}AA` }}
            >
              {m.label}
            </span>
            <span
              className="mt-0.5 font-mono text-[12px] font-semibold tabular-nums leading-none"
              style={{ color: m.hex }}
            >
              {fmtUsd(m.price)}
            </span>
    ...
    <>
      ...
      {target && spot ? (
        <>
          ; path points toward{" "}
          <Hi tone="up">
            {fmtUsd(target)} ({fmtPct(((target - spot) / spot) * 100)})
          </Hi>
        </>
      ) : null}
      {invalidation && spot ? (
        <>
          ...
          <Hi tone="down">
            {fmtUsd(invalidation)} ({fmtPct(((invalidation - spot) / spot) * 100)})
          </Hi>
        </>
      ) : null}
      .
      ...
      <div className="flex flex-col gap-2.5 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between md:px-5 md:py-3">
        ...
        >
          ...
          <span className="flex min-w-0 flex-wrap items-center gap-x-2.5 gap-y-1">
            ...
            <span
              className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-md border px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.1em] ${dir.tag}`}
            >
              {dir.arrow} {dir.label}
          ...
          <div className="grid gap-x-6 gap-y-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(260px,1fr)]">
            ...
            <div className="flex items-stretch">
              <div className="w-full rounded-lg border border-ink/[0.08] bg-ink/[0.03] px-4 py-3">
                ...
                <div className="mt-1 text-[16px] font-semibold leading-tight text-text-primary">
                  {mode}
                </div>
