...
const FollowupHeader = ({ onCreate, onGenerate, generating }) => (
 ...
 style={{ background: 'rgba(138,138,147,0.10)', color: '#8a8a93', border: '1px solid rgba(138,138,147,0.28)' }}
 onMouseEnter={(e) => { if (!generating) e.currentTarget.style.background = 'rgba(138,138,147,0.18)'; }}
 onMouseLeave={(e) => { if (!generating) e.currentTarget.style.background = 'rgba(138,138,147,0.10)'; }}
 >
 <SparkIcon size={13} />
...
const StatCard = ({ label, value, accent, Icon, active, onClick, alert }) => (
 ...
 onMouseEnter={(e) => { if (!active) e.currentTarget.style.borderColor = 'rgb(var(--accent) / 0.25)'; }}
 onMouseLeave={(e) => { if (!active) e.currentTarget.style.borderColor = 'rgb(var(--ink) / 0.07)'; }}
 >
 <div
 className="absolute inset-x-0 top-0 h-px pointer-events-none"
 style={{ background: `linear-gradient(to right, transparent, rgb(var(--accent) / ${active ? 0.4 : 0.2}), transparent)` }}
 />
...
export const FollowupTab = ({ onRefreshStats }) => {
 ...
 (f) => (f.status === 'pending' || f.status === 'in_progress') && new Date(f.due_date) < new Date()
 ).length,
 ...
 <StatCard label="Open" value={counts.open} accent="#8a8a93" Icon={ClockIcon}
 active={statusFilter === 'open'} onClick={() => setStatusFilter('open')} />
 <StatCard label="Overdue" value={counts.overdue} accent="#f87171" Icon={AlertTriangleIcon}
 active={statusFilter === 'overdue'} onClick={() => setStatusFilter('overdue')} alert={counts.overdue > 0} />
 <StatCard label="Due Today" value={counts.today} accent="#fb923c" Icon={ClockIcon}
 active={false} onClick={() => {}} />
 <StatCard label="Done" value={counts.done} accent="#34d399" Icon={CheckCircleIcon}
 active={statusFilter === 'done'} onClick={() => setStatusFilter('done')} />
 </div>
 ...
 {followups.map((f) => (
 <FollowupCard key={f.id} followup={f} onEdit={handleEdit} onStatusChange={handleStatusChange} onDelete={handleDelete} />
 ))}
 </div>
 )}
