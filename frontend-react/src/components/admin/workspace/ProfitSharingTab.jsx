...
const Stat = ({ label, value, color, sub }) => (
  <div
    ...
    style={{ background: "rgb(var(--surface-raised))", border: `1px solid ${tint(color, 0.18)}` }}
  >
    <div
    ...
    <div className="text-[20px] font-bold mt-1 tabular-nums" style={{ color }}>
      {value}
    </div>
    {sub && (
...
export const ProfitSharingTab = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setData(await workspaceApi.getProfitSharing({ from, to }));
    } catch (e) {
      console.error("profit-sharing load failed", e);
    } finally {
      setLoading(false);
    }
  }, [from, to]);
  ...
  const retag = useCallback(
    async (paymentId, source) => {
      setBusy((b) => ({ ...b, [paymentId]: true }));
      try {
        await workspaceApi.setPaymentPartnerSource(paymentId, source);
        await load();
      } catch (e) {
        window.alert("Tag failed: " + (e?.response?.data?.detail || e.message));
      } finally {
        setBusy((b) => {
          const n = { ...b };
          delete n[paymentId];
  ...
  const exportCsv = useMemo(
    () => () => {
      ...
      a.href = url;
      a.download = `luxquant_profit_sharing_${from}_to_${to}.csv`;
      a.click();
    ...
    <div>
      ...
      <div className="flex flex-wrap items-end gap-3 mb-4">
        <div>
          ...
          <input
            ...
            className="text-[12px] rounded-md px-2 py-1.5 outline-none"
            style={{
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[11px] font-semibold disabled:opacity-40 ml-auto"
          style={{
            background: tint(palette.green[400], 0.1),
            border: `1px solid ${tint(palette.green[400], 0.28)}`,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 mb-4">
        <Stat
          ...
          color={palette.gold[300]}
          sub={`${rows.length} transaksi`}
        />
        <Stat
          label="Bagian kamu"
          value={money(totals.owner)}
          color={palette.green[400]}
          sub="owner"
        />
        <Stat
          label="Bagian bigstar"
          value={money(totals.bigstar)}
          ...
          {Object.entries(byScheme).map(([sc, v]) => (
            <div
              ...
              style={{
                ...
                border: `1px solid ${tint(SCHEME_COLOR[sc] || palette.warm[400], 0.25)}`,
              }}
            >
              <span
                className="font-semibold"
                style={{ color: SCHEME_COLOR[sc] || "rgb(var(--fg-secondary))" }}
              >
                {SCHEME_LABEL[sc] || sc}
              </span>
              <span style={{ color: "rgb(var(--fg-muted))" }}>
                {" "}
                · {v.count} tx · gross {money(v.gross)} · kamu {money(v.owner)} · bigstar{" "}
                {money(v.bigstar)}
              </span>
            </div>
          ))}
      ...
      {loading && !data ? (
        ...
        <div className="text-center py-16 text-sm" style={{ color: "rgb(var(--fg-muted))" }}>
          Tidak ada pembayaran confirmed di periode ini.
        </div>
      ) : (
        <div
          className="overflow-x-auto rounded-xl"
          style={{ border: `1px solid ${tint(palette.warm[100], 0.1)}` }}
        >
            ...
            <thead>
              ...
              >
                {["Tgl", "User", "Metode", "Skema", "Gross", "Sam", "Kamu", "Bigstar"].map((h) => (
                  <th key={h} className="text-left font-semibold px-2.5 py-2 whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td
                    className="px-2.5 py-1.5 truncate max-w-[140px]"
                    style={{ color: "rgb(var(--fg))" }}
                  >
                    {r.username || `#${r.user_id}`}
                  </td>
                  <td className="px-2.5 py-1.5" style={{ color: "rgb(var(--fg-muted))" }}>
                    {r.method}
                  </td>
                  <td className="px-2.5 py-1.5">
                    <select
                      ...
                      style={{
                        background: "rgb(var(--surface-secondary))",
                        border: `1px solid ${tint(SCHEME_COLOR[r.scheme] || palette.warm[400], 0.3)}`,
                        color: SCHEME_COLOR[r.scheme] || "rgb(var(--fg-secondary))",
                      }}
                    >
                      <option
                        value="regular"
                  ...
                  <td className="px-2.5 py-1.5 tabular-nums" style={{ color: palette.gold[300] }}>
                    {money(r.gross)}
                  </td>
                  <td
                    className="px-2.5 py-1.5 tabular-nums"
                    style={{ color: r.external ? palette.teal[400] : "rgb(var(--fg-muted))" }}
                  >
                    {r.external ? money(r.external) : "—"}
                  </td>
                  <td
                    className="px-2.5 py-1.5 tabular-nums font-semibold"
                    style={{ color: palette.green[400] }}
                  >
                    {money(r.owner)}
                  </td>
                  <td className="px-2.5 py-1.5 tabular-nums" style={{ color: palette.blue[400] }}>
                    {money(r.bigstar)}
                  </td>
                </tr>
              ))}
