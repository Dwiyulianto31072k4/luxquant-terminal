...
const CATFN = {
  ...
  "Market Data":
    "Fetches and renders market data: liquidations, metadata, charts, PnL, money flow.",
  Distribution: "Delivers signals, alerts and posts out to Telegram, Discord and X.",
  Discord: "Discord-facing bots and relays.",
  News: "Crypto news curation and posting.",
...
const hstatus = (l) =>
  l.some((x) => x.health === "down")
    ? "down"
    : l.some((x) => x.health === "warn")
      ? "warn"
      : l.every((x) => x.health === "idle")
        ? "idle"
        : "ok";
const fmtUptime = (s) => {
  if (s == null) return "—";
    ...
    <div style={{ position: "relative", width: size, height: size }}>
      ...
      <div
        style={{
          ...
          height: size,
          borderRadius: "50%",
          background: hex(c, 0.16),
          border: `2px solid ${hex(c, 0.72)}`,
      ...
      >
        {data.count != null ? (
          <span style={{ color: "rgb(var(--fg))", fontWeight: 700, fontSize: size > 52 ? 16 : 14 }}>
            {data.count}
          </span>
        ) : (
...
function buildGraph(topo) {
  ...
  const clusterOf = (name) => {
    const n = byName[name];
    return n && !["Infrastructure", "Core API"].includes(n.category) ? "cat:" + n.category : name;
  };
  ...
  const tiers = [
    {
      x: 0,
      label: "External APIs",
      items: externals.map((e) => ({ id: e.id, kind: "ext", ext: e })),
      size: 42,
    },
    {
      x: 320,
      label: "Infrastructure",
      items: infra.map((n) => ({ id: n.name, kind: "node", node: n })),
      size: 50,
    },
    {
      x: 620,
      label: "Core",
      items: core.map((n) => ({ id: n.name, kind: "node", node: n })),
      size: 62,
    },
    {
      x: 940,
      label: "Worker clusters",
      items: clusters.map((c) => ({ id: "cat:" + c, kind: "cluster", cluster: c })),
      size: 58,
    },
  ];
  ...
  tiers.forEach((t, ti) => {
    ...
    t.items.forEach((it, i) => {
      ...
      if (it.kind === "ext") {
        ...
        d = {
          title: it.cluster.replace(" / Cryptobot", ""),
          sub: `${list.length} services`,
          color: CATCOLOR[it.cluster] || C.idle,
          health,
          count: list.length,
          size: t.size,
        };
        ...
        d = {
          ...
          sub:
            nn.kind === "timer"
              ? "timer"
              : nn.uptime_seconds != null
                ? "up " + fmtUptime(nn.uptime_seconds)
                : nn.active_state || "inactive",
          color: cById(health),
          health,
          size: t.size,
        };
...
const Pill = ({ h }) => {
  ...
  return (
    <span className="lqd-pill" style={{ background: hex(m[1], 0.14), color: m[1] }}>
      {m[0]}
    </span>
  );
};
...
const ConnRow = ({ name, type, dir }) => {
  ...
  return (
    <div className="lqd-conn">
      <span
        className="lqd-line"
        style={{ borderTopColor: v.c, borderTopStyle: v.flow ? "dashed" : "solid" }}
      />
      {dir === "out" ? "→" : "←"} {name}
...
function DetailModal({ selectedId, topo, onClose, onAction }) {
  ...
  const connFor = (id) => {
    const out = edges
      .filter((e) => e.from === id)
      .map((e, i) => <ConnRow key={"o" + i} name={nameOf(e.to)} type={e.type} dir="out" />);
    const inc = edges
      .filter((e) => e.to === id)
      .map((e, i) => <ConnRow key={"i" + i} name={nameOf(e.from)} type={e.type} dir="in" />);
    const all = out.concat(inc);
    return all.length ? (
      all
    ) : (
      <div className="lqd-conn" style={{ color: "rgb(var(--fg-muted))" }}>
  ...
  if (view.kind === "cluster") {
    ...
    body = (
      <>
        ...
        <div style={{ margin: "10px 0" }}>
          <Pill h={hstatus(list)} />{" "}
          <span style={{ color: "rgb(var(--fg-muted))", fontSize: 12 }}>
        ...
        {list.map((n) => (
          <div
            key={n.name}
            className="lqd-svc"
            onClick={() => setView({ kind: "service", id: n.name })}
          >
            <div className="lqd-row">
              <span className="lqd-st">
                <span className="lqd-dot" style={{ background: cById(n.health) }} />
                {n.name}
              </span>
              <Pill h={n.health} />
            </div>
    ...
    body = (
      <>
        <h3>{e.name}</h3>
        <div className="lqd-sub">{e.fn || ""}</div>
        <div className="lqd-caps">Used by</div>
        {connFor(e.id)}
      </>
    );
  } else {
    ...
    if (!n) return null;
    const backToCluster =
      view.kind === "service" && n.category && !["Infrastructure", "Core API"].includes(n.category);
    body = (
      <>
        {backToCluster && (
          <button className="lqd-back" onClick={() => setView({ kind: "cluster", id: n.category })}>
            ← {n.category}
          </button>
        )}
        <h3>{n.name}</h3>
        ...
        <div className="lqd-statwrap">
          ...
          <div className="lqd-kv">
            <span>uptime</span>
            <span>{n.uptime_seconds != null ? fmtUptime(n.uptime_seconds) : "—"}</span>
          </div>
          <div className="lqd-kv">
            <span>memory</span>
            <span>{fmtBytes(n.memory_bytes)}</span>
          </div>
          <div className="lqd-kv">
            <span>pid</span>
            <span>{n.main_pid || "—"}</span>
          </div>
          <div className="lqd-kv">
            <span>restarts</span>
            <span>{n.restarts || 0}</span>
          </div>
        </div>
        <div className="lqd-caps">Connections</div>
        {connFor(n.name)}
        <div className="lqd-acts">
          <button
            className="lqd-cbtn"
            style={{ borderColor: hex(C.gold, 0.3), color: C.gold }}
            onClick={() => onAction(n.unit, "restart")}
          >
            ↻ Restart
          </button>
          {n.active_state === "active" ? (
            <button
              className="lqd-cbtn"
              style={{ borderColor: hex(C.down, 0.3), color: C.down }}
              onClick={() => onAction(n.unit, "stop")}
            >
              ⊘ Stop
            </button>
          ) : (
            <button
              className="lqd-cbtn"
              style={{ borderColor: hex(C.ok, 0.3), color: C.ok }}
              onClick={() => onAction(n.unit, "start")}
            >
              ⚡ Start
            </button>
          )}
...
function MapFlow({ topo, onAction }) {
  ...
  const onNodeEnter = useCallback(
    (_e, node) => {
      if (node.type !== "tier") applyFocus(node.id);
    },
    [applyFocus]
  );
...
.lqf-box::before{content:"";position:absolute;inset:0 0 auto 0;height:1px;background:linear-gradient(to right,transparent,rgb(var(--accent) / .45),transparent);z-index:5;pointer-events:none}
...
.lqf-box .react-flow__controls-button svg{fill:rgb(var(--accent));max-width:14px;max-height:14px}
...
.lqd-svc:hover{border-color:rgb(var(--accent) / .35);background:rgb(var(--ink) / .04);transform:translateY(-1px)}
...
export default function SystemMap() {
  ...
  const load = useCallback(async () => {
    try {
      setTopo(await workspaceApi.getServicesTopology());
      setError(null);
    } catch (e) {
  ...
  const onAction = useCallback(
    async (unit, action) => {
      ...
      try {
        const r = await workspaceApi.controlService(unit, action);
        if (!r.ok) window.alert(`${verb} failed:\n${r.message || "unknown"}`);
        await load();
      } catch (e) {
  ...
  if (loading && !topo)
    return (
      <div style={{ padding: "60px 0", textAlign: "center", color: "rgb(var(--fg-muted))" }}>
        Loading topology…
      </div>
    );
  if (error)
  ...
  if (topo && topo.available === false)
    return (
      <div style={{ padding: "40px 0", textAlign: "center", color: "rgb(var(--fg-muted))" }}>
        {topo.reason || "Topology unavailable."}
      </div>
    );
  if (!topo) return null;
