...
import { TrendingUpIcon, ClockIcon, AlertTriangleIcon } from "../../Icons";
...
const KpiTile = ({
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (isClickable && !active) e.currentTarget.style.borderColor = "rgb(var(--accent) / 0.25)";
      }}
      onMouseLeave={(e) => {
        if (isClickable && !active) e.currentTarget.style.borderColor = "rgb(var(--ink) / 0.07)";
      }}
    >
...
export const FinanceStatsGrid = ({ stats, statusFilter, onFilterToggle, loading }) => {
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
        ...
        <KpiTile
          ...
          sub={stats?.failed_value > 0 ? formatUSDTCompact(stats.failed_value) : "No failures"}
          accent="#8a8a93"
          Icon={XCircleIcon}
...
const Divider = () => (
  <span aria-hidden className="h-3 w-px" style={{ background: "rgb(var(--ink) / 0.08)" }} />
);
