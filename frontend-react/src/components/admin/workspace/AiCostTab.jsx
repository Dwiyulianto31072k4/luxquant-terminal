...
export function AiCostTab() {
 ...
 days === d ? 'bg-accent text-black font-semibold' : 'text-text-primary/50 hover:text-text-primary'
 }`}
 >
 {d}d
 </button>
 ...
 <Card label="Today" value={fmtUSD(summary.today.cost)} sub={`${fmtNum(summary.today.calls)} calls`} accent={palette.green[400]} />
 <Card label="This Month" value={fmtUSD(summary.month.cost)} sub={`${fmtNum(summary.month.calls)} calls`} accent="#fff" />
 <Card label={`${days}d Cost`} value={fmtUSD(summary.range.cost)} sub={`${fmtNum(summary.range.calls)} calls`} accent={palette.gold[300]} />
 <Card label={`${days}d Tokens`} value={fmtTokens(summary.range.tokens)} accent="#fff" />
 <Card label="Model Calls" value={fmtNum(summary.range.model_calls)} sub={`${fmtNum(summary.range.cache_calls)} from cache`} accent="#fff" />
 <Card label="Cache Hit Rate" value={`${summary.cache_hit_rate}%`} sub="served free" accent={palette.teal[400]} />
 </div>
 ...
 <td className="px-3 py-2 uppercase tracking-wider text-text-muted">{f.feature}</td>
 ...
 <td className="px-3 py-2 uppercase tracking-wider text-text-muted">{r.feature}</td>
