...
const Bar = ({ pct, color }) => (
  ...
  >
    <div
      ...
      style={{ width: `${Math.min(100, pct)}%`, background: color, transition: motion.base }}
    />
  </div>
);
    ...
    return (
      <div
        ...
        style={{
          ...
          border: `1px solid ${tint(palette.warm[100], 0.1)}`,
      ...
      <div className="flex gap-2.5 flex-wrap">
        ...
        <Tile color={toColor} Icon={AlertTriangleIcon} label="Worker Timeout">
          <div className="flex items-baseline gap-1.5">
            <span className="text-[22px] font-bold tabular-nums" style={{ color: toColor }}>
              {to.count_24h ?? 0}
            </span>
            <span className="text-[10px]" style={{ color: "rgb(var(--fg-muted))" }}>
              / 24h
          ...
          >
            <span>
              1h:{" "}
              <b style={{ color: to.count_1h > 0 ? palette.red[400] : "rgb(var(--fg-secondary))" }}>
                {to.count_1h ?? 0}
              </b>
            </span>
        ...
        <Tile color={palette.amber[400]} Icon={ClockIcon} label="Slow Requests">
          ...
          {slow.top && slow.top[0] ? (
            <p className="text-[10px] mt-1 truncate" style={{ color: "rgb(var(--fg-muted))" }}>
              worst: <span style={{ color: "rgb(var(--fg-secondary))" }}>{slow.top[0].path}</span> (
              {slow.top[0].max_s}s)
            </p>
          ) : (
            <p className="text-[10px] mt-1" style={{ color: palette.green[400] }}>
              none
            </p>
          )}
        </Tile>
        ...
        <Tile color={dbColor} Icon={ServerIcon} label="DB Connections">
          {db.error ? (
            <p className="text-[11px]" style={{ color: palette.red[300] }}>
              {db.error}
            </p>
          ) : (
            <>
              <div className="flex items-baseline gap-1.5">
                <span className="text-[22px] font-bold tabular-nums" style={{ color: dbColor }}>
                  {db.used ?? "–"}
                </span>
                <span className="text-[10px]" style={{ color: "rgb(var(--fg-muted))" }}>
                  / {db.max ?? "?"} ({db.pct ?? 0}%)
              ...
              <div className="text-[10px] mt-1" style={{ color: "rgb(var(--fg-muted))" }}>
                ...
                {db.idle_in_tx > 0 && (
                  <span style={{ color: palette.amber[400] }}> · idle-tx {db.idle_in_tx}</span>
                )}
              </div>
            </>
          )}
        ...
        <Tile color={redisColor} Icon={ZapIcon} label="Redis Memory">
          {redis.ok === false ? (
            ...
            <>
              <div className="flex items-baseline gap-1.5">
                <span className="text-[22px] font-bold tabular-nums" style={{ color: redisColor }}>
                  {redis.used_mb ?? "–"}
                </span>
                <span className="text-[10px]" style={{ color: "rgb(var(--fg-muted))" }}>
        ...
        <div className="mt-2">
          ...
          >
            ...
            {d.errors?.count_24h > 0 && (
              <span className="ml-1.5" style={{ color: palette.red[400] }}>
                · {d.errors.count_24h} error{d.errors.count_24h === 1 ? "" : "s"}/24h
              </span>
            )}
          </button>
          ...
          {open && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2.5 mt-2">
              ...
              <div
                ...
                style={{
                  background: "rgb(var(--surface-raised))",
                  border: `1px solid ${tint(palette.amber[400], 0.14)}`,
                }}
              >
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span
                    className="text-[9px] uppercase tracking-[0.14em] font-semibold"
                    style={{ color: tint(palette.amber[400], 0.85) }}
                  >
                    Top slow endpoints
                  </span>
                </div>
                {slow.top && slow.top.length ? (
                  ...
                  <p className="text-[11px]" style={{ color: palette.green[400] }}>
                    No slow requests in the last 24h.
                  </p>
                )}
              </div>
              ...
              <div
                ...
                style={{
                  background: "rgb(var(--surface-raised))",
                  border: `1px solid ${tint(palette.red[400], 0.14)}`,
                }}
              >
                <div className="flex items-center gap-1.5 mb-2">
                  {d.errors?.count_24h > 0 ? (
                  ...
                  <span
                    className="text-[9px] uppercase tracking-[0.14em] font-semibold"
                    style={{ color: tint(palette.red[400], 0.85) }}
                  >
                    Recent errors
                  </span>
                </div>
                  ...
                  <div className="space-y-1.5">
                    {d.errors.recent.map((e, i) => (
                      <div key={i} className="text-[10px] leading-snug">
                        <span className="tabular-nums" style={{ color: "rgb(var(--fg-muted))" }}>
                          {fmtAgo(e.ts)}
                        </span>
                        <span className="ml-1.5" style={{ color: "rgb(var(--fg-secondary))" }}>
                          {e.msg}
                        </span>
                      </div>
                    ))}
