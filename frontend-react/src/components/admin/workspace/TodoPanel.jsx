...
const TagsInput = ({ tags, onChange }) => {
 ...
 {tags.map((t) => (
 <span key={t} className="flex items-center gap-1 rounded px-2 py-0.5 font-mono text-[11px]" style={{ background: 'rgb(var(--accent) / 0.08)', color: 'rgb(var(--accent))', border: '1px solid rgb(var(--line) / 0.22)' }}>
 #{t}
 <button type="button" onClick={() => removeTag(t)} className="hover:text-loss" style={{ color: 'rgb(var(--fg-muted))' }}><CloseIcon size={9} /></button>
 </span>
 ))}
