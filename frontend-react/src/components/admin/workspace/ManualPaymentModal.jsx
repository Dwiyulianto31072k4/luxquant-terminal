...
const CheckMini = ({ className = '' }) => (
 <svg className={className} width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
);
...
const StepHeader = ({ num, title, complete, locked }) => (
 ...
 style={{
 background: complete ? 'rgba(52,211,153,0.18)' : locked ? 'rgb(var(--ink) / 0.04)' : 'rgb(var(--accent) / 0.16)',
 ...
 <h4 className="text-[10.5px] font-bold uppercase tracking-wider" style={{ color: locked ? '#4a3f39' : '#fff' }}>{title}</h4>
 </div>
);
...
const Pill = ({ tone, children, Icon, pulse }) => {
 ...
 gold: { bg: 'rgb(var(--accent) / 0.10)', color: 'rgb(var(--accent))', border: 'rgb(var(--accent) / 0.3)' },
 };
 const t = tones[tone] || tones.gold;
 return (
 <span className={`inline-flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${pulse ? 'animate-pulse' : ''}`}
...
const TxStep = ({ txHash, setTxHash, verifying, verifyResult, verifyError, onVerify, onReset, paymentDateOverride, setPaymentDateOverride }) => {
 ...
 <Field label="TX Hash" hint="Paste the 66-char BSC USDT transaction hash (0x…)" error={txHash && !looksValid ? 'Invalid hash format' : null}>
 <TextInput value={txHash} onChange={(v) => setTxHash(v.trim())} placeholder="0x abc 123…" mono autoFocus />
 </Field>
 <button
 ...
 style={{ background: looksValid ? 'linear-gradient(135deg, rgb(var(--accent)), rgb(var(--accent)))' : 'rgb(var(--ink) / 0.04)', color: looksValid ? 'rgb(var(--surface))' : '#6b5c52' }}>
 ...
 {blockers.length === 0 ? <Pill tone="green" Icon={CheckCircleIcon}>Verified on chain</Pill> : <Pill tone="red" Icon={AlertCircleIcon}>Cannot proceed</Pill>}
 {exchange && (
 <span className="inline-flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider"
 style={{ background: `${exchangeColor(exchange)}14`, color: exchangeColor(exchange), border: `1px solid ${exchangeColor(exchange)}33` }}>
 <span className="h-1.5 w-1.5 rounded-full" style={{ background: exchangeColor(exchange) }} />{exchange}
 </span>
 )}
 ...
 <Row label="Amount" value={tx?.amount ? `${tx.amount} USDT` : '—'} mono valueColor="rgb(var(--accent))" big />
 <Row label="From" value={tx?.from} mono copyable />
 <Row label="To" value={tx?.to} mono copyable />
 {tx?.timestamp && <Row label="Time" value={new Date(tx.timestamp).toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })} />}
...
const PaymentDateOverride = ({ txTimestamp, value, onChange }) => {
 ...
 const handleToggle = () => {
 if (overriding) { onChange(''); setOverriding(false); }
 else { onChange(txDateStr || ''); setOverriding(true); }
 };
 ...
 style={{ background: overriding ? 'rgb(var(--accent) / 0.04)' : 'rgb(var(--ink) / 0.02)', border: `1px solid ${overriding ? 'rgb(var(--accent) / 0.22)' : 'rgb(var(--ink) / 0.05)'}` }}>
 ...
 <button onClick={handleToggle} className="text-[9.5px] font-semibold uppercase tracking-wider transition-colors" style={{ color: overriding ? '#f87171' : 'rgb(var(--accent))' }}>
...
const UserStep = ({ locked, mode, setMode, selectedUser, setSelectedUser, newUser, setNewUser, suggestedUserId, onSuggestUserResolved }) => {
 ...
 financeApi.searchUsers(`#${suggestedUserId}`).then((d) => {
 const u = (d.users || []).find((u) => u.id === suggestedUserId);
 if (u) { setSuggested(u); if (onSuggestUserResolved) onSuggestUserResolved(u); }
 }).catch(() => {});
 }
 ...
 try { const d = await financeApi.searchUsers(query.trim()); setResults(d.users || []); }
 ...
 return (
 <section>
 <StepHeader num={2} title="Link to User" complete={(mode === 'existing' && !!selectedUser) || (mode === 'new' && newUser.username?.trim().length >= 3)} locked={locked} />
 {!locked && (
 <>
 ...
 style={{ background: mode === opt.id ? 'rgb(var(--accent) / 0.12)' : 'rgb(var(--ink) / 0.02)', color: mode === opt.id ? 'rgb(var(--accent))' : '#8a7a6e', border: `1px solid ${mode === opt.id ? 'rgb(var(--accent) / 0.32)' : 'rgb(var(--ink) / 0.06)'}` }}>
 {opt.label}
 </button>
 ))}
 </div>
 ...
 style={{ background: 'rgb(var(--accent) / 0.06)', border: '1px solid rgb(var(--line) / 0.22)' }}>
 <div className="flex items-center gap-2">
 <span className="flex h-6 w-6 items-center justify-center rounded-md" style={{ background: 'rgb(var(--accent) / 0.14)', color: 'rgb(var(--accent))', boxShadow: 'inset 0 0 0 1px rgb(var(--accent) / 0.3)' }}>
 ...
 <span className="flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold" style={{ background: 'rgb(var(--accent) / 0.12)', color: 'rgb(var(--accent))' }}>{u.username[0].toUpperCase()}</span>
 <div className="min-w-0 flex-1">
 ...
 <TextInput value={newUser.username} onChange={(v) => setNewUser({ ...newUser, username: v })} placeholder="lianprotrader" />
 </Field>
 <Field label="Email" hint="Optional — leave blank to auto-generate (manual_<username>@manual.luxquant.tw)">
 <TextInput value={newUser.email} onChange={(v) => setNewUser({ ...newUser, email: v })} placeholder="user@example.com" />
 </Field>
 <div className="grid grid-cols-2 gap-2">
 <Field label="Telegram" hint="Without @"><TextInput value={newUser.telegram_username} onChange={(v) => setNewUser({ ...newUser, telegram_username: v })} placeholder="lianpro" /></Field>
 <Field label="Discord" hint="Username or ID"><TextInput value={newUser.discord_handle} onChange={(v) => setNewUser({ ...newUser, discord_handle: v })} placeholder="lianpro#1234" /></Field>
 </div>
 </div>
 )}
 </>
...
const SelectedUserCard = ({ user, onClear }) => (
 ...
 <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold" style={{ background: 'rgb(var(--accent) / 0.15)', color: 'rgb(var(--accent))' }}>{user.username[0].toUpperCase()}</span>
 <div className="min-w-0 flex-1">
 <div className="flex items-center gap-1.5">
...
const PlanStep = ({ locked, plans, selectedPlanId, setSelectedPlanId, suggestedPlanId, txAmount }) => (
 ...
 style={{ background: isSelected ? 'rgb(var(--accent) / 0.10)' : 'rgb(var(--ink) / 0.02)', border: `1px solid ${isSelected ? 'rgb(var(--accent) / 0.40)' : 'rgb(var(--ink) / 0.05)'}` }}>
 <div className="mb-1 flex items-center justify-between">
 <p className="text-[12px] font-semibold tracking-tight" style={{ color: isSelected ? 'rgb(var(--accent))' : '#fff' }}>{p.label}</p>
 {isSuggested && !isSelected && <Pill tone="green">Match</Pill>}
 </div>
 <div className="flex items-baseline justify-between">
 <span className="text-base font-light tabular-nums" style={{ color: isSelected ? 'rgb(var(--accent))' : '#fff' }}>${Number(p.price_usdt).toFixed(2)}</span>
...
const MethodSelector = ({ method, setMethod }) => (
 ...
 style={{ background: active ? 'rgb(var(--accent) / 0.16)' : 'rgb(var(--ink) / 0.02)', color: active ? 'rgb(var(--accent))' : '#8a7a6e', border: `1px solid ${active ? 'rgb(var(--accent) / 0.4)' : 'rgb(var(--ink) / 0.08)'}` }}>
 {m.label}
 </button>
 );
 })}
...
const OffchainStep = ({ method, offAmount, setOffAmount, idrAmount, setIdrAmount, idrRate, idrUsd, reference, setReference, methodLabel, setMethodLabel, paymentDateOverride, setPaymentDateOverride }) => (
 <div>
 <StepHeader num={2} title="Payment details" complete={false} locked={false} />
 <div className="space-y-3">
 {method === 'other' && <Field label="Method name" hint="e.g. OVO, GoPay, Cash, PayPal"><TextInput value={methodLabel} onChange={setMethodLabel} placeholder="Method name" autoFocus /></Field>}
 {method === 'bank_transfer' ? (
 <Field label="Amount (IDR)" hint={idrRate ? (idrUsd ? `\u2248 $${idrUsd.toFixed(2)} USD \u00b7 live rate ${Math.round(idrRate).toLocaleString()} IDR/USDT` : `Live rate ${Math.round(idrRate).toLocaleString()} IDR/USDT`) : 'Loading live rate\u2026'}>
 ...
 <Field label={method === 'binance_uid' ? 'Binance UID / reference' : 'Reference'}
 hint={method === 'binance_uid' ? 'Sender Binance UID (+ order id if any)' : method === 'bank_transfer' ? 'Sender name / date / bank ref' : 'Any reference for the audit trail'}>
...
export const ManualPaymentModal = ({ isOpen, onClose, onSuccess, preselectedUserId = null, preselectedUser = null }) => {
 ...
 const [newUser, setNewUser] = useState({ username: '', email: '', telegram_username: '', discord_handle: '' });
 ...
 useEffect(() => {
 if (!isOpen) {
 setTxHash(''); setVerifyResult(null); setVerifyError(null); setVerifying(false); setPaymentDateOverride('');
 setUserMode('existing'); setSelectedUser(preselectedUser); setNewUser({ username: '', email: '', telegram_username: '', discord_handle: '' });
 setSelectedPlanId(null); setNote(''); setSubmitError(null); setSubmitting(false);
 setAcceptAmountMismatch(false); setAcceptWalletNotInPool(false);
 setMethod('onchain_bsc'); setOffAmount(''); setIdrAmount(''); setReference(''); setMethodLabel('');
 }
 }, [isOpen, preselectedUser]);

 useEffect(() => {
 if (isOpen && plans.length === 0) {
 financeApi.getPlans().then((d) => setPlans(d.plans || [])).catch((e) => console.error('Failed to load plans:', e));
 }
 }, [isOpen, plans.length]);

 useEffect(() => {
 if (!isOpen) return;
 fetch('/api/v1/fx/rates').then((r) => r.json()).then((d) => setIdrRate(d?.rates?.IDR ?? null)).catch(() => {});
 }, [isOpen]);

 useEffect(() => {
 if (verifyResult?.suggested_plan_id && !selectedPlanId) setSelectedPlanId(verifyResult.suggested_plan_id);
 }, [verifyResult, selectedPlanId]);

 const blockers = verifyResult?.blockers || [];
 const warnings = verifyResult?.warnings || [];
 const isOnchain = method === 'onchain_bsc';
 const idrUsd = method === 'bank_transfer' && idrAmount && idrRate ? Number(idrAmount) / idrRate : null;
 const offchainUsd = method === 'bank_transfer' ? idrUsd : (offAmount ? Number(offAmount) : null);
 const offchainStep1Done = !isOnchain && !!offchainUsd && offchainUsd > 0 && (method !== 'other' || methodLabel.trim().length > 0) && (method !== 'bank_transfer' || !!idrRate);
 const step1Done = isOnchain ? (!!verifyResult && blockers.length === 0) : offchainStep1Done;
 const step2Done = step1Done && ((userMode === 'existing' && !!selectedUser) || (userMode === 'new' && newUser.username?.trim().length >= 3));
 ...
 const hasAmountMismatch = planPrice !== null && txAmount && Math.abs(Number(txAmount) - planPrice) > 0.5;
 const hasWalletNotInPoolWarning = warnings.some((w) => w.code === 'wallet_not_in_pool');
 const canSubmit = step4Done && (!isOnchain || !hasAmountMismatch || acceptAmountMismatch) && (!isOnchain || !hasWalletNotInPoolWarning || acceptWalletNotInPool) && !submitting;

 const handleVerify = useCallback(async () => {
 if (verifying) return;
 setVerifying(true); setVerifyError(null);
 try { setVerifyResult(await financeApi.verifyTx(txHash.trim())); }
 catch (e) { setVerifyError(e.response?.data?.detail || 'Verification failed. Try again.'); }
 finally { setVerifying(false); }
 }, [txHash, verifying]);

 const handleResetTx = () => { setVerifyResult(null); setVerifyError(null); setSelectedPlanId(null); setPaymentDateOverride(''); };
 ...
 const txDateStr = verifyResult?.tx_data?.timestamp ? new Date(verifyResult.tx_data.timestamp).toISOString().slice(0, 10) : null;
 if (paymentDateOverride && paymentDateOverride !== txDateStr) payload.payment_date_override = paymentDateOverride;
 } else {
 payload.amount_usd = Number(offchainUsd);
 payload.reference = reference.trim() || undefined;
 if (method === 'bank_transfer') { payload.paid_currency = 'IDR'; payload.paid_amount = Number(idrAmount); payload.fx_rate = idrRate; }
 else if (method === 'binance_uid') { payload.paid_currency = 'USDT'; payload.paid_amount = Number(offAmount); }
 else if (method === 'other') { payload.method_label = methodLabel.trim() || undefined; payload.paid_currency = 'USD'; payload.paid_amount = Number(offAmount); }
 ...
 <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" style={{ background: 'rgb(var(--accent) / 0.1)', border: '1px solid rgb(var(--line) / 0.22)' }}>
 ...
 return (
 <Modal isOpen={isOpen} onClose={onClose} size="lg" padded={false} header={header} footer={footer}>
 <div className="space-y-6 px-5 py-5">
 <MethodSelector method={method} setMethod={setMethod} />

 {isOnchain ? (
 <TxStep txHash={txHash} setTxHash={setTxHash} verifying={verifying} verifyResult={verifyResult} verifyError={verifyError} onVerify={handleVerify} onReset={handleResetTx} paymentDateOverride={paymentDateOverride} setPaymentDateOverride={setPaymentDateOverride} />
 ) : (
 <OffchainStep method={method} offAmount={offAmount} setOffAmount={setOffAmount} idrAmount={idrAmount} setIdrAmount={setIdrAmount} idrRate={idrRate} idrUsd={idrUsd} reference={reference} setReference={setReference} methodLabel={methodLabel} setMethodLabel={setMethodLabel} paymentDateOverride={paymentDateOverride} setPaymentDateOverride={setPaymentDateOverride} />
 )}

 <div style={{ opacity: step1Done ? 1 : 0.4, pointerEvents: step1Done ? 'auto' : 'none' }}>
 <UserStep locked={!step1Done && !preselectedUser} mode={userMode} setMode={setUserMode} selectedUser={selectedUser} setSelectedUser={setSelectedUser} newUser={newUser} setNewUser={setNewUser} suggestedUserId={verifyResult?.suggested_user_id} />
 </div>

 <div style={{ opacity: step2Done ? 1 : 0.4, pointerEvents: step2Done ? 'auto' : 'none' }}>
 <PlanStep locked={!step2Done} plans={plans} selectedPlanId={selectedPlanId} setSelectedPlanId={setSelectedPlanId} suggestedPlanId={verifyResult?.suggested_plan_id} txAmount={txAmount} />
 </div>

 {isOnchain && step3Done && hasAmountMismatch && (
 <div className="space-y-2 rounded-lg p-3" style={{ background: 'rgba(251,146,60,0.06)', border: '1px solid rgba(251,146,60,0.25)' }}>
 <div className="flex items-start gap-2">
 <AlertTriangleIcon size={13} className="mt-0.5 shrink-0" style={{ color: '#fb923c' }} />
 <div className="text-[11px]" style={{ color: '#fb923c' }}>
 <strong>Amount mismatch:</strong> TX is <span className="tabular-nums">{txAmount} USDT</span>, but {selectedPlan?.label} is <span className="tabular-nums">${planPrice.toFixed(2)}</span> (diff {(Number(txAmount) - planPrice).toFixed(2)}).
 </div>
 </div>
 <label className="flex cursor-pointer items-center gap-2 text-[11px]" style={{ color: '#fb923c' }}>
 <input type="checkbox" checked={acceptAmountMismatch} onChange={(e) => setAcceptAmountMismatch(e.target.checked)} className="cursor-pointer" />
