...
const UserPicker = ({ selectedUser, onChange }) => {
 ...
 <button type="button" onClick={() => { onChange(null); setSearch(''); }} className="rounded p-1.5 transition-colors" style={{ color: 'rgb(var(--fg-muted))', background: 'rgb(var(--ink) / 0.04)' }}>
 <CloseIcon size={11} />
 </button>
 </div>
 );
 ...
 className="w-full rounded-lg py-2 pl-9 pr-3 text-xs text-text-primary focus:outline-none" style={{ background: 'rgb(var(--scrim) / 0.3)', border: '1px solid rgb(var(--ink) / 0.1)' }} />
 ...
 className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs transition-colors hover:bg-ink/5" style={i > 0 ? { borderTop: '1px solid rgb(var(--ink) / 0.04)' } : {}}>
 <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold" style={{ background: 'rgb(var(--accent) / 0.15)', color: 'rgb(var(--accent))' }}>{u.username?.charAt(0).toUpperCase()}</span>
