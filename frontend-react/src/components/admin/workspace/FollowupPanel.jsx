  ...
  if (selectedUser) {
    return (
      <div
        ...
        style={{ background: "rgba(138,138,147,0.06)", border: "1px solid rgba(138,138,147,0.22)" }}
      >
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate font-mono text-[10px]" style={{ color: "rgb(var(--fg-muted))" }}>
              #{selectedUser.id}
            </p>
          </div>
        </div>
        <button
          ...
          style={{ color: "rgb(var(--fg-muted))", background: "rgb(var(--ink) / 0.04)" }}
        >
          <CloseIcon size={11} />
        </button>
      </div>
  ...
  return (
    <div className="relative">
      ...
      {showResults && results.length > 0 && (
        <div
          className="absolute left-0 right-0 z-10 mt-1 max-h-60 overflow-y-auto rounded-lg shadow-2xl"
          style={{
        ...
        >
          {results.map((u, i) => (
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs transition-colors hover:bg-ink/5"
      ...
      {searching && (
        <p className="mt-1 text-[10px]" style={{ color: "rgb(var(--fg-muted))" }}>
          Searching…
        </p>
      )}
    </div>
...
const PickOption = ({ value, currentValue, onClick, label, Icon, color }) => {
  ...
  return (
    <button
      ...
      style={{
        background: selected ? `${color}18` : "rgb(var(--ink) / 0.02)",
        color: selected ? color : "rgb(var(--fg-muted))",
        border: `1px solid ${selected ? `${color}45` : "rgb(var(--ink) / 0.06)"}`,
      }}
    >
      {Icon && <Icon size={12} />}
      {label}
