...
//
// Slide-in form for create/edit followup. Uses SidePanel wrapper.
...
// ════════════════════════════════════════════════════════════════════
// User picker — search and select user to link
// ════════════════════════════════════════════════════════════════════
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] font-mono truncate" style={{ color: '#6b5c52' }}>
              #{selectedUser.id}
            </p>
          </div>
        </div>
        <button
          ...
          style={{ color: '#8a7a6e', background: 'rgba(255,255,255,0.04)' }}
        >
          <CloseIcon size={11} />
        </button>
      </div>
  ...
  return (
    <div className="relative">
      ...
      {showResults && results.length > 0 && (
        <div
          className="absolute z-10 left-0 right-0 mt-1 max-h-60 overflow-y-auto rounded-lg shadow-2xl"
          style={{
            background: '#12090d',
...
// ════════════════════════════════════════════════════════════════════
// Pick option grid (radio-style for category/priority)
// ════════════════════════════════════════════════════════════════════
...
const CATEGORIES = [
  { value: 'renewal', label: 'Renewal', emoji: '🔄', color: '#60a5fa' },
  { value: 'payment', label: 'Payment', emoji: '💳', color: '#34d399' },
  { value: 'support', label: 'Support', emoji: '🛟', color: '#fbbf24' },
  { value: 'general', label: 'General', emoji: '📝', color: '#8a7a6e' },
];
...
export const FollowupPanel = ({ isOpen, onClose, editingItem, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (editingItem) {
      ...
      setCategory('general');
      setPriority('normal');
      // Default due_date: tomorrow at 10:00 local
