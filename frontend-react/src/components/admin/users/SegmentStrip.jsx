...
const SEGMENTS = [
  { key: "all", label: "All", filter: {}, statKey: "total_users" },
  {
    key: "subscriber",
    label: "Subscriber",
    filter: { role: "subscriber" },
    statKey: "active_subscribers",
  },
  {
    key: "ex_subscriber",
    label: "Ex-Subscriber",
    filter: { exSubscriber: "1" },
    statKey: "ex_subscribers",
  },
  { key: "free", label: "Free", filter: { role: "free" }, statKey: "free_users" },
  {
    key: "lifetime",
    label: "Lifetime",
    filter: { plan: "lifetime" },
    statKey: "lifetime_subscribers",
  },
            ...
            {count != null && (
              <span
                className={`rounded-full px-1.5 py-0.5 font-mono text-[10px] font-semibold tabular-nums ${
                  isActive
                    ? "bg-ink/[0.12] text-text-primary/80"
                    : "bg-ink/[0.05] text-text-muted/70"
                }`}
              >
                {count}
              </span>
