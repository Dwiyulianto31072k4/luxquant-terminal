...
const fmt = (date) => date.toLocaleDateString("en-US", { day: "2-digit", month: "short", year: "numeric" });
...
const DurationOption = ({ Icon, label, desc, selected, onClick }) => (
 ...
 <span className="text-[11px] font-bold tracking-tight" style={{ color: selected ? GOLD : "#fff" }}>{label}</span>
...
const DateInput = ({ label, required, value, onChange, min, helper }) => (
 ...
 style={{ background: "rgb(var(--surface-secondary))", border: `1px solid ${required ? "rgb(var(--accent) / 0.2)" : "rgb(var(--ink) / 0.06)"}`, colorScheme: "dark" }}
 />
 </div>
...
const PreviewCard = ({ start, end, days }) => (
 <div className="relative overflow-hidden rounded-lg" style={{ background: "rgb(var(--accent) / 0.04)", border: "1px solid rgb(var(--line) / 0.22)" }}>
 <span className="pointer-events-none absolute inset-x-0 top-0 h-px" style={{ background: "linear-gradient(to right, transparent, rgb(var(--accent) / 0.4), transparent)" }} />
 <div className="flex items-center justify-between px-3 py-1.5" style={{ background: "rgb(var(--accent) / 0.06)" }}>
 ...
 <span className="rounded px-2 py-0.5 text-[10px] font-bold tabular-nums" style={{ background: "rgb(var(--accent) / 0.18)", color: GOLD }}>
...
export const GrantModal = ({ user, onClose, onGrant }) => {
 ...
 <div className="relative flex h-10 w-10 items-center justify-center rounded-xl" style={{ background: "rgb(var(--accent) / 0.12)", border: "1px solid rgb(var(--line) / 0.3)" }}>
 ...
 <GhostButton onClick={onClose} disabled={loading} className="flex-1">Cancel</GhostButton>
 <GoldButton onClick={handleGrant} disabled={loading || (mode === "custom" && (!startDate || !endDate))} className="flex-1">
 ...
 <DateInput label="End date" required value={endDate} onChange={setEndDate} min={startDate || todayISO()} />
 </div>
 )}
