...
const HBar = ({ rows, height = 8, onRowClick, activeFilter }) => {
 ...
 <div className="h-full rounded-sm transition-all duration-700" style={{ width: `${pct}%`, background: r.color || "rgb(var(--accent) / 0.7)", opacity: isActive ? 1 : 0.85 }} />
 </div>
 </div>
 );
 })}
...
const BetaScatter = ({ signals }) => {
 ...
 {minBeta < 1 && maxBeta > 1 && (<>
 <line x1={oneBetaX} y1={pad.top} x2={oneBetaX} y2={pad.top + innerH} stroke="rgb(var(--accent) / 0.25)" strokeDasharray="3,3" />
 <text x={oneBetaX} y={pad.top - 5} textAnchor="middle" fill="rgb(var(--accent) / 0.6)" fontSize={9} fontFamily="JetBrains Mono">β=1</text>
 </>)}
