...
const Label = ({ children, className = "" }) => (
  <div
    className={`text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 ${className}`}
  >
    {children}
  </div>
);
...
const SectionHeader = ({ label }) => (
  <div className="flex items-center gap-3 my-6">
    ...
    <div className="text-[11px] tracking-[0.25em] text-text-muted font-mono">· {label} ·</div>
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <div className="text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/50 mt-1.5">
          win rate
        </div>
        {total > 0 && (
  ...
  const arcs = segments.map((s, i) => {
    ...
    return (
      <circle
        ...
        className={
          onSegmentClick && s.value > 0 ? "cursor-pointer hover:opacity-80 transition" : ""
        }
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
        <div className="font-mono tabular-nums text-text-primary" style={{ fontSize: size * 0.22 }}>
          {centerLabel}
        </div>
        {centerSublabel && (
          <div className="text-[9px] tracking-[0.2em] font-mono uppercase text-text-primary/45 mt-0.5">
            {centerSublabel}
          </div>
        )}
      </div>
    ...
    <div className="space-y-3">
      {rows.map((r, i) => {
        ...
        return (
          <div
            key={i}
            className={`${clickable ? "cursor-pointer group" : ""} ${isActive ? "ring-1 ring-accent/40 rounded-sm -mx-2 px-2 -my-1 py-1" : ""}`}
            onClick={() => clickable && onRowClick(r.label)}
          >
            <div className="flex justify-between items-baseline mb-1.5">
              <span
                className={`text-[12px] font-mono uppercase tracking-wider ${isActive ? "text-accent" : "text-text-primary/75 group-hover:text-text-primary"}`}
              >
                {r.label}
              </span>
              <span className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs font-mono tabular-nums min-w-[3rem] text-right ${isActive ? "text-accent" : "text-text-primary/85"}`}
                >
                  {r.value}
                </span>
              </span>
            ...
            <div className="bg-ink/[0.04] rounded-sm overflow-hidden" style={{ height }}>
              <div
                ...
                style={{
                  width: `${pct}%`,
                  background: r.color || "rgb(var(--accent) / 0.7)",
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {entries.map(([key, value]) => (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm bg-accent/12 border border-ink/12 text-[11px] font-mono uppercase tracking-wider text-accent hover:bg-accent transition group"
          >
        ...
        <span className="ml-auto flex items-center gap-3 text-[11px] font-mono">
          <span className="text-text-primary/40">
            {totalUnfiltered}
            <span className="text-text-primary/30 mx-1">→</span>
            <span className="text-accent tabular-nums">{totalFiltered}</span>
            <span className="text-text-primary/30 ml-1">signals</span>
          </span>
          <button
            onClick={onClear}
            className="px-2 py-0.5 rounded-sm border border-ink/[0.08] text-text-primary/50 hover:text-text-primary hover:border-ink/20 text-[10px] uppercase tracking-wider transition"
          >
            Clear All
          </button>
        </span>
    ...
    <Card className="px-4 py-3 mb-5 border-accent/20 bg-accent/[0.03]">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 text-xs text-text-primary/75">
          {isPreV3 ? (
            <>
              <span className="text-accent font-mono uppercase tracking-wider text-[10px]">
                Limited enrichment data
              </span>
              <div className="mt-1 text-text-primary/55">
                BTC context, patterns, and per-signal tags unavailable for this date — enrichment
                v3.0 launched 2026-05-14. Win rate and sector breakdown remain accurate.
              </div>
            </>
          ) : (
            <>
...
const HeroSection = ({
  ...
  const hasCorrelationInsight =
    !hasFilters &&
    corrAdvantage !== null &&
    corrAdvantage !== undefined &&
    Math.abs(corrAdvantage) >= 10;

  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
      <Card className="lg:col-span-5 p-6 flex flex-col items-center justify-center">
        <div className="text-[10px] tracking-[0.25em] font-mono uppercase text-text-muted mb-3">
            ...
            {summary?.delta_vs_yesterday !== undefined &&
              ...
              !hasFilters && (
                <>
                  ...
                  <span className={summary.delta_vs_yesterday > 0 ? "text-profit" : "text-loss"}>
                    {summary.delta_vs_yesterday > 0 ? "▲" : "▼"}{" "}
      ...
      <Card className="lg:col-span-7 p-6">
        <div className="text-[10px] tracking-[0.25em] font-mono uppercase text-text-muted mb-4">
          · Daily Story {hasFilters ? "(filtered)" : ""} ·
        </div>
        <p className="text-base text-text-primary/85 leading-relaxed">
          {hasFilters ? (
            <>
              <span className="font-mono tabular-nums text-accent text-lg">{total}</span> of{" "}
              {totalUnfiltered} signals match current filters with{" "}
              <span className="font-mono tabular-nums text-text-primary">{fmtPct(wr, 2)}</span> win
              rate
              {total > 0 && (
                <>
                  , avg peak{" "}
                  <span
                    className={`font-mono ${avgPeak > 0 ? "text-profit" : avgPeak < 0 ? "text-loss" : "text-text-primary/60"}`}
                  >
                    {avgPeak > 0 ? "+" : ""}
                    {avgPeak.toFixed(2)}%
                  </span>
            ...
            <>
              <span className="font-mono tabular-nums text-accent text-lg">
        ...
        <div className="mt-6 space-y-3">
          {topSector && (
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-text-primary/70">
                <span className="text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/45 mr-2">
                  Best sector
                </span>
                <span className="font-mono uppercase tracking-wider text-text-primary/90">
                  {topSector.sector}
                </span>
                <span className="text-text-primary/50 ml-2 font-mono tabular-nums text-xs">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-text-primary/70">
                ...
                {summary.btc_dom_trend_mode && (
                  <span className="text-text-primary/40 ml-2 text-xs">
                    · DOM {summary.btc_dom_trend_mode}
                  </span>
                )}
          ...
          {hasEnrichment && topPattern && topPattern.count >= 2 && (
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-text-primary/70">
                <span className="text-[10px] tracking-[0.2em] font-mono uppercase text-text-muted mr-2">
                  Top flag
                </span>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-text-primary/70">
                <span
                  ...
                  style={{
                    color: corrAdvantage > 0 ? "rgba(16,185,129,0.7)" : "rgba(245,158,11,0.7)",
                  }}
                >
                  Correlation edge
                </span>
                ...
                <span className="text-text-primary/50 ml-2 font-mono tabular-nums text-xs">
                  outperformed by{" "}
                  <span className={corrAdvantage > 0 ? "text-profit" : "text-accent"}>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      <KpiCard
        ...
        sub="across filtered"
        valueColor={
          avgPeak > 0 ? "text-profit" : avgPeak < 0 ? "text-loss" : "text-text-primary/95"
        }
      />
      <KpiCard
        label="Decoupled"
        ...
        subColor={hasAnyEnrichment ? "text-text-primary/40" : "text-accent/60"}
      />
    </div>
  );
};
    ...
    {TAB_ITEMS.map((t) => {
      ...
      return (
        <button
          ...
          className={`relative px-4 py-3 text-[12px] font-mono uppercase tracking-wider transition whitespace-nowrap ${isActive ? "text-accent" : "text-text-primary/40 hover:text-text-primary/70"}`}
        >
          {t.label}
    ...
    <Card className="p-5">
      <div className="flex justify-between items-center mb-4">
        ...
        {showInteractiveDonut && (
          <div className="text-[10px] font-mono uppercase tracking-wider text-text-primary/30">
            click to filter
          </div>
        )}
      </div>
            ...
            {btcSegments.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 text-left transition ${s.value === 0 ? "opacity-30 pointer-events-none" : "hover:opacity-90"} ${isActive ? "ring-1 ring-accent/40 rounded-sm -mx-1 px-1" : ""}`}
                >
                  <span
                  ...
                  />
                  <span
                    className={`text-[11px] font-mono uppercase tracking-wider w-16 ${isActive ? "text-accent" : "text-text-primary/65"}`}
                  >
                    {s.label}
                  </span>
                  ...
                  <span className="font-mono tabular-nums text-xs text-text-primary/75 w-8 text-right">
                    {s.value}
                  </span>
                </button>
              );
      ...
      {showSummaryFallback && (
        <div className="space-y-4 py-1">
          <div className="grid grid-cols-2 gap-4">
            {btcMode && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="font-mono uppercase tracking-wider text-base text-text-primary/90">
                    {btcMode}
                  </span>
                </div>
              </div>
            ...
            {btcDomMode && (
              <div>
                ...
                <div className="font-mono uppercase tracking-wider text-base text-text-primary/90">
                  {btcDomMode}
                </div>
              </div>
            )}
          ...
          <div className="text-[10px] font-mono uppercase tracking-wider text-text-primary/30 pt-1">
        ...
        <div className="space-y-3 py-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="font-mono tabular-nums text-base text-text-primary/90">
                {dailyRegime.win_rate?.toFixed(1)}%
              </div>
              <div className="text-[10px] font-mono tabular-nums text-text-primary/40 mt-0.5">
      ...
      {showEmpty && (
        <div className="text-xs font-mono text-text-primary/30 py-8 text-center uppercase tracking-wider">
  ...
  if (!signals?.length)
    return (
      <Card className="p-10 text-center">
        <div className="text-text-primary/30 text-sm font-mono uppercase tracking-wider">
    ...
    <div className="space-y-5">
              ...
              {outcomes.map((o) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 text-left transition ${o.value === 0 ? "opacity-30 pointer-events-none" : "hover:opacity-90"} ${isActive ? "ring-1 ring-accent/40 rounded-sm -mx-1 px-1" : ""}`}
                  >
                    <span
                      className="w-2 h-2 rounded-sm flex-shrink-0"
                    ...
                    />
                    <span
                      className={`text-[11px] font-mono uppercase tracking-wider w-8 ${isActive ? "text-accent" : "text-text-primary/65"}`}
                    >
                      {o.label}
                    </span>
                    <div className="flex-1 h-1 bg-ink/[0.04] rounded-sm overflow-hidden">
      ...
      <Card className="p-5">
        ...
        <HBar
          ...
          onRowClick={(n) => addFilter("sector", n)}
  ...
  const SortHeader = ({ id, label, className = "" }) => {
    ...
    return (
      <th
        ...
        className={`px-3 py-3 text-[10px] tracking-[0.2em] font-mono uppercase font-normal cursor-pointer hover:text-text-primary transition ${isActive ? "text-accent" : "text-text-primary/40"} ${className}`}
      >
  ...
  if (sorted.length === 0)
    return (
      <Card className="p-10 text-center">
        ...
        <div className="text-text-primary/20 text-xs font-mono mt-2 normal-case">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-ink/[0.06]">
              <th className="text-left px-4 py-3 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                Pattern
              </th>
            ...
            {sorted.map((p) => {
              ...
              return (
                <tr
                  ...
                  className={`border-b border-ink/[0.04] cursor-pointer transition ${isActive ? "bg-accent/10 border-ink/10" : "hover:bg-ink/[0.02]"}`}
                >
                  <td
                  ...
                  <td
                    className={`px-3 py-2.5 text-right font-mono tabular-nums ${p.avg_peak > 0 ? "text-profit" : p.avg_peak < 0 ? "text-loss" : "text-text-primary/40"}`}
                  >
                    {p.avg_peak > 0 ? "+" : ""}
                    {p.avg_peak.toFixed(2)}%
                  </td>
                  <td
                    className={`px-3 py-2.5 text-right font-mono tabular-nums ${p.total_peak > 0 ? "text-profit/80" : p.total_peak < 0 ? "text-loss/80" : "text-text-primary/40"}`}
                  >
                    {p.total_peak > 0 ? "+" : ""}
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <span
                      className={`text-xs font-mono ${isActive ? "text-accent" : "text-text-primary/30"}`}
                    >
                      {isActive ? "● filtered" : "→"}
                    </span>
                  </td>
...
const BetaScatter = ({ signals }) => {
  if (!signals.length)
    return (
      <div className="text-text-primary/30 text-sm font-mono uppercase tracking-wider text-center py-8">
  ...
  const minBeta = Math.min(...betas, 0),
    maxBeta = Math.max(...betas, 1.5);
  const minPeak = Math.min(...peaks, 0),
    maxPeak = Math.max(...peaks, 0);
  const width = 700,
    height = 380;
    ...
    <div className="overflow-x-auto">
      <svg width={width} height={height} className="block mx-auto">
        ...
        {signals.map((s, i) => (
          <circle
            ...
            r={5}
            fill={OUTCOME_COLORS[s.outcome] || "rgb(var(--ink) / 0.3)"}
            fillOpacity={0.7}
            stroke={OUTCOME_COLORS[s.outcome] || "rgb(var(--ink) / 0.5)"}
            strokeWidth={1}
          >
      ...
      <div className="mt-4 pt-4 border-t border-ink/[0.05] flex items-center justify-center gap-4 flex-wrap">
        {Object.entries(OUTCOME_COLORS).map(([key, color]) => (
          <span
            key={key}
            className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-wider text-text-primary/50"
          >
            <span
...
const AverageStat = ({ label, value, suffix = "", hint, digits = 2 }) => (
  <div className="rounded-sm bg-ink/[0.02] border border-ink/[0.05] px-3 py-2.5">
    <div className="text-[9px] tracking-[0.2em] font-mono uppercase text-text-primary/40 mb-1">
      {label}
    </div>
    <div className="font-mono tabular-nums text-lg text-text-primary/90">
...
const CorrelationTab = ({ signals, correlationSummary }) => {
  if (!correlationSummary || correlationSummary.coverage === 0)
    return (
      <Card className="p-10 text-center">
        <div className="text-text-primary/40 text-sm font-mono uppercase tracking-wider">
          No correlation data available
        </div>
        <div className="text-text-primary/30 text-xs font-mono mt-2 normal-case">
    ...
    <div className="space-y-5">
      <Card className="p-5">
        <div className="flex justify-between items-center mb-5">
          ...
          <span className="text-[10px] font-mono uppercase tracking-wider text-text-primary/30">
            {sum.coverage} signals enriched
          </span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div
            className={`rounded-md border p-4 transition ${decoupledWins ? "border-profit/25 bg-profit/[0.04]" : "border-ink/[0.06] bg-surface-raised"}`}
          >
            ...
            <div className="flex items-baseline gap-2">
              <span
                className={`text-3xl font-mono tabular-nums ${dec.win_rate === null ? "text-text-primary/30" : dec.win_rate >= 70 ? "text-profit" : dec.win_rate >= 50 ? "text-text-primary/90" : "text-loss"}`}
              >
                {dec.win_rate === null ? "—" : `${dec.win_rate.toFixed(1)}%`}
              </span>
              <span className="text-xs text-text-primary/40 font-mono">win rate</span>
            </div>
            <div className="mt-2 text-[11px] font-mono tabular-nums text-text-primary/50">
              {dec.wins}W / {dec.total - dec.wins}L
            </div>
            <div className="mt-3 h-1.5 bg-ink/[0.04] rounded-sm overflow-hidden">
              <div
                className="h-full bg-profit/70 transition-all duration-700"
                style={{ width: `${dec.win_rate || 0}%` }}
              />
            </div>
            <div className="mt-3 text-[10px] text-text-primary/40 leading-relaxed">
              Coins trading independently of BTC — low correlation, beta near zero
            </div>
          </div>
          <div
            className={`rounded-md border p-4 transition ${coupledWins ? "border-profit/25 bg-profit/[0.04]" : "border-ink/[0.06] bg-surface-raised"}`}
          >
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-baseline gap-2">
              <span
                className={`text-3xl font-mono tabular-nums ${cou.win_rate === null ? "text-text-primary/30" : cou.win_rate >= 70 ? "text-profit" : cou.win_rate >= 50 ? "text-text-primary/90" : "text-loss"}`}
              >
        ...
        {adv !== null && (
          ...
          >
            <div className="text-xs text-text-primary/70">
              {Math.abs(adv) < 1 ? (
                <>
                  <span className="font-mono uppercase tracking-wider text-[10px] text-text-primary/40 mr-2">
                    Insight
                  </span>
                ...
                <>
                  ...
                  <span
                    className={`font-mono tabular-nums ${decoupledWins ? "text-profit" : "text-accent"}`}
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <Card className="p-5">
                ...
                <div key={r.key}>
                  <div className="flex justify-between items-baseline mb-1.5">
                    <span className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] font-mono uppercase tracking-wider text-text-primary/65">
                        {r.label}
                      </span>
                    </span>
                    <span className="font-mono tabular-nums text-xs text-text-primary/75">
                      {r.count} <span className="text-text-primary/30">({pct.toFixed(0)}%)</span>
                    </span>
                  </div>
                  <div className="h-1.5 bg-ink/[0.04] rounded-sm overflow-hidden">
                    <div
                      className="h-full transition-all duration-700"
          ...
          <div className="mt-4 pt-3 border-t border-ink/[0.05] text-[10px] text-text-primary/35 leading-relaxed">
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-3 gap-2.5">
            <div className="text-center px-2 py-3 rounded-sm bg-ink/[0.02] border border-ink/[0.05]">
              <div className="text-[9px] font-mono uppercase tracking-wider text-profit/70 mb-1">
                ↑ Leads
              </div>
              <div className="font-mono tabular-nums text-2xl text-text-primary/90">
                {sum.lead_lag.leads}
              </div>
              <div className="text-[9px] font-mono text-text-primary/30 mt-1">moves first</div>
            </div>
            <div className="text-center px-2 py-3 rounded-sm bg-ink/[0.02] border border-ink/[0.05]">
              <div className="text-[9px] font-mono uppercase tracking-wider text-text-primary/50 mb-1">
                ◌ Sync
              </div>
              <div className="font-mono tabular-nums text-2xl text-text-primary/90">
                {sum.lead_lag.sync}
            ...
            <div className="text-center px-2 py-3 rounded-sm bg-ink/[0.02] border border-ink/[0.05]">
              <div className="text-[9px] font-mono uppercase tracking-wider text-loss/70 mb-1">
                ↓ Lags
              </div>
              <div className="font-mono tabular-nums text-2xl text-text-primary/90">
                {sum.lead_lag.lags}
      ...
      <Card className="p-5">
        ...
        <div className="mt-3 text-[10px] text-text-primary/35 text-center font-mono">
      ...
      {sectors.map((s) => {
        ...
        return (
          <button
            ...
            className={`text-left relative rounded-md bg-surface-raised border transition p-5 hover:border-ink/12 ${isActive ? "border-ink/15" : "border-ink/[0.06]"}`}
          >
            <div
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <Label>Signals</Label>
                <div className="font-mono tabular-nums text-lg text-text-primary/90 mt-1">
                  {s.total}
                </div>
              </div>
              <div>
                ...
                <div
                  className={`font-mono tabular-nums text-lg mt-1 ${s.win_rate >= 75 ? "text-profit" : s.win_rate >= 50 ? "text-text-primary/85" : "text-loss"}`}
                >
              ...
              <div>
                ...
                <div
                  className={`font-mono tabular-nums text-sm mt-1 ${s.avg_peak > 0 ? "text-profit/85" : s.avg_peak < 0 ? "text-loss/85" : "text-text-primary/50"}`}
                >
                  {s.avg_peak > 0 ? "+" : ""}
              ...
              <div>
                ...
                <div className="font-mono tabular-nums text-sm text-profit/85 mt-1">
                  {s.max_peak > 0 ? "+" : ""}
            ...
            <div className="flex justify-between text-[10px] font-mono tabular-nums text-text-primary/40 mt-1.5">
              <span>{s.wins}W</span>
...
const LossAutopsy = ({ signals }) => {
  const analysis = useMemo(() => {
    ...
    if (totalLosers === 0)
      return { losers: 0, winners: totalWinners, tagStats: [], coverageOk: false };
    const lc = {},
      wc = {};
    ...
    stats.sort((a, b) =>
      b.inLosers !== a.inLosers ? b.inLosers - a.inLosers : Math.abs(b.lift) - Math.abs(a.lift)
    );
  ...
  if (analysis.losers === 0)
    return (
      <Card className="p-5">
        <div className="flex justify-between items-center mb-4">
          <Label>Loss Autopsy</Label>
          <span className="text-[10px] font-mono uppercase tracking-wider text-profit/70">
            zero losses today
          </span>
        </div>
        <div className="py-8 text-center">
          <div className="text-profit/80 text-base font-mono mb-1">No SL hits today ✓</div>
          <div className="text-text-primary/40 text-xs font-mono">
            Clean session — all resolved signals reached at least TP1
          </div>
        </div>
      </Card>
    );
  if (!analysis.coverageOk)
    return (
      <Card className="p-5">
        <div className="flex justify-between items-center mb-4">
          <Label>Loss Autopsy</Label>
          <span className="text-[10px] font-mono uppercase tracking-wider text-text-primary/40">
            {analysis.losers} loss{analysis.losers !== 1 ? "es" : ""}
          </span>
        </div>
        <div className="py-6 text-center text-text-primary/35 text-xs font-mono">
          Losers on this date have no enrichment tags — pre-v3.0 signals
        </div>
      </Card>
    );
  ...
  return (
    <Card className="p-5">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <Label>Loss Autopsy</Label>
          <SmallSampleBadge n={analysis.losers} threshold={5} />
        </div>
        <span className="text-[10px] font-mono uppercase tracking-wider text-text-primary/40">
          {analysis.losers} loss{analysis.losers !== 1 ? "es" : ""} · {analysis.winners} win
          {analysis.winners !== 1 ? "s" : ""}
        </span>
      </div>
      <div className="text-xs text-text-primary/55 mb-4 leading-relaxed">
        Tags most frequently associated with stopped-out signals today, compared with their
        frequency in winners.
      </div>
      <div className="space-y-3">
        {top.map((s) => {
          const barPct = (s.loserFreq / maxFreq) * 100;
          const isBad = s.lift > 15,
            isGood = s.lift < -15;
          return (
            <div key={s.tag}>
              <div className="flex justify-between items-baseline mb-1 gap-3 flex-wrap">
                <span className="flex items-center gap-2">
                  ...
                  {isBad && (
                    <span className="text-[9px] font-mono uppercase tracking-wider text-loss/80 bg-loss/10 border border-loss/20 rounded-sm px-1.5 py-0.5">
                      bad signal
                    </span>
                  )}
                  {isGood && (
                    <span className="text-[9px] font-mono uppercase tracking-wider text-profit/80 bg-profit/10 border border-profit/20 rounded-sm px-1.5 py-0.5">
                      good signal
                    </span>
                  )}
                </span>
                <span className="font-mono tabular-nums text-[10px] text-text-primary/50">
                  {s.inLosers}/{analysis.losers} losers ({s.loserFreq.toFixed(0)}%)
                  <span className="text-text-primary/30 mx-1.5">vs</span>
                  {s.inWinners}/{analysis.winners} winners ({s.winnerFreq.toFixed(0)}%)
                </span>
              </div>
...
const SetupCombinations = ({ signals }) => {
  const combos = useMemo(() => {
    ...
    for (const s of signals) {
      ...
      if (!map[key])
        map[key] = { key, sector, pattern: tag, signals: [], wins: 0, losses: 0, total_peak: 0 };
      const c = map[key];
      c.signals.push(s);
    ...
    const arr = Object.values(map)
      ...
      .map((c) => ({
        ...c,
        total: c.signals.length,
        win_rate: c.signals.length ? (c.wins / c.signals.length) * 100 : 0,
        avg_peak: c.signals.length ? c.total_peak / c.signals.length : 0,
      }));
    const best = arr
      .filter((c) => c.win_rate >= 75)
      .sort((a, b) => b.total - a.total || b.avg_peak - a.avg_peak);
    const worst = arr
      .filter((c) => c.win_rate <= 25)
      .sort((a, b) => b.total - a.total || a.avg_peak - b.avg_peak);
  ...
  if (!hasEnrichment)
    return (
      <Card className="p-5">
        <Label>Today's Best/Worst Setups</Label>
        <div className="mt-6 py-6 text-center text-text-primary/35 text-xs font-mono">
          No pattern tags available — signals on this date predate v3.0 enrichment
        </div>
      </Card>
    );
  if (combos.best.length === 0 && combos.worst.length === 0)
    return (
      <Card className="p-5">
        <Label>Today's Best/Worst Setups</Label>
        <div className="mt-6 py-6 text-center text-text-primary/35 text-xs font-mono normal-case leading-relaxed">
          No setup combinations have ≥2 signals with consistent outcomes today.
          <br />
          <span className="text-text-primary/25">
            Mixed results — no clear winning or losing combos.
          </span>
        </div>
      </Card>
    );
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          <span className="text-[11px] font-mono uppercase tracking-wider text-text-primary/85">
            {c.sector}
          </span>
          <span className="text-text-primary/25 text-[10px]">+</span>
          <span className="text-[11px] font-mono text-text-primary/85 break-all">{c.pattern}</span>
        </div>
        <div className="text-[10px] font-mono tabular-nums text-text-primary/45">
          {c.wins}W / {c.losses}L · avg{" "}
          <span className={c.avg_peak > 0 ? "text-profit/80" : "text-loss/80"}>
            {c.avg_peak > 0 ? "+" : ""}
            {c.avg_peak.toFixed(2)}%
          </span>
        </div>
      </div>
      <div className="text-right flex-shrink-0">
        <div
          className={`font-mono tabular-nums text-base ${kind === "best" ? "text-profit" : "text-loss"}`}
        >
          {c.win_rate.toFixed(0)}%
        </div>
        <div className="text-[9px] font-mono tabular-nums text-text-primary/40">
  ...
  return (
    <Card className="p-5">
      <div className="flex justify-between items-center mb-4">
        <Label>Today's Best/Worst Setups</Label>
        <span className="text-[10px] font-mono uppercase tracking-wider text-text-primary/30">
          sector × pattern combos
        </span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          <div className="text-[10px] font-mono uppercase tracking-wider text-profit/70 mb-2.5">
            ⚡ Winning combos
          </div>
          {combos.best.length === 0 ? (
            <div className="text-[11px] font-mono text-text-primary/30 italic py-3">
              No combos with ≥75% win rate today
            </div>
          ) : (
            <div className="space-y-2">
              {combos.best.slice(0, 5).map((c) => (
                <Row key={c.key} c={c} kind="best" />
              ))}
            </div>
          )}
        </div>
        <div>
          <div className="text-[10px] font-mono uppercase tracking-wider text-loss/70 mb-2.5">
            ⚠ Losing combos
          </div>
          {combos.worst.length === 0 ? (
            <div className="text-[11px] font-mono text-text-primary/30 italic py-3">
              No combos with ≤25% win rate today
            </div>
          ) : (
            <div className="space-y-2">
              {combos.worst.slice(0, 5).map((c) => (
                <Row key={c.key} c={c} kind="worst" />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="mt-4 pt-3 border-t border-ink/[0.05] text-[10px] text-text-primary/35 leading-relaxed">
        Combos with ≥2 signals and ≥75% / ≤25% win rate. Dominant pattern picked alphabetically per
        signal.
      </div>
    </Card>
  );
...
const PatternByOutcomeMatrix = ({ signals }) => {
  const matrix = useMemo(() => {
    ...
    for (const s of signals)
      for (const tag of s.important_tags || []) {
        ...
        if (map[tag][s.outcome] !== undefined) {
          map[tag][s.outcome]++;
          map[tag].total++;
        }
      }
  ...
  if (matrix.length === 0)
    return (
      <Card className="p-5">
        <Label>Pattern × Outcome Distribution</Label>
        <div className="mt-6 py-6 text-center text-text-primary/35 text-xs font-mono">
          No patterns with ≥2 signals today
        </div>
      </Card>
    );
  return (
    <Card className="p-5 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-ink/[0.06]">
              <th className="text-left px-2 py-2 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                Pattern
              </th>
              {["tp4", "tp3", "tp2", "tp1", "sl"].map((o) => (
                <th
                  key={o}
                  className="text-center px-2 py-2 text-[10px] tracking-[0.2em] font-mono uppercase font-normal"
                  style={{ color: OUTCOME_COLORS[o] }}
                >
                  {o.toUpperCase()}
                </th>
              ))}
              <th className="text-right px-2 py-2 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                Total
              </th>
            </tr>
          </thead>
          <tbody>
            {matrix.map((m) => (
              <tr key={m.tag} className="border-b border-ink/[0.03]">
                ...
                {["tp4", "tp3", "tp2", "tp1", "sl"].map((o) => {
                  const v = m[o],
                    pct = m.total ? (v / m.total) * 100 : 0;
                  return (
                    <td key={o} className="px-2 py-2 text-center">
                      {v > 0 ? (
                        <div
                          className="inline-flex items-center justify-center w-7 h-7 rounded-sm font-mono tabular-nums text-xs text-text-primary/90"
                          style={{
                            background: OUTCOME_COLORS[o],
                            opacity: 0.35 + (pct / 100) * 0.55,
                          }}
                        >
                          {v}
                        </div>
                      ) : (
                        <span className="text-text-primary/15 text-xs">·</span>
                      )}
                    </td>
                  );
                })}
                <td className="px-2 py-2 text-right font-mono tabular-nums text-xs text-text-primary/60">
                  {m.total}
                </td>
              </tr>
            ))}
      ...
      <div className="mt-3 pt-3 border-t border-ink/[0.05] text-[10px] text-text-primary/35 leading-relaxed">
        Color intensity = % of signals with that pattern ending in that outcome. Patterns with ≥2
        signals only.
      </div>
    </Card>
  );
...
const SignalRow = ({ s, onClick }) => {
  ...
  return (
    <tr
      onClick={() => onClick(s)}
      className="border-b border-ink/[0.04] hover:bg-ink/[0.02] cursor-pointer transition"
    >
      ...
      <td className="px-3 py-2.5">
        <span className="inline-flex items-center gap-1.5">
          ...
          />
          <span className="text-[11px] font-mono uppercase tracking-wider text-text-primary/55">
            {s.sector || "—"}
          </span>
        </span>
      </td>
      <td className="px-3 py-2.5">
        {s.signal_direction === "BULLISH" ? (
          <span className="text-profit text-xs font-mono">↑ LONG</span>
        ) : s.signal_direction === "BEARISH" ? (
          <span className="text-loss text-xs font-mono">↓ SHORT</span>
        ) : (
          <span className="text-text-primary/20 text-xs">—</span>
        )}
      </td>
      <td
        className={`px-3 py-2.5 text-right font-mono tabular-nums text-sm ${peakPos ? "text-profit" : peakNeg ? "text-loss" : "text-text-primary/40"}`}
      >
        {s.peak_pct !== null && s.peak_pct !== undefined
          ? `${peakPos ? "+" : ""}${s.peak_pct.toFixed(2)}%`
          : "—"}
      </td>
      <td className="px-4 py-2.5 text-right font-mono tabular-nums text-text-primary/40 text-xs">
        {s.outcome_at
...
const TopSignalsList = ({ signals, onPickSignal, onShowAll }) => {
  ...
  return (
    <Card className="overflow-hidden">
      ...
      {preview.length === 0 ? (
        <div className="p-10 text-center text-text-primary/30 text-sm font-mono uppercase tracking-wider">
          No signals match current filters
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink/[0.06]">
                  ...
                  <th className="text-left px-3 py-3 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                    Outcome
                  </th>
                  <th className="text-left px-3 py-3 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                    Sector
                  ...
                  <th className="text-right px-3 py-3 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                    Peak %
                  </th>
                  <th className="text-right px-4 py-3 text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/40 font-normal">
                    Hit At
                  </th>
                </tr>
              </thead>
          ...
          {remaining > 0 && (
            <div className="border-t border-ink/[0.06] p-3 flex justify-center">
              <button
                onClick={onShowAll}
                className="px-4 py-2 rounded-sm text-[11px] font-mono uppercase tracking-wider text-accent border border-ink/12 hover:bg-accent/12 transition"
              >
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-ink/[0.06]">
          <div>
            <div className="text-[10px] tracking-[0.25em] font-mono uppercase text-text-muted">
              · All Filtered Signals ·
            </div>
            <div className="text-lg text-text-primary/90 mt-0.5">
              {signals?.length || 0} signals
            </div>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-sm border border-ink/[0.08] text-text-primary/60 hover:text-text-primary hover:border-ink/20 transition flex items-center justify-center"
          >
            ✕
          </button>
        </div>
...
const DailyPerformancePage = ({ activeTab: controlledTab, onTabChange, hideTabBar } = {}) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-surface-raised border border-ink/[0.08]">
            ...
            <input
              ...
              className="bg-transparent text-text-primary/85 font-mono tabular-nums text-sm focus:outline-none"
            />
          </div>
          <button
            onClick={() => fetchData(selectedDate)}
            disabled={loading}
            className="px-3 py-2 rounded-md bg-surface-raised border border-ink/[0.08] text-[10px] tracking-[0.2em] font-mono uppercase text-text-primary/60 hover:border-ink/12 hover:text-text-primary transition disabled:opacity-50"
          >
            {loading ? "..." : "Refresh"}
          </button>
        </div>
      ...
      {error && (
        <Card className="p-5 mb-6 border-loss/20">
          <div className="text-sm text-loss">
            <Label className="text-loss/80 mb-1">· Error ·</Label>
            {error}
          </div>
        </Card>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-5 gap-3">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="h-20 rounded-xl bg-surface-raised border border-ink/[0.07] animate-pulse"
              />
            ))}
          </div>
        </div>
      ...
      {data && (
        <div className="space-y-5">
          ...
          <div className="mt-5">
            ...
            {activeTab === "patterns" && (
              <PatternsTab signals={filteredSignals} filters={filters} addFilter={addFilter} />
            )}
            {activeTab === "correlation" && (
              <CorrelationTab signals={filteredSignals} correlationSummary={correlationSummary} />
            )}
            {activeTab === "sectors" && (
              <SectorsTab signals={filteredSignals} filters={filters} addFilter={addFilter} />
            )}
      ...
      {loadingSignal && (
        <div className="fixed bottom-6 right-6 z-50 bg-surface-raised border border-ink/12 rounded-md px-4 py-2 flex items-center gap-3">
          ...
          <span className="text-[11px] font-mono uppercase tracking-wider text-text-primary/70">
            Loading signal...
          </span>
        </div>
      )}
