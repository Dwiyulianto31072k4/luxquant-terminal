...
export default function PnLShareModal({ order, isOpen, onClose }) {
 ...
 <div className="flex h-7 w-7 items-center justify-center rounded-lg text-[10px] font-black" style={{ background: "linear-gradient(135deg, rgb(var(--accent)), rgb(var(--accent)))", color: "rgb(var(--surface))" }}>LQ</div>
 <div>
 ...
 <span className="rounded px-2 py-0.5 text-[10px] font-black uppercase" style={{ background: "rgb(var(--accent) / 0.15)", color: "rgb(var(--accent))", border: "1px solid rgb(var(--line) / 0.3)" }}>{data.leverage}x · {data.margin_mode === "cross" ? "CROSS" : "ISOLATED"}</span>
 )}
 {data.market_type === "spot" && (
 <span className="rounded px-2 py-0.5 text-[10px] font-black uppercase" style={{ background: "rgb(var(--accent) / 0.15)", color: "rgb(var(--accent))", border: "1px solid rgb(var(--line) / 0.3)" }}>SPOT</span>
 )}
 </div>
 </div>
 </div>
 ...
 <p className="mt-0.5 text-[9px] text-accent">{new Date(data.generated_at).toLocaleString()}</p>
 </div>
 ...
 <>
 <svg className="h-4 w-4 text-profit" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
