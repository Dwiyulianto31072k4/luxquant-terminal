...
const EXCHANGE_DEEPLINKS = {
  ...
  okx: {
    futures: (pair) =>
      `https://www.okx.com/trade-swap/${pair.toLowerCase().replace("usdt", "-usdt-swap")}`,
  ...
  mexc: {
    futures: (pair) =>
      `https://www.mexc.com/exchange/${pair.replace("USDT", "_USDT")}?type=linear_swap`,
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="sm" accent={false}>
      ...
      {loading && (
        <div className="flex flex-col items-center py-10">
          <div className="mb-3 h-10 w-10 animate-spin rounded-full border-2 border-ink/10 border-t-accent" />
          <p className="font-semibold text-text-primary">Fetching live data…</p>
          <p className="mt-1 text-xs text-text-muted">
            Querying {order.exchange_id.toUpperCase()} API
          </p>
        </div>
      )}
      ...
      {data && !loading && (
        <div>
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(145deg, rgb(var(--surface)) 0%, #1a1410 50%, rgb(var(--surface)) 100%)",
          ...
          >
            <div
              className="pointer-events-none absolute left-0 top-0 h-32 w-32 rounded-full opacity-30 blur-3xl"
            ...
            <div className="relative mb-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold uppercase text-text-primary">
                  {data.exchange_id}
                </span>
                {data.live_verified && (
                  <span
            ...
            <div className="relative mb-5 flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="font-display text-xl font-bold leading-none text-text-primary">
                    {data.pair}
                  </h3>
                  <span
            ...
            <div className="relative mb-4 py-4 text-center">
              ...
              <p
                ...
                style={{ color: roeColor, fontSize: "52px", textShadow: `0 0 30px ${roeColor}60` }}
              >
                {fmtPct(data.market_type === "futures" ? data.roe_pct : data.pnl_pct)}
              </p>
              <p className="mt-2 font-display text-lg font-bold" style={{ color: pnlColor }}>
            ...
            <div
              className="relative mt-4 flex items-center justify-between pt-4"
              style={{ borderTop: "1px solid rgb(var(--ink) / 0.08)" }}
            >
              <div>
                <p className="mb-1 text-[9px] uppercase tracking-widest text-text-muted">
          ...
          <div className="mt-4 space-y-2">
            ...
            {deeplinkUrl && (
              ...
              >
                <svg
                  className="-mt-0.5 mr-1.5 inline h-3.5 w-3.5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  strokeWidth={1.5}
            ...
            <p className="px-2 text-center text-[10px] leading-relaxed text-text-muted">
              Card generated from live {data.exchange_id.toUpperCase()} API data via your own API
              key. Click "Verify" to cross-check on {data.exchange_id.toUpperCase()} directly.
            </p>
          </div>
        </div>
      )}
