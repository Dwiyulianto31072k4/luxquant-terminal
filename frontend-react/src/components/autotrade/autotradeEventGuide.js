...
export const SKIP_EVENTS = {
  "execution.skip_missing_exchange_account": {
    ...
    fix: "Connect or reconnect the exchange key. Any position you already hold cannot be managed by the bot until you do.",
  },
  "execution.skip_spot_min_notional": {
    title: "Below the exchange's minimum order size",
    what: "The order was smaller than the minimum this venue accepts for this coin.",
  ...
  "execution.skip_symbol_not_listed": {
    title: "Coin not listed on this exchange",
    what: "The pair is offline or does not exist on the venue you connected.",
    why: "Each exchange lists a different book. Agent will not map the signal onto a different ticker.",
    fix: "Nothing for this signal. Enable another venue only if you actually hold that account.",
  },
  "execution.skip_venue_price_outside_entry": {
    title: "Venue price is outside the entry zone",
    what: "The last price on your exchange is outside the signal entry band (including the small pad around it).",
    why: "Entering on a gapped book would mean buying a trade that has already moved. This is not a failed order — nothing was sent.",
    fix: "Nothing. If you want fills farther from the printed entry, that is a product change, not a setting.",
  },
  "execution.skip_leverage_cap": {
    title: "Coin caps leverage below your setting",
    what: "The exchange allows less leverage on this coin than you have configured, and your fallback is set to skip.",
...
export const ALERT_EVENTS = {
  position_unprotected: {
    ...
    fix: "Set a stop on the exchange, or close the position. Agent will not invent a stop for a trade it did not open.",
  },
  exchange_key_invalid: {
    title: "Exchange key stopped working",
    severity: "error",
    what: "Agent can no longer use this API key.",
  ...
  execution_failed: {
    ...
    why: "Exchange rejection, insufficient balance, or a network failure mid-request. If an entry reached the venue but the response was lost, it is detected and flagged rather than assumed away.",
...
export const EXIT_EVENTS = {
  ...
  liquidated: {
    ...
    what: "The exchange force-closed the position because the margin ran out.",
  ...
  auto_deleveraged: {
    ...
    what: "The exchange closed the position as the counterparty to someone else's liquidation.",
  ...
  manual_exit: {
    ...
    why: "Usually closed by hand in the exchange app.",
    fix: "Nothing to do.",
  },
  exchange_close: {
    ...
    why: "Most often this is a trade you opened by hand, which never carried our orders. It can also happen if the position was closed manually on the exchange.",
