...
const statusStyle = (status) => {
  const map = {
    filled: "bg-profit/10 text-profit border-profit/25",
    ...
    <div className="relative overflow-hidden bg-surface-raised border border-ink/[0.06] rounded-md hover:border-ink/[0.12] transition-all">
      ...
      <div className="relative p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <span
            className={`shrink-0 text-[9px] font-mono uppercase tracking-[0.1em] px-2 py-0.5 rounded border ${statusStyle(order.status)}`}
          >
            {order.status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-3">
          ...
          <PriceCell
            label="SL"
            value={fmtNum(order.sl_current || order.sl_price, 6)}
            tone="danger"
          />
        </div>
              ...
              {order.tp_orders.map((t, i) => (
                <div
                  ...
                  className={`flex-1 h-1 rounded-sm transition-all ${
                    t.filled ? "bg-accent" : "bg-ink/[0.06]"
                  }`}
              ...
              {order.tp_orders.map((t, i) => (
                <span
                  ...
                  className={`flex-1 text-center text-[8px] uppercase tracking-[0.1em] ${
                    t.filled ? "text-text-muted" : "text-text-muted/50"
                  }`}
                >
                  {t.level}
                </span>
        ...
        {order.trailing_enabled && (
          <div className="relative overflow-hidden bg-surface-secondary border border-ink/10 rounded mb-3 p-2.5">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <span
                  className={`w-1.5 h-1.5 rounded-full ${order.trailing_activated ? "bg-accent animate-pulse" : "bg-accent/40"}`}
                />
              ...
              <span className="text-[10px] font-mono text-accent tabular-nums shrink-0">
                {order.trailing_value}
                {order.trailing_type === "percent" ? "%" : " USDT"}
              </span>
            </div>
...
const PriceCell = ({ label, value, tone = "neutral" }) => {
  ...
  return (
    <div className="bg-ink/[0.02] border border-ink/[0.04] rounded p-2">
      <p className="text-[9px] font-mono uppercase tracking-[0.15em] text-text-muted mb-1">
        {label}
      </p>
