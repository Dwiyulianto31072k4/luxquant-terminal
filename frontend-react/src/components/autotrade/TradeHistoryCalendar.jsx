        ...
        <div className="overflow-hidden">
          ...
          <div className="grid gap-4 p-5 lg:grid-cols-2">
            <DetailPanel title="Trade outcome">
              ...
              <DetailRow label="Exit order" value={exitOrder?.exchange_order_id || "Recorded in OCO"} />
            </DetailPanel>
            ...
            <DetailPanel title="Execution settings">
              ...
              <DetailRow label="Exit mode" value={config.spot_exit_mode || config.exit_mode || "—"} />
            ...
            <div className="lg:col-span-2">
              <DetailPanel title="Signal snapshot">
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  ...
                  <ModalMetric label="Entry range" value={(signal.entries || []).slice(0, 2).map((v) => fmtNum(v, 8)).join(" – ") || "—"} compact />
                  <ModalMetric label="Selected target" value={fmtNum((signal.tps || [])[Number(config.spot_tp_level ?? config.tp_level ?? 1) - 1], 8)} compact />
                  <ModalMetric label="Selected stop" value={fmtNum((signal.sls || [])[Number(config.spot_sl_level ?? config.sl_level ?? 1) - 1], 8)} compact />
                </div>
                <p className="mt-3 text-xs text-text-muted">
                  Signal text is intentionally hidden here. This view records only the execution-relevant snapshot.
                </p>
              </DetailPanel>
            </div>
          </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard label="Realized PnL" value={formatBasis(totalDisplay, basis)} valueColor={pnlClass(totalDisplay)} />
        <StatCard label="Win rate" value={`${winRate.toFixed(0)}%`} />
      </div>

      {closedTrades.length === 0 ? (
        <EmptyState icon="C" title="No closed live trades yet" hint="Completed live trades will appear here." />
      ) : (
        <>
          <div className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
            <Card>
              <div className="mb-4">
                <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-gold-primary">Cumulative performance</p>
                <p className="mt-1 text-xs text-text-muted">Running realized result across closed live trades.</p>
              </div>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area type="monotone" dataKey="cumulative" stroke={COLORS.gold} strokeWidth={2} fill="url(#pnlFill)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card>
              <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-gold-primary">Outcome mix</p>
              <div className="h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={outcomes} dataKey="value" nameKey="name" innerRadius={55} outerRadius={78} paddingAngle={4}>
                      {outcomes.map((item) => <Cell key={item.name} fill={item.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
          ...
          <Card>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-gold-primary">Daily result</p>
                <p className="mt-1 text-xs text-text-muted">Calendar basis summarized as a compact daily bar chart.</p>
              </div>
            </div>
            <div className="h-44">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailyData}>
                  ...
                  <Bar dataKey="pnl" radius={[3, 3, 0, 0]}>
                    {dailyData.map((item) => <Cell key={item.date} fill={item.pnl >= 0 ? COLORS.up : COLORS.down} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
      ...
      <TradeDetailModal trade={selectedTrade} onClose={() => setSelectedTrade(null)} basis={basis} />
    </div>
  );
}
