...
const RichEditor = ({ value, onChange }) => {
  ...
  return (
    <div className="rounded-xl border border-gold-primary/15 overflow-hidden bg-bg-card">
...
const ResourceEditor = ({ resource, categories = [], onClose, onSaved }) => {
  ...
  const handleCoverChange = (e) => {
    ...
    if (file) { setCoverFile(file); setCoverPreview(URL.createObjectURL(file)); setCoverUrlExt(''); }
  };
  ...
  const fetchPreview = useCallback(async () => {
    ...
    try {
      ...
      if (p.thumbnail_url) { setCoverUrlExt(p.thumbnail_url); setCoverPreview(p.thumbnail_url); setCoverFile(null); }
      ...
      if (yt) { setCoverUrlExt(yt); setCoverPreview(yt); setProvider('youtube'); }
    } finally {
      setFetching(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (type === 'pdf' && !isEdit && !pdfFile) { setError('Please choose a PDF file'); return; }
    if ((type === 'video' || type === 'link') && !sourceUrl.trim()) { setError('Please paste a URL'); return; }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <Field label="Title" required>
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Resource title…" className={inputCls} />
          </Field>
            ...
            <Field label={type === 'video' ? 'Video URL' : 'Link URL'} required>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="shrink-0 px-4 rounded-xl bg-gold-primary/20 text-gold-primary text-sm font-bold hover:bg-gold-primary/30 transition-colors disabled:opacity-40"
                >
                  {fetching ? '…' : 'Fetch'}
                </button>
              </div>
          ...
          <Field label="Excerpt / Summary">
            <textarea value={excerpt} onChange={(e) => setExcerpt(e.target.value)} rows={2} placeholder="One or two lines shown on the card…" className={`${inputCls} resize-none`} />
          </Field>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Category">
              <div className="flex gap-2">
                <select value={category} onChange={(e) => { setCategory(e.target.value); setNewCategory(''); }} className={inputCls}>
            ...
            <Field label="Tags (comma separated)">
              <input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="btc, macro, defi" className={inputCls} />
            </Field>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Author / Source">
              <input value={authorName} onChange={(e) => setAuthorName(e.target.value)} placeholder="e.g. LuxQuant Research" className={inputCls} />
            </Field>
            <Field label="Cover Image">
              <div className="flex gap-3">
                <label className="flex-1 flex items-center justify-center bg-bg-card border-2 border-dashed border-gold-primary/20 rounded-xl px-4 py-3 cursor-pointer hover:border-gold-primary/40 transition-colors text-text-muted text-xs">
                ...
                {coverPreview && (
                  <div className="w-14 h-14 rounded-xl overflow-hidden border border-gold-primary/15 flex-shrink-0">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-3 bg-gradient-to-r from-gold-dark to-gold-primary text-bg-primary rounded-xl text-sm font-bold hover:shadow-gold-glow transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
