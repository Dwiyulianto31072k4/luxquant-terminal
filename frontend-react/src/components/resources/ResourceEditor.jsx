...
const TB = ({ onClick, title, children, active }) => (
 ...
 active ? 'bg-accent/25 text-accent' : 'text-text-secondary hover:bg-ink/10 hover:text-text-primary'
 }`}
 >
 {children}
 </button>
...
const ResourceEditor = ({ resource, categories = [], onClose, onSaved }) => {
 ...
 >
 <div className={`text-sm font-bold ${type === tt.id ? 'text-accent' : 'text-text-primary'}`}>{tt.label}</div>
 ...
 <p className="text-[10px] text-accent uppercase tracking-wider font-bold">{provider || 'preview'}</p>
 ...
 contentFormat === f ? 'bg-accent/20 text-accent' : 'bg-bg-card text-text-muted hover:text-text-primary'
 }`}
 >
 ...
 <input value={newCategory} onChange={(e) => setNewCategory(e.target.value)} placeholder="+ new" className="w-24 bg-bg-card border border-ink/10 rounded-xl px-3 text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-ink/15" />
 </div>
 </Field>
 ...
 <input type="checkbox" checked={isFeatured} onChange={(e) => setIsFeatured(e.target.checked)} className="accent-accent w-4 h-4" />
