...
const ResourceReader = ({ resource: initial, onClose, _onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (initial?.type === "article" && !initial?.content) {
      setLoading(true);
      resourcesApi
        .get(initial.slug || initial.id)
        .then((full) => {
          if (alive) setResource(full);
        })
        .catch(() => {})
        .finally(() => {
      ...
      >
        ...
        {isPdf && (
          <div className="flex-1 overflow-hidden bg-[#525659] rounded-b-2xl">
            <iframe
              src={`${pdfUrl(resource)}#toolbar=0&navpanes=0&view=FitH`}
        ...
        {isVideo && (
          <div className="flex-1 overflow-y-auto">
            <div className="w-full bg-black" style={{ aspectRatio: "16/9" }}>
              {embedSrc ? (
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: resource.embed_html }}
                />
              ) : null}
            </div>
            <div className="px-5 sm:px-8 py-6 max-w-3xl mx-auto">
              ...
              <h1 className="font-display text-2xl lg:text-3xl font-semibold text-text-primary tracking-tight mt-1 mb-2">
                {resource.title}
              </h1>
        ...
        {isArticle && (
          <div className="flex-1 overflow-y-auto">
            ...
            <article className="px-5 sm:px-10 py-8 max-w-3xl mx-auto">
              ...
              <h1 className="font-display text-2xl lg:text-3xl font-semibold text-text-primary tracking-tight mt-2 mb-3">
                {resource.title}
              </h1>
              ...
              {loading ? (
                <div className="space-y-3 animate-pulse">
                  {[...Array(6)].map((_, i) => (
                    <div
                      key={i}
                      className="h-4 rounded bg-ink/5"
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: resource.content || '<p class="text-text-muted">No content.</p>',
                  }}
                />
              )}
            </article>
