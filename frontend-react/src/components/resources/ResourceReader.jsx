...
const ResourceReader = ({ resource: initial, onClose, onNavigate }) => {
 ...
 <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-ink/12 to-transparent z-10" />
 ...
 {isPdf && (
 <>
 <a href={pdfUrl(resource)} download onClick={(e) => e.stopPropagation()} className="p-2 text-text-muted hover:text-text-primary hover:bg-accent/12 rounded-xl transition-all" title="Download">
 <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
 </a>
 <a href={pdfUrl(resource)} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 text-text-muted hover:text-text-primary hover:bg-accent/12 rounded-xl transition-all" title="Open in new tab">
 <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
 </a>
 </>
 )}
 {isVideo && resource.source_url && (
 <a href={resource.source_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="p-2 text-text-muted hover:text-text-primary hover:bg-accent/12 rounded-xl transition-all" title="Open on source">
 <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
 </a>
 )}
 <button onClick={onClose} className="p-2 text-text-muted hover:text-loss hover:bg-red-500/10 rounded-xl transition-all" title="Close (Esc)">
 ...
 <span className="text-[11px] uppercase tracking-wider text-accent font-bold">{resource.category}</span>
