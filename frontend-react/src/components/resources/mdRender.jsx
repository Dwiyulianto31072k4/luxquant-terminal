  ...
  while ((m = INLINE_RE.exec(text))) {
    ...
    if (m[2] !== undefined)
      ...
      nodes.push(
        <code
          key={key}
          className="rounded bg-ink/10 px-1.5 py-0.5 font-mono text-[.85em] text-accent"
        >
          {m[6]}
        </code>
      );
      ...
      nodes.push(
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-accent underline underline-offset-2 hover:text-accent break-all"
        >
          {m[0]}
        </a>
      );
...
export function renderRich(text) {
  ...
  for (const raw of lines) {
    ...
    if (/^(-{3,}|\*{3,}|_{3,})$/.test(t)) {
      flushAll();
      blocks.push(<hr key={k++} className="my-4 border-ink/10" />);
      continue;
    }
