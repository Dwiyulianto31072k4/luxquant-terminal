...
function renderInline(text, keyBase = 'i') {
 ...
 else if (m[6] !== undefined) nodes.push(<code key={key} className="rounded bg-ink/10 px-1.5 py-0.5 font-mono text-[.85em] text-accent">{m[6]}</code>);
 else if (m[7] !== undefined && m[8] !== undefined) {
 nodes.push(
 <a key={key} href={m[8]} target="_blank" rel="noopener noreferrer" className="text-accent underline underline-offset-2 hover:text-accent">
 {m[7]}
 </a>
 );
 } else {
 // bare URL
 nodes.push(
 <a key={key} href={m[0]} target="_blank" rel="noopener noreferrer" className="text-accent underline underline-offset-2 hover:text-accent break-all">
 {m[0]}
 </a>
 );
 }
