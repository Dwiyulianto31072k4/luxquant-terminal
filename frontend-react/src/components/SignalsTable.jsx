...
const ColumnsMenu = ({ visibleCols, onToggle, onReset }) => {
 ...
 active ? 'bg-accent/20 border-ink/18 text-accent' : 'border-ink/[0.15] text-transparent'
 }`}>
...
const SignalsTable = ({
 ...
 const wrColor = (wr) =>
 wr >= 70 ? 'text-profit' : wr >= 50 ? 'text-amber-400' : 'text-loss';
 ...
 {potentialPct != null ? (
 <span className="text-profit font-medium tabular-nums">+{potentialPct.toFixed(1)}%</span>
 ) : null}
 </>
 ) : null}
 </div>
 ...
 {priceChange !== null ? (
 <span className={`tabular-nums font-medium ${priceChange >= 0 ? "text-profit" : "text-loss"}`}>
 ...
 {v && v.verdict !== "neutral" ? (
 <span className={`px-1.5 py-0.5 border rounded-sm text-[9px] uppercase tracking-wider ${v.verdict === "avoid" ? "bg-red-500/10 text-loss border-red-500/30" : "bg-profit/10 text-profit border-profit/25"}`}>
 {v.verdict === "avoid" ? "Avoid" : "Worth"}{v.coin.risk_score != null ? ` ${v.coin.risk_score}` : ""}
 </span>
 ) : wr != null ? (
 <span className={`px-1.5 py-0.5 border rounded-sm text-[9px] tabular-nums ${wr >= 70 ? "bg-profit/10 text-profit border-profit/25" : wr >= 50 ? "bg-amber-500/10 text-amber-400 border-amber-500/30" : "bg-red-500/10 text-loss border-red-500/30"}`}>
 {wr}%
 </span>
 ) : null}
 </div>
 ...
 {wr != null ? (
 <span className={`px-2 py-0.5 border font-mono text-[9px] uppercase tracking-wider rounded-sm ${wr >= 70 ? "bg-profit/10 text-profit border-profit/25" : wr >= 50 ? "bg-amber-500/10 text-amber-400 border-amber-500/30" : "bg-red-500/10 text-loss border-red-500/30"}`}>
 {wr}%
 </span>
 ) : null}
 {streak ? (
 <span className={`px-2 py-0.5 border font-mono text-[9px] uppercase tracking-wider rounded-sm ${streak.type === "win" ? "bg-profit/10 text-profit border-profit/25" : "bg-red-500/10 text-loss border-red-500/30"}`}>
 {streak.length}{streak.type === "win" ? "W" : "L"}
 </span>
 ) : null}
 {topTag ? (
 <span title={`${fmtTag(topTag.tag)}: ${topTag.wr}% historical win rate when present`} className="px-2 py-0.5 border font-mono text-[9px] uppercase tracking-wider rounded-sm bg-accent/12 text-accent border-ink/12 normal-case max-w-[160px] truncate">
 ...
 {currentPrice ? (
 <p className={`font-mono text-[12px] tabular-nums font-medium ${priceChange !== null ? (priceChange >= 0 ? "text-profit" : "text-loss") : "text-text-primary"}`}>
 ...
 {priceChange !== null ? (
 <p className={`font-mono text-[12px] tabular-nums font-medium ${priceChange >= 0 ? "text-profit" : "text-loss"}`}>
 {priceChange >= 0 ? "+" : ""}{priceChange.toFixed(2)}%
 </p>
 ) : (
 <p className="text-text-primary/30 text-[12px]">-</p>
 ...
 {pct != null ? (
 <p className="text-profit/70 font-mono text-[8px] tabular-nums mt-0.5">+{pct.toFixed(1)}%</p>
 ) : null}
 </div>
 );
 })}
 ...
 {priceChange !== null && (
 <span className={`font-mono text-[10px] tabular-nums mt-0.5 font-medium ${priceChange >= 0 ? 'text-profit' : 'text-loss'}`}>
 {priceChange >= 0 ? '+' : ''}{priceChange.toFixed(2)}%
 </span>
 )}
 </div>
 ...
 <td className="py-3 px-4 text-right">
 <div className="flex flex-col items-end">
 <span className="text-profit font-mono text-sm tabular-nums font-medium">{maxTarget ? formatPrice(maxTarget) : '-'}</span>
 {maxTarget && (() => {
 ...
 <td className="py-3 px-4 text-right">
 <div className="flex flex-col items-end">
 <span className="text-loss font-mono text-sm tabular-nums font-medium">{signal.stop1 ? formatPrice(signal.stop1) : '-'}</span>
 {signal.stop1 && (() => {
 ...
 const c = /high/i.test(rl) ? 'text-loss'
 : /low/i.test(rl) ? 'text-profit'
 : 'text-amber-400';
 return (
 ...
 {s && (
 <span className={`font-mono text-[10px] tabular-nums mt-0.5 font-medium ${s.type === 'win' ? 'text-profit/80' : 'text-loss/80'}`}>
 ...
 <span className="font-mono text-[10px] uppercase tracking-wider text-accent">👆 Click for detail</span>
 <button
