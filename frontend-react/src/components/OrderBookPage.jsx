...
const ImbalanceStrip = ({ imb }) => {
  ...
  const sentimentLabel =
    imb.sentiment_label ||
    (imb.sentiment === "strong_buy"
      ? "Strong Buy"
      : imb.sentiment === "buy"
        ? "Buy"
        : imb.sentiment === "strong_sell"
          ? "Strong Sell"
          : imb.sentiment === "sell"
            ? "Sell"
            : "Balanced");
    ...
    <div className="relative overflow-hidden bg-surface-raised border border-ink/[0.06] rounded-md">
      <div className="relative px-4 py-3 flex flex-wrap items-center gap-x-6 gap-y-2">
        ...
        <div className="flex items-center gap-2.5">
          <span
            className={`w-1.5 h-1.5 rounded-full ${sentimentDot(imb.sentiment)} animate-pulse`}
          />
          <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-text-muted">
            Imbalance
          </span>
          <span
            className={`font-mono text-[11px] uppercase tracking-[0.2em] font-semibold ${sentimentColor(imb.sentiment)}`}
          >
            {sentimentLabel}
          </span>
        </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[10px] font-mono uppercase tracking-[0.2em] text-${color}-400/80 font-semibold`}
          >
            {label}
          </span>
              ...
              <div key={i} className="relative">
                <div className="flex justify-between items-center gap-2 mb-0.5">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={`font-mono text-[11px] tabular-nums text-${color}-400 font-semibold`}
                    >
                      {fmt(w.usd)}
                    </span>
                  </div>
...
const LiqCard = ({ data }) => {
  ...
  return (
    <DerivCard label="Liquidations" value={fmt(total)}>
      {/* Bar */}
          ...
          top.map((l, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-1.5 font-mono text-[10px]">
                <span
                  className={`text-[9px] uppercase tracking-[0.1em] px-1 py-0.5 rounded border ${
                    isLong
      ...
      <div className="space-y-1 mb-3">
        {longs.map((f, i) => (
          <div key={i} className="flex items-center justify-between font-mono">
            ...
            <span className="text-[11px] text-profit tabular-nums">+{f.rate_pct?.toFixed(3)}%</span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-1 mb-3">
        {shorts.map((f, i) => (
          <div key={i} className="flex items-center justify-between font-mono">
            ...
            <span className="text-[11px] text-loss tabular-nums">{f.rate_pct?.toFixed(3)}%</span>
          </div>
        ))}
      </div>
...
const LSCard = ({ data, symbol }) => {
  ...
  return (
    <DerivCard label={`L/S Ratio · ${symbol}`} value={ratio.toFixed(2)}>
      {/* Compact bar */}
      <div className="h-[3px] rounded-sm overflow-hidden flex mb-2 bg-ink/[0.03]">
        <div
      ...
      <p className="text-[10px] font-mono text-text-muted/70 leading-relaxed">
        {dom ? "Longs dominant — squeeze risk on drop" : "Shorts dominant — squeeze risk on rise"}
      </p>
    </DerivCard>
  );
};
            ...
            <div className="lg:col-span-5 space-y-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-3 mt-3">
                  <div className="relative overflow-hidden bg-surface-raised border border-ink/[0.06] rounded-md p-4">
                    <WallsCard walls={walls.buy} total={walls.buy_total_usd} type="buy" />
                  </div>
                  <div className="relative overflow-hidden bg-surface-raised border border-ink/[0.06] rounded-md p-4">
                    <WallsCard walls={walls.sell} total={walls.sell_total_usd} type="sell" />
                  </div>
                </div>
              </div>
            </div>
