...
export const SEV = {
 ...
 info: { border: 'rgb(var(--accent))', bg: 'rgb(var(--accent) / 0.06)', text: 'rgb(var(--accent))' },
};
...
export const MonthlyLineChart = ({ data }) => {
 ...
 {[0, 0.5, 1].map((f, i) => (
 <line key={i} x1={padX} x2={W - padX} y1={padY + f * (H - padY * 2)} y2={padY + f * (H - padY * 2)} stroke="rgb(var(--accent) / 0.06)" strokeWidth="0.5" />
 ))}
 <path d={areaD} fill="url(#wrGrad)" />
 <path d={pathD} fill="none" stroke="rgb(var(--accent))" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
 {points.map((p, i) => (
 <g key={i}>
...
export const CoinDetailModal = ({ coin, currentFlow, onClose }) => {
 ...
 { l:'Avg Outcome', v:coin.avg_outcome, c:coin.avg_outcome==='SL'?'#ef4444':'rgb(var(--accent))' },
 ...
 {statCards.map((s,i) => (
 <div key={i} className="flex flex-col items-center justify-center py-3.5 px-2 rounded-xl bg-ink/[0.02] border border-ink/[0.06] hover:border-ink/12 transition-colors">
 ...
 <p className="text-[9px] font-bold uppercase tracking-widest text-text-muted">Outcome Distribution</p>
 ...
 <div className="border-l border-ink/08"><p className="text-[8px] text-text-muted uppercase tracking-widest mb-1">Best Streak</p><p className="font-mono font-bold text-lg text-text-primary">{coin.tp4_streaks.longest_streak}</p></div>
 <div className="border-l border-ink/08"><p className="text-[8px] text-text-muted uppercase tracking-widest mb-1">Current</p><p className="font-mono font-bold text-lg" style={{ color:coin.tp4_streaks.current_tp4_streak>0?'#22c55e':'rgb(var(--fg-muted))' }}>{coin.tp4_streaks.current_tp4_streak}</p></div>
 </div>
 )}
 </Section>
 ...
 <div className="grid grid-cols-2 gap-y-4 gap-x-6">
 <StatBox label="Score" value={coin.entry_quality.score} color={coin.entry_quality.score==='excellent'?'#22c55e':coin.entry_quality.score==='poor'?'#ef4444':'rgb(var(--accent))'} />
 ...
 <StatBox label="Best Block" value={coin.hour_analysis.best_block?.split(' ')[0]||'—'} color="rgb(var(--accent))" />
 ...
 <p className="text-[10px] font-bold text-loss uppercase tracking-widest">Correlated SL Risk</p>
 </div>
 ...
 <span className="text-[9px] text-loss font-semibold bg-red-500/10 px-1.5 py-0.5 rounded">{cp.co_sl_count}× together</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <p className="text-[9px] font-bold uppercase tracking-widest text-text-muted">Signal History</p>
 ...
 <th key={h} className="px-4 py-3 text-[8px] uppercase tracking-widest text-accent font-semibold">{h}</th>
 ))}
 </tr>
 </thead>
 <tbody className="divide-y divide-accent/5">
 {rows.map((s,i) => (
 <tr key={start+i} className="hover:bg-surface-secondary transition-colors">
 ...
 <td className={`px-4 py-2.5 font-mono text-[12px] font-bold ${s.outcome!=='sl'?'text-green-400':'text-loss'}`}>
 {s.pl_pct}
 </td>
 </tr>
 ))}
