...
const pctColor = (v) =>
 ...
 : Number(v) > 0 ? "text-profit"
 : Number(v) < 0 ? "text-loss" : "text-text-primary/70";
...
export default function SectorCoinsModal({ sector, isOpen, onClose, onOpenSignal, loadingSym }) {
 ...
 <p className="font-mono text-[9px] uppercase tracking-[0.25em] text-text-muted">Narrative · Coins</p>
 ...
 <span className={`text-sm font-semibold truncate ${called ? "text-accent" : "text-text-primary"}`}>
 {c.symbol}
 </span>
 ...
 Tap any coin to view its markets on CoinGecko · <span className="text-text-muted">Call</span> = has a LuxQuant signal · Data: CoinGecko
 </p>
 </Modal>
 );
}
