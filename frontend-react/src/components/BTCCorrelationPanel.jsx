...
export default function BTCCorrelationPanel({ signalId, apiBase = '/api/signals' }) {
  ...
  const { metrics, btc_context, interpretation, is_decoupled, sample_quality, data_source, computed_at } = data;
  ...
  return (
    <div className="rounded-xl border border-gray-700/50 bg-gray-900/60 backdrop-blur p-5 space-y-4 text-gray-200">
      ...
      <div className="flex flex-wrap gap-2">
        <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium border ${riskClass}`}>
      ...
      <div className="space-y-2 pt-2 border-t border-gray-800">
        ...
        {interpretation.regime_warning && (
          <HintRow icon="⚠️"  label="Warning"  text={interpretation.regime_warning} className="text-amber-300" />
        )}
        {interpretation.decoupling_note && (
          <HintRow icon="⚡" label="Catalyst" text={interpretation.decoupling_note} className="text-purple-300" />
        )}
      ...
      <div className="pt-3 border-t border-gray-800 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
        ...
        <SnapshotItem
          ...
          valueClass={btc_context.change_24h_pct >= 0 ? 'text-emerald-400' : 'text-rose-400'}
        />
        <SnapshotItem label="Dominance" value={btc_context.dominance ? `${btc_context.dominance}%` : '—'} />
      </div>

      {/* === Footer === */}
      <div className="text-[10px] text-gray-600 flex justify-between pt-2">
        <span>Source: {data_source} · Quality: {sample_quality}</span>
