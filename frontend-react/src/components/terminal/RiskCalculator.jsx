...
export function RiskTab({ view }) {
  ...
  const inputCls = "w-full bg-[#15120d] border border-white/[0.12] rounded-md px-3 py-2 text-[13px] text-white font-mono focus:outline-none focus:border-gold-primary/50";
  ...
  return (
    <>
      <SectionBand title="Position Size & Risk Calculator" desc="Turn any call into a sized, risk-defined trade — how much to buy, your R:R, and where you'd get liquidated. Pick a signal to prefill, or enter levels manually." />

      <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-3">
        {/* ── inputs ── */}
        <div className="relative rounded-2xl bg-[#0a0805] border border-white/[0.07] overflow-hidden p-4 space-y-3">
          ...
          <div>
            ...
            {q && options.length > 0 && (
              <div className="mt-1 rounded-md bg-[#120e08] border border-white/[0.1] max-h-52 overflow-auto">
                {options.map((s) => (
                  <button key={s.signal_id} onClick={() => prefill(s)} className="w-full flex items-center gap-2 px-2.5 py-1.5 hover:bg-white/[0.05] text-left">
                    ...
                    <span className="font-mono text-[11px] text-white/85">{s.pair.replace(/USDT$/i, "")}</span>
          ...
          <div className="flex gap-1 rounded-md bg-[#0c0a07] border border-white/[0.1] p-0.5">
            {["long", "short"].map((sd) => (
              <button key={sd} onClick={() => setSide(sd)} className={`flex-1 py-1.5 rounded-sm font-mono text-[10px] uppercase tracking-wider transition-colors ${side === sd ? (sd === "long" ? "bg-positive text-[#08160c] font-semibold" : "bg-negative text-[#180808] font-semibold") : "text-text-muted hover:text-white"}`}>{sd}</button>
            ))}
          </div>

          <div className="grid grid-cols-2 gap-2">
            <div><label className={labelCls}>Account ($)</label><input type="number" value={account} onChange={(e) => setAccount(e.target.value)} className={inputCls} /></div>
            <div><label className={labelCls}>Risk / trade (%)</label><input type="number" value={riskPct} onChange={(e) => setRiskPct(e.target.value)} className={inputCls} /></div>
          </div>
          <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div><label className={labelCls}>Stop-loss</label><input type="number" value={sl} onChange={(e) => setSl(e.target.value)} className={inputCls} /></div>
            <div><label className={labelCls}>Target</label><input type="number" value={target} onChange={(e) => setTarget(e.target.value)} className={inputCls} /></div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {!calc ? (
            <div className="rounded-2xl bg-[#0a0805] border border-white/[0.07] py-20 text-center font-mono text-[11px] uppercase tracking-wider text-text-muted">
            ...
            <>
              ...
              {(calc.slPastLiq || !calc.slValidSide || !calc.tgtValidSide) && (
                <div className="rounded-xl border border-negative/30 bg-negative/[0.06] px-4 py-2.5 font-mono text-[10.5px] text-negative/90 space-y-1">
                  {!calc.slValidSide && <div>⚠ Stop-loss is on the wrong side of entry for a {side}.</div>}
                  {!calc.tgtValidSide && <div>⚠ Target is on the wrong side of entry for a {side}.</div>}
                  {calc.slPastLiq && <div>⚠ At {lev}× your liquidation ({fmtP(calc.liq)}) triggers before your stop — lower leverage or widen margin.</div>}
                </div>
              )}
                  ...
                  <div className="relative h-16 rounded-lg bg-white/[0.02] border border-white/[0.06]">
                    ...
                    {[["Stop", S, "#f87171"], ["Entry", E, "#d4a853"], ladder.hasT ? ["Target", T, "#4ade80"] : null].filter(Boolean).map(([lab, v, c]) => (
                      <div key={lab} className="absolute top-0 bottom-0 flex flex-col items-center justify-between" style={{ left: ladder.pos(v), transform: "translateX(-50%)" }}>
                        ...
                        <span className="font-mono text-[9px] text-white/70">{fmtP(v)}</span>
                      </div>
                    ))}
                  </div>
                  <div className="mt-2 font-mono text-[9px] text-text-muted text-center">
                    risk 1R = {fmtUsd(calc.riskUsd)} · {calc.rr != null ? `target = ${calc.rr.toFixed(2)}R (${fmtUsd(calc.rewardUsd)})` : "add a target for R:R"}
                  </div>
                </div>
              )}
            </>
