  ...
  return (
    <div className="rounded-md bg-surface-secondary border border-ink/12 px-3 py-2 font-mono text-[10px] shadow-lg">
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-text-primary">{sym(d.pair)}</span>
      </div>
      <div className="text-text-primary/60">
        RSI(14 · {tf}): <span className="text-text-primary/90">{d.y.toFixed(1)}</span>
      </div>
      {d.pctb != null && (
        <div className="text-text-primary/60">
          %B: <span className="text-text-primary/90">{d.pctb.toFixed(0)}</span>{" "}
          <span className="text-text-primary/40">
            {d.pctb >= 100 ? "above upper band" : d.pctb <= 0 ? "below lower band" : "in band"}
          </span>
        </div>
      )}
      <div className="text-text-primary/45">
        {d.band}
        {d.rsi != null && d.pctb != null && d.rsi <= 30 && d.pctb <= 5
          ? " · double-oversold ✓"
          : d.rsi != null && d.pctb != null && d.rsi >= 70 && d.pctb >= 95
            ? " · double-overbought ✗"
            : ""}
      </div>
    </div>
...
const rsiBand = (v) =>
  v >= 70
    ? { k: "overbought", c: "#F6465D" }
    : v >= 60
      ? { k: "strong", c: "#F0B90B" } // stretched but not extreme — accent, not red
      : v > 40
        ? { k: "neutral", c: "rgb(var(--fg) / 0.42)" }
        : v > 30
          ? { k: "weak", c: "#3DDC97" } // soft pos between mid and oversold
          : { k: "oversold", c: "#0ECB81" };
...
export function RsiHeatmapTab({ view, deriv, openPair }) {
  ...
  const rs = useMemo(
    () => rows(view, deriv).filter((r) => rsiOf(r, tf) != null),
    [view, deriv, tf]
  );
  const data = useMemo(
    () =>
      rs.map((r) => {
        ...
        return {
          ...
          rsi: v,
          pctb: r[`pctb_${tf}`],
          sc: statusColorOf(statusMap, r.pair),
        };
      }),
    [rs, statusMap, tf]
  );
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          ...
          desc="Mean across active calls."
          tone={avg >= 60 ? "text-negative" : avg <= 40 ? "text-positive" : undefined}
        />
        <Kpi
          label="Overbought ≥70"
          value={ob}
          desc="Stretched — mean-reversion risk."
          tone={ob ? "text-negative" : undefined}
        />
        <Kpi
          label="Oversold ≤30"
          value={os}
          desc="Beaten down — bounce watch."
          tone={os ? "text-positive" : undefined}
        />
...
const atrTier = (v) =>
  v >= 100
    ? { k: "EXCEEDED", c: "rgb(var(--neg))" }
    : v >= 80
      ? { k: "CRITICAL", c: "rgb(var(--neg))" }
      : v >= 60
    ...
    return (
      <>
        ...
        <div className="rounded-2xl bg-surface-raised border border-ink/[0.07] py-16 text-center font-mono text-[10px] uppercase tracking-wider text-text-muted">
          Warming up — range/ATR fills in after the next worker sweep.
        </div>
      </>
    );
  }
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          label="Exceeded ≥100%"
          value={exceeded}
          desc="Used up their expected day — reversion risk."
          tone={exceeded ? "text-negative" : undefined}
        />
        <Kpi
          label="Most exhausted"
          value={data[0] ? `${data[0].exh.toFixed(0)}%` : "—"}
          desc={data[0] ? sym(data[0].pair) : "—"}
          tone="text-negative"
        />
        <Kpi
          label="Freshest"
          value={data[data.length - 1] ? `${data[data.length - 1].exh.toFixed(0)}%` : "—"}
          desc={data[data.length - 1] ? sym(data[data.length - 1].pair) : "—"}
          tone="text-positive"
        />
            ...
            return (
              ...
              >
                <span className="w-24 flex items-center gap-1.5 shrink-0">
                  ...
                  <span className="font-mono text-[10.5px] text-text-primary/85 group-hover:text-text-primary truncate">
                    {sym(d.pair)}
                  </span>
                </span>
                <span className="flex-1 h-4 rounded-sm bg-ink/[0.03] overflow-hidden relative">
                  <span
                    ...
                    style={{
                      ...
                      background: d.tier.c,
                      opacity: 0.85,
                      outline: sc ? `1px solid ${sc}` : "none",
                    }}
                  />
                  <span
                    className="absolute top-0 bottom-0 border-l border-dashed border-ink/25"
                    style={{ left: `${Math.min((100 / max) * 100, 100)}%` }}
                  />
                </span>
                <span
                  className="w-14 text-right font-mono text-[10.5px] tabular-nums"
                  style={{ color: d.tier.c }}
                >
                  {d.exh.toFixed(0)}%
                </span>
              </button>
...
const sqzTier = (v) =>
  v <= 10
    ...
    : v <= 25
      ...
      : v <= 50
        ? { k: "NORMAL", c: "rgb(var(--accent))" }
        : v <= 75
          ? { k: "LOOSE", c: "rgb(var(--warn))" }
          : { k: "EXPANDED", c: "rgb(var(--neg))" };
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          label="Coiled ≤10%"
          value={coiled}
          desc="Tightest ranges — breakout fuel."
          tone={coiled ? "text-positive" : undefined}
        />
        <Kpi
          label="Most coiled"
          value={data[0] ? `${data[0].bw.toFixed(0)}%` : "—"}
          desc={data[0] ? sym(data[0].pair) : "—"}
          tone="text-positive"
        />
        <Kpi
          label="Most expanded"
          value={data.length ? `${data[data.length - 1].bw.toFixed(0)}%` : "—"}
          desc={data.length ? sym(data[data.length - 1].pair) : "—"}
          tone="text-negative"
        />
      ...
      {!data.length ? (
        <div className="rounded-2xl bg-surface-raised border border-ink/[0.07] py-16 text-center font-mono text-[10px] uppercase tracking-wider text-text-muted">
          Warming up — band width fills in after the next worker sweep.
        </div>
      ) : (
        <div className="relative rounded-2xl bg-surface-raised border border-ink/[0.07] overflow-hidden">
          <span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-ink/12 to-transparent" />
...
function FlowTip({ active, payload }) {
  ...
  return (
    <div className="rounded-md bg-surface-secondary border border-ink/12 px-3 py-2 font-mono text-[10px] shadow-lg">
      ...
      <div className="text-text-primary/60">
        CVD 1h: <span className={d.y >= 0 ? "text-positive" : "text-negative"}>{fmtUsd(d.y)}</span>
      </div>
      <div className="text-text-primary/60">
        Price 24h:{" "}
        <span className={d.x >= 0 ? "text-positive" : "text-negative"}>{d.x.toFixed(1)}%</span>
      </div>
      {d.imb != null && (
        <div className="text-text-primary/60">
          Book:{" "}
          <span className={d.imb >= 0 ? "text-positive" : "text-negative"}>
            {d.imb >= 0 ? "+" : ""}
            {d.imb.toFixed(0)}%
          </span>{" "}
          <span className="text-text-primary/40">{d.imb >= 0 ? "bid-stacked" : "ask-stacked"}</span>
        </div>
      )}
...
export function OrderFlowTab({ view, deriv, cvd, ob, openPair }) {
  ...
  const rowsD = useMemo(() => {
    ...
    (view || []).forEach((s) => {
      ...
      const tag =
        x >= 0 && y < 0
          ? "distribution — sold into strength"
          : x < 0 && y > 0
            ? "accumulation — bought on weakness"
            : x >= 0 && y >= 0
              ? "healthy up — buyers confirm"
      ...
      out.push({
        ...
        tag,
        imb: book[s.pair]?.imb,
        sc: statusColorOf(statusMap, s.pair),
      });
    });
    return out;
  ...
  const bookRows = useMemo(
    () => rowsD.filter((r) => r.imb != null).map((r) => ({ pair: r.pair, imb: r.imb })),
    [rowsD]
  );
  const bidStacked = useMemo(
    () => [...bookRows].sort((a, b) => b.imb - a.imb).slice(0, 6),
    [bookRows]
  );
  const askStacked = useMemo(
    () => [...bookRows].sort((a, b) => a.imb - b.imb).slice(0, 6),
    [bookRows]
  );
  ...
  if (cvd?.warming)
    return <Warming text="Warming up — order flow streams in after the trade-feed connects." />;
  if (!rowsD.length)
    return (
      <>
        <SectionBand
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          label="Divergences"
          value={distrib.length + accum.length}
          desc="Price & flow disagree — highest-signal."
          tone={distrib.length + accum.length ? "text-warning" : undefined}
        />
        <Kpi
          label="Distribution"
          value={distrib.length}
          desc="Up on falling CVD — reversal risk."
          tone={distrib.length ? "text-negative" : undefined}
        />
        <Kpi
          label="Accumulation"
          value={accum.length}
          desc="Down on rising CVD — bounce watch."
          tone={accum.length ? "text-positive" : undefined}
        />
            ...
            distrib.map((d) => (
              ...
              >
                <span className="flex items-center gap-1.5">
                  <CoinLogo pair={d.pair} size={14} />
                  <span className="font-mono text-[10.5px] text-text-primary/85 group-hover:text-text-primary">
                    {sym(d.pair)}
                  </span>
                </span>
                <span className="font-mono text-[10px] text-negative tabular-nums">
                  {fmtUsd(d.y)}
                </span>
              </button>
            ))
            ...
            <div className="rounded-2xl bg-surface-raised border border-ink/[0.07] p-3">
              ...
              {bidStacked.map((d) => (
                ...
                >
                  ...
                  <span className="flex-1 h-3 rounded-sm bg-ink/[0.03] overflow-hidden relative">
                    <span
                      className="absolute inset-y-0 left-0 rounded-sm bg-positive/70"
                      style={{ width: `${Math.min(Math.abs(d.imb), 100)}%` }}
                    />
                  </span>
                  <span className="w-12 text-right font-mono text-[10px] text-positive tabular-nums">
                    +{d.imb.toFixed(0)}%
                  </span>
                </button>
              ))}
            ...
            <div className="rounded-2xl bg-surface-raised border border-ink/[0.07] p-3">
              ...
              {askStacked.map((d) => (
                ...
                >
                  ...
                  <span className="flex-1 h-3 rounded-sm bg-ink/[0.03] overflow-hidden relative">
                    <span
                      className="absolute inset-y-0 left-0 rounded-sm bg-negative/70"
                      style={{ width: `${Math.min(Math.abs(d.imb), 100)}%` }}
                    />
                  </span>
                  <span className="w-12 text-right font-mono text-[10px] text-negative tabular-nums">
                    {d.imb.toFixed(0)}%
                  </span>
                </button>
              ))}
