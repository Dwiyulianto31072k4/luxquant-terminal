  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Kpi
          label="Net leaving (accumulation)"
          value={fmtMoney(Math.abs(totalOut))}
          tone="text-profit"
        />
        <Kpi label="Net to exchanges (selling)" value={fmtMoney(totalIn)} tone="text-loss" />
      </div>
      ...
      {empty ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <FlowColumn
            title="Leaving exchanges"
            sub="accumulation · bullish"
            rows={bullish}
            color={POS}
          />
          <FlowColumn
            title="Into exchanges"
            sub="selling pressure · bearish"
            rows={bearish}
            color={NEG}
          />
        </div>
