...
export function OITab({ view, deriv, pairFc, openPair }) {
  ...
  const gain = [...rows]
    ...
    .sort((a, b) => b.oi_chg_1h - a.oi_chg_1h);
  const top = gain.slice(0, 8).map((r) => ({ pair: r.pair, v: r.oi_chg_1h }));
  const bottom = gain
    .slice(-8)
    .reverse()
    .map((r) => ({ pair: r.pair, v: r.oi_chg_1h }));
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          ...
          value={fmtMoney(oiTotal)}
          desc={t("terminal.viz.kOiTotalDesc")}
        />
        <Kpi
          label={t("terminal.viz.kOiBuild")}
          value={gain[0] ? fmtPct(gain[0].oi_chg_1h) : "—"}
          desc={gain[0]?.pair}
          tone="text-positive"
        />
        <Kpi
          label={t("terminal.viz.kOiUnwind")}
          value={gain.length ? fmtPct(gain[gain.length - 1].oi_chg_1h) : "—"}
          desc={gain[gain.length - 1]?.pair}
          tone="text-negative"
        />
        <Kpi
          label={t("terminal.viz.kDerivCov")}
          value={`${rows.length}/${rows.length + noDeriv.length}`}
          desc={t("terminal.viz.kDerivCovDesc")}
        />
      </div>
...
export function LongShortTab({ view, deriv, pairFc, openPair, liq }) {
  ...
  const lsrVals = useMemo(() => rows.map((r) => r.lsr).filter((v) => v != null), [rows]);
  const lsrBins = useMemo(
    () =>
      makeBins(
        lsrVals.map((v) => Math.min(v, 4)),
        0.25,
        0,
        4
      ),
    ...
    <div className="space-y-2.5">
      ...
      <div className="rounded-xl border border-ink/[0.06] bg-ink/[0.02] overflow-hidden">
        <div className="px-3.5 py-2 border-b border-ink/[0.04] flex items-center justify-between gap-2">
          <div>
            <div className="text-[12.5px] font-medium text-text-primary/90">
              Liquidations · live
            </div>
            <div className="text-[10px] text-text-muted/70">
              Forced closes stream · big prints = local extremes
            </div>
          </div>
        ...
        {liq?.warming || !liq ? (
          ...
          <div className="max-h-[260px] overflow-auto divide-y divide-ink/[0.03] [scrollbar-width:thin]">
            {events.map((e, i) => (
              <div
                key={`${e.pair}-${e.ts}-${i}`}
                className="flex items-center gap-2 px-3 py-1.5 hover:bg-ink/[0.02]"
              >
                <CoinLogo pair={e.pair} size={15} />
                <span className="font-mono text-[11px] text-text-primary/85 w-16 truncate">
                ...
                <span
                  ...
                  style={{ color: e.side === "long" ? NEG : POS }}
                >
                  {fmtMoney(e.usd)}
                </span>
              ...
              {lsBin && (
                <div className="mt-2 shrink-0 rounded-lg border border-ink/[0.07] bg-ink/[0.02]">
                  ...
                  <div className="max-h-[120px] overflow-y-auto divide-y divide-ink/[0.04] [scrollbar-width:thin]">
                    {binPairs.length === 0 ? (
                      <div className="px-2.5 py-3 text-center font-mono text-[10px] text-text-muted">
                        No pairs in band
                      </div>
                    ) : (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {crowdedLong.slice(0, 12).map((r) => (
                    <span
                      key={r.pair}
                      className="flex items-center gap-1.5 px-2 py-1 rounded-sm border border-negative/25 bg-negative/[0.06] font-mono text-[10px]"
                    >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {crowdedShort.slice(0, 12).map((r) => (
                    <span
                      key={r.pair}
                      className="flex items-center gap-1.5 px-2 py-1 rounded-sm border border-positive/25 bg-positive/[0.06] font-mono text-[10px]"
                    >
                      <CoinPill pair={r.pair} onPair={openPair} />
...
export function FundingTab({ view, deriv, pairFc, openPair }) {
  ...
  const basisRich = bSorted
    .slice(-8)
    .reverse()
    .map((r) => ({ pair: r.pair, v: r.basis }));
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          ...
          value={negTop[0] ? `${negTop[0].v.toFixed(3)}%` : "—"}
          desc={negTop[0]?.pair}
          tone="text-positive"
        />
        <Kpi
          label={t("terminal.viz.kFundPos")}
          value={posTop[0] ? `+${posTop[0].v.toFixed(3)}%` : "—"}
          desc={posTop[0]?.pair}
          tone="text-negative"
        />
        <Kpi
          label={t("terminal.viz.kShortSq")}
          value={shortSqueeze.length}
          desc={t("terminal.viz.kShortSqDesc")}
          tone={undefined}
        />
        <Kpi
          label={t("terminal.viz.kLongSq")}
          value={longSqueeze.length}
          desc={t("terminal.viz.kLongSqDesc")}
          tone={undefined}
        />
      </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {shortSqueeze.slice(0, 10).map((r) => (
                  <span
                    key={r.pair}
                    className="flex items-center gap-1.5 px-2 py-1 rounded-sm border border-ink/12 bg-accent/10 font-mono text-[10px]"
                  >
                    <CoinPill pair={r.pair} onPair={openPair} />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {longSqueeze.slice(0, 10).map((r) => (
                  <span
                    key={r.pair}
                    className="flex items-center gap-1.5 rounded-md border border-negative/25 bg-negative/[0.08] px-2 py-1 font-mono text-[10px]"
                  >
                    <CoinPill pair={r.pair} onPair={openPair} />
                    <span className="text-text-muted">f +{r.fPct.toFixed(3)}%</span>
...
const WINDOWS = {
  ...
  "48h": { interval: "30m", limit: 96 },
  "7d": { interval: "2h", limit: 84 },
};
...
export function VsBtcTab({ view, deriv, pairFc, openPair, movers }) {
    ...
    (async () => {
      ...
      await Promise.all(
        allSymbols.map(async (sym) => {
          try {
            const r = await fetch(
              `${API_BASE}/api/v1/market/klines?symbol=${sym}&interval=${interval}&limit=${limit}`
            );
            if (r.ok) {
  ...
  const chartData = useMemo(() => {
    ...
    return btc.map((k, i) => {
      const row = {
        t: new Date(k.t).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      };
        ...
        render={(h) => (
          <>
            <div className="flex items-center gap-1.5 flex-wrap mb-2.5">
              ...
              <span className="flex items-center gap-1.5 px-2 py-1 rounded-md border border-accent/25 bg-accent/10 font-mono text-[10px]">
                <CoinLogo pair="BTCUSDT" size={14} />
                <span className="text-accent">BTC</span>
              </span>
              {selected.map((p, i) => (
                <span
                  key={p}
                  className="flex items-center gap-1.5 px-2 py-1 rounded-md border border-ink/[0.08] bg-ink/[0.02] font-mono text-[10px]"
                >
                  ...
                  <button
                    type="button"
                    onClick={() => setSel(selected.filter((x) => x !== p))}
                  ...
                  <div className="absolute z-40 mt-1 left-0 min-w-[160px] rounded-lg bg-surface border border-ink/[0.1] shadow-xl p-1">
                    {searchOpts.map((p) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          if (selected.length < 10) setSel([...selected, p]);
                          setQ("");
                        }}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 6, right: 66, left: -14, bottom: 0 }}>
                    ...
                    <Line
                      ...
                      strokeWidth={2.2}
                      dot={false}
                      label={makeEndLabel("BTCUSDT", GOLD, chartData.length)}
                  ...
                  >
                    ...
                    <Bar dataKey="count" name="pairs" radius={[2, 2, 0, 0]}>
                      {makeBins(
                        ...
                        <Cell
                          key={i}
                          fill={b.mid < 30 ? POS : b.mid > 70 ? NEG : GOLD}
                          fillOpacity={0.75}
                        />
                      ))}
                    </Bar>
              ...
              <div className="mt-2 space-y-2">
                <div className="flex flex-wrap gap-1.5 items-center">
                  ...
                  {oversold.slice(0, 8).map((r) => (
                    <span
                      key={r.pair}
                      className="flex items-center gap-1 px-1.5 py-0.5 rounded-sm border border-positive/25 bg-positive/[0.06] font-mono text-[10px]"
                    >
                      <CoinPill pair={r.pair} onPair={openPair} />{" "}
                      <span className="text-positive">{r.rsi}</span>
                    </span>
                  ))}
                </div>
                <div className="flex flex-wrap gap-1.5 items-center">
                  ...
                  {overbought.slice(0, 8).map((r) => (
                    <span
                      key={r.pair}
                      className="flex items-center gap-1 px-1.5 py-0.5 rounded-sm border border-negative/25 bg-negative/[0.06] font-mono text-[10px]"
                    >
                      <CoinPill pair={r.pair} onPair={openPair} />{" "}
                      <span className="text-negative">{r.rsi}</span>
                    </span>
                  ))}
                </div>
              </div>
...
export function MomentumTab({ view, deriv, pairFc, openPair }) {
  ...
  const byRs = [...rows]
    .filter((r) => r.rs_btc != null)
    .sort((a, b) => b.rs_btc - a.rs_btc)
    .slice(0, 10)
    .map((r) => ({ pair: r.pair, v: r.rs_btc }));
  const byAccel = [...rows]
    .filter((r) => r.vol_chg_1h != null)
    .sort((a, b) => b.vol_chg_1h - a.vol_chg_1h)
    .slice(0, 10)
    .map((r) => ({ pair: r.pair, v: r.vol_chg_1h }));
  const scatter = rows
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          ...
          value={accelerating}
          desc={t("terminal.viz.kAccelDesc")}
          tone={accelerating ? "text-positive" : undefined}
        />
        <Kpi
          label={t("terminal.viz.kMedMom")}
          value={medM == null ? "—" : Math.round(medM)}
          desc={t("terminal.viz.kMedMomDesc")}
          tone="text-text-primary"
        />
        <Kpi
          label={t("terminal.viz.kStrongest")}
          value={topMom[0] ? Math.round(topMom[0].momentum) : "—"}
          desc={topMom[0]?.pair}
          tone="text-positive"
        />
        <Kpi
          label={t("terminal.viz.kFading")}
          value={fading}
          desc={t("terminal.viz.kFadingDesc")}
          tone={fading ? "text-negative" : undefined}
        />
      </div>
...
export function SqueezeTab({ view, deriv, pairFc, openPair }) {
  ...
  const longList = [...crowdedLong]
    .sort((a, b) => b.squeeze - a.squeeze)
    .slice(0, 10)
    .map((r) => ({ pair: r.pair, v: r.squeeze, color: NEG }));
  const shortList = [...crowdedShort]
    .sort((a, b) => b.squeeze - a.squeeze)
    .slice(0, 10)
    .map((r) => ({ pair: r.pair, v: r.squeeze, color: POS }));
  const scatter = rows
    ...
    <>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Kpi
          ...
          value={crowdedLong.length}
          desc={t("terminal.viz.kCrowdedLongDesc")}
          tone={crowdedLong.length ? "text-negative" : undefined}
        />
        <Kpi
          label={t("terminal.viz.kCrowdedShort")}
          value={crowdedShort.length}
          desc={t("terminal.viz.kCrowdedShortDesc")}
          tone={crowdedShort.length ? "text-positive" : undefined}
        />
        <Kpi
          label={t("terminal.viz.kTopSqueeze")}
          value={topS[0] ? Math.round(topS[0].squeeze) : "—"}
          desc={topS[0]?.pair}
          tone="text-text-primary"
        />
        <Kpi
          label={t("terminal.viz.kMedSqueeze")}
          value={medS == null ? "—" : Math.round(medS)}
          desc={t("terminal.viz.kMedSqueezeDesc")}
        />
      </div>
