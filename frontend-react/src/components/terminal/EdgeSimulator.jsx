...
const evColor = (ev) =>
  ev == null ? "rgb(var(--fg-muted))" : ev >= 0 ? "rgb(var(--pos))" : "rgb(var(--neg))";
const TIER = {
  reliable: "rgb(var(--pos))",
  moderate: "rgb(var(--accent))",
  unreliable: "rgb(var(--fg-muted))",
};
...
function EdgeTip({ active, payload }) {
  ...
  return (
    <div className="rounded-md bg-surface-secondary border border-ink/12 px-3 py-2 font-mono text-[10px] shadow-lg max-w-[250px]">
      ...
      <div className="text-text-primary/60">
        Sample: <span className="text-text-primary/90">{p.count}</span> ·{" "}
        <span style={{ color: TIER[p.reliability] || "#9ca3af" }}>{p.reliability}</span>
      </div>
...
export function EdgeTab() {
  ...
  useEffect(() => {
    let alive = true;
    Promise.all([
      fetch(`${API_BASE}/api/v1/signals/analyze?time_range=${econRange}`, {
        headers: authHeaders(),
      })
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
      fetch(`${API_BASE}/api/v1/signals/journey-insights/ALL`, { headers: authHeaders() })
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
    ]).then(([a, j]) => {
      if (!alive) return;
      const tierAvg = {};
      (j?.hit_rate_per_tp || []).forEach((h) => {
        tierAvg[h.tp] = h.avg_exit_gain_pct;
      });
  ...
  const economics = useMemo(() => {
    ...
    tiers.forEach((tt) => {
      ...
      if (tt.avg >= 0) grossWin += tt.n * tt.avg;
      else grossLoss += tt.n * Math.abs(tt.avg);
    });
  ...
  const openDrill = useCallback(
    (pattern) => {
      ...
      edgeLabApi
        .getDrill("pattern", pattern, days, "all", 200)
        .then((d) => setDrill(d))
        .catch(() => setDrill(null))
  ...
  const pts = useMemo(
    () =>
      pev.map((p) => ({
        x: Math.max(p.count, 1),
        y: p.win_rate ?? 0,
        z: Math.max(p.count, 1),
        d: p,
      })),
    [pev]
  );
  ...
  const xsA = pts.map((p) => p.x),
    ysA = pts.map((p) => p.y);
  const zEdge = useZoom(
    xsA.length ? Math.min(...xsA) : 1,
    xsA.length ? Math.max(...xsA) : 10,
    ysA.length ? Math.min(...ysA) : 0,
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl bg-surface-raised border border-ink/[0.07] py-16 text-center font-mono text-[10px] uppercase tracking-wider text-text-muted">
          {t("terminal.viz.edgeEmpty")}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
            <Kpi
              label={t("terminal.viz.edgeKBaseline")}
              value={baseline == null ? "—" : baseline.toFixed(1) + "%"}
              desc={t("terminal.viz.edgeKBaselineDesc")}
              tone="text-text-primary"
            />
            <Kpi
              label={t("terminal.viz.edgeKBreadth")}
              value={`${posCount}/${pev.length}`}
              desc={t("terminal.viz.edgeKBreadthDesc")}
              tone={posCount ? "text-positive" : undefined}
            />
            <Kpi
              label={t("terminal.viz.edgeKBest")}
            ...
            />
            <Kpi
              label={t("terminal.viz.edgeKSample")}
              value={
                data?.totals?.signals_resolved != null
                  ? data.totals.signals_resolved.toLocaleString()
                  : "—"
              }
              desc={t("terminal.viz.edgeKSampleDesc")}
            />
          </div>
            ...
            <div className="relative rounded-2xl bg-surface-raised border border-ink/[0.07] overflow-hidden">
              ...
              <div className="px-4 py-2.5 border-b border-ink/[0.12] bg-ink/[0.03]">
                ...
                <div className="text-[10px] text-text-muted mt-0.5 leading-relaxed">
                  Win rate alone is marketing. This is the money math: expected profit per trade,
                  profit factor, realized reward:risk, and where winners exit.
                </div>
              </div>
              <div className="grid grid-cols-2 xl:grid-cols-4 gap-2 p-3">
                <Kpi
                  label="Expectancy / trade"
                  value={(economics.exp >= 0 ? "+" : "") + economics.exp.toFixed(2) + "%"}
                  desc="Avg % return per signal across every outcome."
                  tone={economics.exp >= 0 ? "text-positive" : "text-negative"}
                />
                <Kpi
                  label="Profit Factor"
                  value={economics.pf == null ? "—" : economics.pf.toFixed(2)}
                  desc="Gross win ÷ gross loss. >1 = profitable."
                  tone={economics.pf >= 1.5 ? "text-positive" : undefined}
                />
                <Kpi
                  label="Reward : Risk"
                  value={economics.avgR == null ? "—" : economics.avgR.toFixed(2) + "R"}
                  desc="Expectancy in units of the average loss."
                  tone="text-text-primary"
                />
                <Kpi
                  label="Win Rate"
                  value={economics.winRate != null ? economics.winRate.toFixed(1) + "%" : "—"}
                  desc={`${economics.closed.toLocaleString()} resolved`}
                />
              </div>
                ...
                {economics.tiers.map((tt) => {
                  ...
                  return (
                    <div key={tt.k} className="flex items-center gap-2">
                      <span
                        className="w-9 font-mono text-[10px] font-bold"
                      ...
                      <div className="flex-1 h-3.5 rounded bg-ink/[0.04] overflow-hidden">
                        <div
                          ...
                          style={{ width: `${share}%`, background: tt.color, opacity: 0.85 }}
                        />
                      </div>
                      <span className="w-12 text-right font-mono text-[10px] text-text-primary/70">
                        {share.toFixed(0)}%
                      </span>
                      <span
                        className="w-16 text-right font-mono text-[10px]"
                        style={{ color: tt.avg >= 0 ? "rgb(var(--pos))" : "rgb(var(--neg))" }}
                      >
                        {tt.avg >= 0 ? "+" : ""}
                        {tt.avg.toFixed(1)}%
                      </span>
          ...
          <div className="relative rounded-2xl bg-surface-raised border border-ink/[0.07] overflow-hidden">
            <span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-ink/12 to-transparent" />
            <div className="px-4 py-2.5 border-b border-ink/[0.12] bg-ink/[0.03]">
              <div className="text-[12.5px] text-text-primary/90">
                {t("terminal.viz.edgeMapTitle")}
              </div>
              <div className="text-[10px] text-text-muted mt-0.5 leading-relaxed">
                {t("terminal.viz.edgeMapDesc")}
              </div>
            </div>
            <div
              className="p-3"
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 16, right: 24, left: 10, bottom: 28 }}>
                  ...
                  <XAxis
                    ...
                    dataKey="x"
                    scale="log"
                    domain={[Math.max(zEdge.domX[0], 0.9), zEdge.domX[1]]}
                    allowDataOverflow
                    tick={TICK_SM}
                    axisLine={false}
                    tickLine={false}
          ...
          {sel && (
            <div className="relative rounded-2xl bg-surface-raised border border-ink/[0.07] overflow-hidden">
              ...
              <div className="px-4 py-2.5 border-b border-ink/[0.06] flex items-center justify-between gap-3">
                <span className="text-[12.5px] text-text-primary/90">
                  {t("terminal.viz.edgeDrillTitle")}{" "}
                  <span className="text-accent">{nice(sel)}</span>
                </span>
                <button
                  onClick={() => {
                    setSel(null);
                  ...
                  className="font-mono text-[10px] text-text-muted hover:text-text-primary shrink-0"
                >
                  ✕
                </button>
              </div>
                  ...
                  <div className="p-3 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2 max-h-[380px] overflow-auto [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-ink/20">
                    {drill.signals.slice(0, 120).map((s, i) => {
                      ...
                      return (
                        <div
                          key={s.signal_id || i}
                          className="flex items-center gap-2 rounded-lg bg-ink/[0.02] border border-ink/[0.06] px-2.5 py-1.5"
                        >
                          <CoinLogo pair={s.pair} size={18} />
                          <span className="font-mono text-[11px] text-text-primary/85 truncate">
                          ...
                          {pk != null && (
                            <span
                              className={`font-mono text-[10px] tabular-nums shrink-0 ${pk >= 0 ? "text-positive" : "text-negative"}`}
                            >
                              {pk >= 0 ? "+" : ""}
                              {pk.toFixed(1)}%
                            </span>
                  ...
                  {drill.win_rate != null && (
                    <div className="px-4 py-2.5 border-t border-ink/[0.06] font-mono text-[10px] text-text-muted">
                      ...
                      <span className="text-text-primary/80">
                        {drill.win_rate.toFixed(1)}% win rate
                      </span>
                      {baseline != null && (
                        <span>
                          {" "}
                          · vs baseline {baseline.toFixed(1)}% (
                          <span
                            style={{
                              color:
                                drill.win_rate >= baseline ? "rgb(var(--pos))" : "rgb(var(--neg))",
                            }}
                          >
                            {drill.win_rate >= baseline ? "+" : ""}
                            {(drill.win_rate - baseline).toFixed(1)}pp
                          </span>
                          )
                        </span>
                      )}
