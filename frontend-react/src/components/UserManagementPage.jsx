...
// Icons
import { UsersIcon, ShieldIcon, AlertTriangleIcon } from "./admin/Icons";
  ...
  >
    ...
    <div>
      ...
      <p className="text-[11px] mt-0.5" style={{ color: "rgb(var(--fg-muted))" }}>
        As co-admin / founder you can browse users and stats, but grant, revoke, ban, send, and role
...
const UserManagementPage = () => {
  ...
  const fetchContactStats = useCallback(async () => {
    try {
      setContactStats(await adminApi.getContactStats());
    } catch (e) {
  ...
  return (
    <div className="w-full px-4 lg:px-8 space-y-4 pb-24">
      ...
      <OpsQueueBar
        ...
        crm={filters.crm}
        onAnomalyToggle={(key) => setFilters({ ...DEFAULT_FILTERS, anomaly: key })}
        onCrmToggle={(key) => setFilters({ ...DEFAULT_FILTERS, crm: key })}
      />
      ...
      <div className="space-y-3 pt-1">
        ...
        >
          ...
          />
          <UsersPagination page={page} totalPages={totalPages} total={total} onChange={setPage} />
        </div>
      </div>
      ...
      {/* Modals */}
      {grantModal && canWrite && (
        <GrantModal user={grantModal} onClose={() => setGrantModal(null)} onGrant={handleGrant} />
      )}
