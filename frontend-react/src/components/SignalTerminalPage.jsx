...
function squarify(items, x0, y0, W0, H0) {
  ...
  while (i < nodes.length) {
    ...
    for (const n of row) {
      ...
      if (w >= h) {
        rects.push({ d: n.d, x, y: off, w: rowThick, h: len });
        off += len;
      } else {
        rects.push({ d: n.d, x: off, y, w: len, h: rowThick });
        off += len;
      }
    }
    if (w >= h) {
...
const METRICS = {
  ...
  flow_intensity: {
    lbl: "Vol/MCap",
    get: (d) => d.flow_intensity * 100,
    fmt: (v) => (v ? v.toFixed(1) + "%" : "—"),
  },
  price_change_24h: {
    lbl: "Price Δ 24h",
    get: (d) => d.price_change_24h,
...
export default function SignalTerminalPage() {
  ...
  const [colorBy, setColorBy] = useState(() =>
    searchParams.get("view") === "bubble" ? "from_call" : "max_target"
  );
  ...
  const model = useMemo(() => {
    return filteredSignals.map((s) => {
      ...
      const livePrice = priceObj
        ? typeof priceObj === "number"
          ? priceObj
          : priceObj.price
        : (flow?.price ?? null);
      const liveVol =
        priceObj && typeof priceObj !== "number" ? priceObj.volume : (flow?.volume_24h ?? 0);
      const liveChange =
        priceObj && typeof priceObj !== "number" && priceObj.change != null
          ? priceObj.change
          : null;
      ...
      return {
        ...
        streak: ci?.current_streak
          ? ci.current_streak.type === "win"
            ? ci.current_streak.length
            : -ci.current_streak.length
          : 0,
  ...
  return (
    <label className="flex items-center gap-2">
      ...
      <select
        ...
        className="appearance-none bg-surface border border-ink/[0.1] rounded-lg font-mono text-[11px] text-text-primary/80 px-2.5 py-1.5 pr-6 focus:outline-none focus:border-ink/15 cursor-pointer"
      >
  ...
  return (
    <div className="sticky top-0 z-30 flex flex-wrap items-center gap-2 rounded-xl border border-ink/[0.06] bg-surface-raised/95 p-2.5 backdrop-blur-md">
      ...
      <select
        ...
        onChange={(e) => setF({ statusFilter: e.target.value })}
      >
      ...
      <select
        ...
        onChange={(e) => setF({ riskFilter: e.target.value })}
      >
      ...
      <div
        className={chip(filters.streakFilter === "hot")}
        onClick={() => setF({ streakFilter: filters.streakFilter === "hot" ? "all" : "hot" })}
      >
        Hot Streak
      </div>
      <div
        className={chip(filters.corrDecoupled)}
        onClick={() => setF({ corrDecoupled: !filters.corrDecoupled })}
      >
        BTC Decoupled
      </div>
      <div
        className={chip(filters.corrHighAlign)}
      ...
      <button
        ...
        className="px-2 py-1.5 font-mono text-[10px] uppercase tracking-wide text-text-primary/45 hover:text-loss"
      >
        × Reset
      </button>
    </div>
    ...
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
      ...
      <div className="rounded-xl border border-ink/[0.06] bg-surface-raised p-4 lg:col-span-2">
        ...
        <div className="space-y-1.5">
          {topSectors.length === 0 && (
            <div className="font-mono text-[11px] text-text-primary/30">
              No sector snapshot yet.
            </div>
          )}
          {topSectors.map((s) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                width: r.w - 2,
                height: r.h - 2,
                background: colorByMetric(d, colorBy, model),
              }}
            >
  ...
  return (
    <div className="rounded-md bg-surface-secondary border border-ink/12 px-3 py-2 font-mono text-[10px] shadow-lg">
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-text-primary">{d.sym}</span>
        {meta && (
          <span className="font-bold ml-auto" style={{ color: meta.color }}>
            {meta.label}
          </span>
        )}
      </div>
      {ago && <div className="text-text-primary/45 mb-1">called {ago}</div>}
      <div className="text-text-primary/55">
        Win rate:{" "}
      ...
      <div className="text-text-primary/55">
        Vol/MCap:{" "}
        <span className="text-text-primary/90">{(d.flow_intensity * 100).toFixed(2)}%</span>
      </div>
      <div className="text-text-primary/55">
        MCap: <span className="text-text-primary/90">${shortNum(d.market_cap)}</span>
      </div>
      <div className="text-text-primary/55">
        {METRICS[colorBy].lbl}:{" "}
        <span className="text-text-primary/90">
          {METRICS[colorBy].get(d) == null ? "—" : METRICS[colorBy].fmt(METRICS[colorBy].get(d))}
        </span>
      </div>
    </div>
  );
...
function MarketScatter({
  ...
  const xsS = xs.slice().sort((a, b) => a - b),
    ysS = ys.slice().sort((a, b) => a - b);
  const xMed = quantile(xsS, 0.5),
    yMed = quantile(ysS, 0.5);

  const mcaps = pts.map((p) => Math.max(p.d.market_cap || 1, 1));
  const sqLo = Math.sqrt(Math.min(...mcaps)),
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 16, right: 24, left: 14, bottom: 30 }}>
          ...
          <XAxis
            ...
            domain={logX ? [Math.max(z.domX[0], 1e-4), z.domX[1]] : z.domX}
            allowDataOverflow
            tick={TICK_SM}
            axisLine={false}
            tickLine={false}
...
function Matrix({ model, onPick }) {
  ...
  const rows = [...model].sort(
    (a, b) => ((METRICS[sortK].get(a) ?? -1e9) - (METRICS[sortK].get(b) ?? -1e9)) * dir
  );
  return (
              ...
              {MX_COLS.map((k) => {
                ...
                return (
                  <td key={k} className="p-0 text-center">
                    <div
                      onClick={() => onPick(d)}
                      className="m-1 h-9 rounded-md flex items-center justify-center font-mono text-[12px] font-bold cursor-pointer hover:outline hover:outline-1 hover:outline-white transition-transform hover:-translate-y-0.5"
                      style={
                        missing
                          ? {
                          ...
                          : {
                              background: colorByMetric(d, k, model),
                              color: "#ffffff",
...
function SectorView({ model, colorBy, onPick }) {
  const groups = {};
  model.forEach((d) => {
    const k = d.risk ? d.risk.toUpperCase() : "UNKNOWN";
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
      {keys.map((k) => {
        const list = [...groups[k]].sort(
          (a, b) => (METRICS[colorBy].get(b) ?? 0) - (METRICS[colorBy].get(a) ?? 0)
        );
                ...
                return (
                  ...
                  >
                    <span className="flex w-20 shrink-0 items-center gap-1.5">
                      ...
                      <span className="truncate font-mono text-[11px] text-text-primary/70 group-hover:text-text-primary">
                        {d.sym}
                      </span>
                    </span>
                    ...
                    <span className="font-mono text-[10px] w-14 text-right text-text-primary/70">
                      {METRICS[colorBy].fmt(v)}
                    </span>
                  </div>
                );
              })}
  ...
  return (
    <label className="flex items-center gap-1.5">
      ...
      <select
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="appearance-none bg-surface-raised border border-ink/[0.12] rounded-md font-mono text-[11px] text-text-primary/85 px-2 py-1.5 pr-6 focus:outline-none focus:border-ink/15 cursor-pointer"
      >
        {Object.entries(METRICS).map(([k, m]) => (
          <option key={k} value={k} className="bg-surface">
            {m.lbl}
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3 mb-3">
        ...
        <button onClick={() => setLogX((v) => !v)} className={toggle(logX)}>
          {logX ? "Log X" : "Lin X"}
        </button>
        <button onClick={() => setLabels((v) => !v)} className={toggle(labels)}>
          Labels
        </button>
      </div>
      <MarketScatter
...
const SCR = [
  ["sym", "Pair"],
  ["status", "Status"],
  ["price_change_24h", "Δ24h"],
  ["from_call", "From Call"],
  ["win_rate", "WR"],
  ["flow_intensity", "Vol/MC"],
  ["btc_align", "BTC"],
  ["max_target", "Max Tgt"],
  ["market_cap", "MCap"],
];
function Screener({ model, onPick }) {
  ...
  const [dir, setDir] = useState(-1);
  const val = (d, k) =>
    k === "sym" ? d.sym : k === "status" ? d.status : (METRICS[k]?.get(d) ?? 0);
  const rows = [...model].sort((a, b) => {
    const va = val(a, sortK),
      vb = val(b, sortK);
          ...
          <tbody>
            {rows.map((d) => (
              ...
              >
                ...
                <td
                  className="border-b border-ink/[0.04] px-2 py-1.5 text-right"
                  style={{ color: d.price_change_24h >= 0 ? "#34d399" : "#f87171" }}
                >
                ...
                <td
                  ...
                  style={{
                    color:
                      d.from_call == null ? "#8a8478" : d.from_call >= 0 ? "#34d399" : "#f87171",
                  }}
                >
                  {d.from_call == null
                    ? "—"
                ...
                <td className="border-b border-ink/[0.04] px-2 py-1.5 text-right text-text-primary/80">
                  {(d.flow_intensity * 100).toFixed(1)}%
                </td>
                <td className="border-b border-ink/[0.04] px-2 py-1.5 text-right text-text-primary/80">
                  {d.btc_align ?? "—"}
                </td>
                <td className="border-b border-ink/[0.04] px-2 py-1.5 text-right text-profit/90">
                  +{d.max_target.toFixed(0)}%
                </td>
                <td className="border-b border-ink/[0.04] px-2 py-1.5 text-right text-text-primary/80">
                  ${shortNum(d.market_cap)}
                </td>
              </tr>
            ))}
          </tbody>
