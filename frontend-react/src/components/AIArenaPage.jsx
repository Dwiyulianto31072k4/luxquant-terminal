...
const MA_COLORS = {
 ema20: 'rgb(var(--accent))', ema21: 'rgb(var(--accent))', ema50: '#e87e38', ema55: '#e87e38',
...
const ZONE_COLORS = {
 ...
 fair_value: { bg: 'rgb(var(--accent) / 0.08)', border: 'rgb(var(--accent) / 0.25)', text: 'rgb(var(--accent))', label: 'FAIR VALUE' },
...
function BlufHero({ report }) {
 ...
 return (
 <div className="glass-card rounded-2xl p-5 lg:p-6 border border-ink/10" style={{ background: sentimentCfg.bg }}>
...
function TripleScreenStrip({ alignment, tfSummary, onTFClick }) {
 ...
 RSI <span className={`font-bold ${rsi >= 70 ? 'text-loss' : rsi <= 30 ? 'text-green-400' : 'text-accent'}`}>{rsi}</span>
 </div>
 )}
...
function PriceChart({ activeTF, onTFChange }) {
 ...
 grid: { vertLines: { color: 'rgb(var(--accent) / 0.04)' }, horzLines: { color: 'rgb(var(--accent) / 0.04)' } },
 crosshair: { mode: 0, vertLine: { color: 'rgb(var(--accent) / 0.3)', width: 1, style: 2 }, horzLine: { color: 'rgb(var(--accent) / 0.3)', width: 1, style: 2 } },
 rightPriceScale: { borderColor: 'rgb(var(--accent) / 0.1)', scaleMargins: { top: 0.08, bottom: 0.15 } },
 timeScale: { borderColor: 'rgb(var(--accent) / 0.1)', timeVisible: true, secondsVisible: false },
 });
 ...
 grid: { vertLines: { color: 'rgb(var(--accent) / 0.03)' }, horzLines: { color: 'rgb(var(--accent) / 0.03)' } },
 rightPriceScale: { borderColor: 'rgb(var(--accent) / 0.1)', scaleMargins: { top: 0.05, bottom: 0.05 } },
 ...
 <div className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-ink/15 border-t-accent rounded-full animate-spin" /><span className="text-text-muted text-sm">Loading {activeTF}...</span></div>
 </div>
 )}
 ...
 {tech.rsi_14 != null && (
 <span className={`text-[10px] font-bold ${tech.rsi_14 >= 70 ? 'text-loss' : tech.rsi_14 <= 30 ? 'text-green-400' : 'text-accent'}`}>
 ...
 {tech.ema_bullish_cross != null && <span>EMA Cross: <span className={tech.ema_bullish_cross ? 'text-green-400' : 'text-loss'}>{tech.ema_bullish_cross ? 'Bullish' : 'Bearish'}</span></span>}
 {tech.golden_cross != null && <span>Golden Cross: <span className={tech.golden_cross ? 'text-green-400' : 'text-loss'}>{tech.golden_cross ? 'Yes' : 'No'}</span></span>}
 ...
 {tech.trend && <span>Trend: <span className={tech.trend.includes('UP') ? 'text-green-400' : tech.trend.includes('DOWN') ? 'text-loss' : 'text-yellow-400'}>{tech.trend}</span></span>}
 {tech.momentum_12h != null && <span>12h ROC: <span className={tech.momentum_12h > 0 ? 'text-green-400' : 'text-loss'}>{tech.momentum_12h > 0 ? '+' : ''}{tech.momentum_12h}%</span></span>}
 {tech.divergence && <span>Divergence: <span className={tech.divergence === 'bullish' ? 'text-green-400' : 'text-loss'}>{tech.divergence}</span></span>}
 </div>
 </div>
 );
}
...
function InstitutionalFlowRadar({ flow, etfLive }) {
 ...
 <span className={`text-[11px] font-medium ${isPos ? 'text-green-400' : 'text-loss'}`}>
 {isPos ? '+' : ''}${v.toFixed(1)}M
 </span>
 </div>
 );
...
function MacroPulse({ macro, macroLive }) {
 ...
 <div className={`text-base font-bold ${isUp ? 'text-green-400' : delta < 0 ? 'text-loss' : 'text-text-muted'}`}>
 ...
 Corr 30D: <span className={Math.abs(corr) >= 0.4 ? (corr > 0 ? 'text-green-400/80' : 'text-loss/80') : 'text-text-muted'}>
...
function WhatChanged({ data }) {
 ...
 <span className={`ml-1 text-[10px] font-bold ${isPositive ? 'text-green-400' : 'text-loss'}`}>
...
function MetricsRow({ report }) {
 ...
 { label: 'RSI (4H)', value: tfSummary?.['4H']?.rsi_14 || '—', color: (tfSummary?.['4H']?.rsi_14 >= 70) ? '#f87171' : (tfSummary?.['4H']?.rsi_14 <= 30) ? '#4ade80' : 'rgb(var(--accent))' },
...
function RiskFactors({ factors }) {
 ...
 <div className="text-[10px] text-loss uppercase tracking-widest font-bold mb-2">Risk Factors</div>
