...
const MA_COLORS = {
  ema20: "rgb(var(--accent))",
  ema21: "rgb(var(--accent))",
  ema50: "#e87e38",
  ema55: "#e87e38",
  sma100: "#22d3ee",
  sma200: "#8b5cf6",
...
function parseSmartTags(text) {
  ...
  let parsed = text.replace(
    /\[([^\]]+)\]\(([^)]+)\)/g,
    '<span class="text-accent font-mono text-sm font-semibold">$1</span>'
  );
  ...
  return (
    <div className="flex items-center justify-between flex-wrap gap-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="font-display text-xl lg:text-2xl font-semibold text-text-primary">
            AI Arena
          </h2>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={onRefresh}
          className="p-1.5 rounded-lg hover:bg-ink/[0.06] text-text-muted hover:text-accent transition-colors"
          title="Refresh"
        >
        ...
        {history.length > 1 && (
          <div className="relative">
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg text-[10px] font-medium bg-ink/[0.04] text-text-muted hover:bg-ink/[0.08] border border-ink/5 transition-colors flex items-center gap-1"
            >
              Previous
              <svg
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-full mt-1 w-64 max-h-72 overflow-y-auto rounded-xl bg-bg-card border border-ink/10 shadow-xl z-50">
                {history.map((r) => {
...
function BlufHero({ report }) {
  ...
  return (
    <div
      className="glass-card rounded-2xl p-5 lg:p-6 border border-ink/10"
      style={{ background: sentimentCfg.bg }}
    >
      <div
      ...
      {/* BLUF text */}
      <p
        className="text-text-primary text-sm lg:text-base leading-relaxed mb-4 font-medium"
        dangerouslySetInnerHTML={{ __html: parseSmartTags(report.bluf || "") }}
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-ink/[0.04]">
          ...
          <span className="text-sm font-bold" style={{ color: sentimentCfg.color }}>
            {primaryBias.direction || report.bias_direction || "NEUTRAL"} (
            {primaryBias.confidence || report.confidence}%)
          </span>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-ink/[0.04]">
          ...
          <span className="text-sm font-bold text-text-primary">
            ...
            <span className="text-[10px] text-text-muted ml-1">
              ({alignment["1D"]?.state?.[0] || "?"}·{alignment["4H"]?.state?.[0] || "?"}·
              {alignment["1H"]?.state?.[0] || "?"})
            </span>
          </span>
        </div>
        {whatChanged && (
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-ink/[0.04]">
            ...
            <span className="text-[11px] text-text-secondary">
              {whatChanged.diffs
                ...
                .map((d) => {
                  if (d.metric === "Price")
                    return `Price ${d.delta_pct > 0 ? "+" : ""}${d.delta_pct}%`;
...
function ZonesToWatch({ zones, currentPrice }) {
  ...
  return (
    <div className="glass-card rounded-2xl p-4 lg:p-5 border border-ink/08">
      <div className="text-[10px] uppercase tracking-widest font-bold text-accent mb-3">
        Zones to Watch
      </div>
      ...
      {zones.confluence_note && (
        <div className="text-[11px] text-text-secondary italic px-1">{zones.confluence_note}</div>
      )}
    </div>
  );
}
        ...
        {screens.map(({ tf, label, data, tech }) => {
          ...
          return (
            ...
            >
              ...
              <div className="text-base lg:text-lg font-bold mb-1" style={{ color }}>
                {state}
              </div>
              {rsi != null && (
              ...
              {data?.note && (
                <div className="text-[9px] text-text-muted mt-1 leading-snug">{data.note}</div>
              )}
            </button>
          );
        })}
...
function PriceChart({ activeTF, onTFChange }) {
  ...
  useEffect(() => {
    ...
    api
      .get(`/api/v1/ai-arena/chart-data?tf=${activeTF}`)
      .then((res) => {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      try {
        const { createChart, LineSeries, CandlestickSeries, HistogramSeries } =
        ...
        const chart = createChart(containerRef.current, {
          ...
          grid: {
            vertLines: { color: "rgb(var(--accent) / 0.04)" },
            horzLines: { color: "rgb(var(--accent) / 0.04)" },
          },
          crosshair: {
            mode: 0,
            vertLine: { color: "rgb(var(--accent) / 0.3)", width: 1, style: 2 },
            horzLine: { color: "rgb(var(--accent) / 0.3)", width: 1, style: 2 },
          },
          rightPriceScale: {
        ...
        if (kl) {
          [
            { k: "strong_support", c: "#22c55e", st: 0 },
            { k: "support", c: "#86efac", st: 2 },
            { k: "resistance", c: "#fca5a5", st: 2 },
            { k: "strong_resistance", c: "#ef4444", st: 0 },
          ].forEach(({ k, c, st }) => {
            const val = kl[k];
            if (val && val > 0) {
              const s = chart.addSeries(LineSeries, {
        ...
        if (liq) {
          if (liq.nearest_long_cluster) {
            ...
            s.setData(
              chartData.candles.map((c) => ({ time: c.time, value: liq.nearest_long_cluster }))
            );
          }
          if (liq.nearest_short_cluster) {
            ...
            s.setData(
              chartData.candles.map((c) => ({ time: c.time, value: liq.nearest_short_cluster }))
            );
          }
        }
  ...
  useEffect(() => {
    ...
    const initRSI = async () => {
      ...
      const chart = createChart(rsiRef.current, {
        ...
        grid: {
          vertLines: { color: "rgb(var(--accent) / 0.03)" },
          horzLines: { color: "rgb(var(--accent) / 0.03)" },
        },
        rightPriceScale: {
          borderColor: "rgb(var(--accent) / 0.1)",
  ...
  return (
    <div className="glass-card rounded-2xl border border-ink/08 overflow-hidden">
      {/* TF Tabs + Legend */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-ink/5 flex-wrap gap-2">
        <div className="flex items-center gap-1">
          {[
            { key: "1D", label: "Daily", sub: "Tide" },
            { key: "4H", label: "4H", sub: "Wave" },
            { key: "1H", label: "1H", sub: "Ripple" },
          ].map((tf) => (
            <button
              key={tf.key}
              onClick={() => onTFChange(tf.key)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${activeTF === tf.key ? "bg-accent/12 text-accent border border-ink/12" : "text-text-muted hover:text-text-primary hover:bg-ink/[0.04] border border-transparent"}`}
            >
          ...
          {maKeys.map((key) => (
            <span key={key} className="flex items-center gap-1 text-[9px]">
              ...
              <span className="text-text-muted">
                {key.toUpperCase().replace("EMA", "EMA ").replace("SMA", "SMA ")}
              </span>
            </span>
          ))}
        </div>
      ...
      <div className="relative">
        {chartLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-bg-card/60 z-10">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 border-2 border-ink/15 border-t-accent rounded-full animate-spin" />
              <span className="text-text-muted text-sm">Loading {activeTF}...</span>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-t border-ink/5 overflow-x-auto text-[10px] text-text-muted">
        ...
        {tech.volume_ratio != null && (
          <span>
            Vol: <span className="text-text-primary">{tech.volume_ratio}x</span>
          </span>
        )}
        {tech.ema_spread_pct != null && (
          <span>
            EMA Spread: <span className="text-text-primary">{tech.ema_spread_pct}%</span>
          </span>
        )}
        {tech.trend && (
          <span>
          ...
          return (
            ...
            >
              ...
              <p
                ...
                dangerouslySetInnerHTML={{ __html: parseSmartTags(content) }}
              />
            </div>
          );
        })}
...
function DeepAnalysis({ analysis, sections }) {
  ...
  const items = [
    {
      key: "price_structure",
      fallbackKey: "market_overview",
      icon: "📊",
      label: "Price Structure",
    },
    {
      key: "derivatives_liquidity",
      fallbackKey: "derivatives_liquidity",
      icon: "⚡",
      label: "Derivatives & Liquidity",
    },
    {
      key: "onchain_sentiment",
      fallbackKey: "sentiment_onchain",
      icon: "🔗",
      label: "On-Chain & Sentiment",
    },
    { key: "institutional_macro", fallbackKey: null, icon: "🏦", label: "Institutional & Macro" },
    {
      key: "macro_catalysts",
      fallbackKey: "catalysts_stance",
      icon: "🌍",
        ...
        {items.map(({ key, fallbackKey, icon, label }) => {
          ...
          return (
            <div key={key}>
              ...
              <p
                className="text-sm text-text-secondary leading-relaxed pl-6"
                dangerouslySetInnerHTML={{ __html: parseSmartTags(text) }}
              />
            </div>
          );
        })}
...
function InstitutionalFlowRadar({ flow, etfLive }) {
  ...
  const maxAbs = history7d.length ? Math.max(...history7d.map((d) => Math.abs(d.total || 0))) : 0;
  ...
  return (
    <div className="glass-card rounded-2xl p-4 lg:p-5 border border-ink/08">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        ...
        <div className="rounded-xl p-3 bg-ink/[0.02] border border-ink/5">
          ...
          <div className="text-lg font-bold" style={{ color: cbSignalColor }}>
            {cbPct !== null && cbPct !== undefined
              ? `${cbPct >= 0 ? "+" : ""}${cbPct.toFixed(3)}%`
              : "N/A"}
          </div>
          <div className="text-[9px] capitalize" style={{ color: cbSignalColor }}>
            {cbSignalLabel}
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {flow.top_contributors.map((tc, i) => {
              ...
              return (
                <div
                  key={i}
                  className="px-2.5 py-1.5 rounded-lg bg-ink/[0.03] border border-ink/5 flex items-center gap-2"
                >
      ...
      {/* Interpretation */}
      {flow.interpretation && (
        <p className="text-[12px] text-text-secondary leading-relaxed mt-4 italic border-l-2 border-ink/12 pl-3">
          {flow.interpretation}
        </p>
      )}
    </div>
  ...
  const rows = [
    {
      key: "spx",
      label: "S&P 500",
      deltaKey: "spx_change_1d_pct",
      corrKey: "btc_spx_correlation_30d",
    },
    {
      key: "dxy",
      label: "Dollar (DXY)",
      deltaKey: "dxy_change_1d_pct",
      corrKey: "btc_dxy_correlation_30d",
    },
...
function WhatChanged({ data }) {
  ...
  return (
    <div className="glass-card rounded-2xl p-4 border border-ink/08">
      <div className="flex items-center justify-between mb-3">
        <div className="text-[10px] uppercase tracking-widest font-bold text-accent">
          What Changed
        </div>
        <div className="text-[10px] text-text-muted">
          vs {data.vs_previous_id} · {data.hours_ago ? `${data.hours_ago}h ago` : ""}
        </div>
      </div>
...
function MetricsRow({ report }) {
  ...
  const items = [
    ...
    {
      label: "Fear & Greed",
      value: report.fear_greed ?? "—",
      color:
        report.fear_greed <= 25
          ? "#f87171"
          : report.fear_greed <= 45
            ? "#f97316"
            : report.fear_greed <= 55
              ? "#fbbf24"
              : "#4ade80",
    },
    {
      label: "RSI (4H)",
      value: tfSummary?.["4H"]?.rsi_14 || "—",
      color:
        tfSummary?.["4H"]?.rsi_14 >= 70
          ? "#f87171"
          : tfSummary?.["4H"]?.rsi_14 <= 30
            ? "#4ade80"
            : "rgb(var(--accent))",
    },
    {
      label: "Cascade Risk",
      value: (liq.cascade_risk || "—").toUpperCase(),
      color:
        liq.cascade_risk === "high"
          ? "#f87171"
          : liq.cascade_risk === "medium"
            ? "#fbbf24"
            : "#4ade80",
    },
    ...
    {
      label: "Generated",
      value: `${report.generated_in_seconds?.toFixed(0) || "?"}s`,
      color: "#94a3b8",
    },
  ];
  ...
  return (
    <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
      {items.map(({ label, value, color }) => (
        <div key={label} className="glass-card rounded-xl p-2.5 border border-ink/08">
      ...
      <div className="space-y-1.5">
        {factors.map((rf, i) => (
          <div key={i} className="flex items-start gap-2 text-sm text-text-secondary">
            <span className="text-loss/60 mt-0.5 text-xs">⚠</span>
            <span>{rf}</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="text-center py-3 space-y-1">
      ...
      <div className="text-text-muted text-[9px]">
        {report.id} · {report.data_sources} sources ·{" "}
        {report.is_anomaly_triggered ? "⚡ anomaly" : "scheduled"} ·{" "}
...
export default function AIArenaPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      api
        .get("/api/v1/ai-arena/etf-flows")
        .then((r) => setEtfLive(r.data))
        .catch(() => {});
      api
        .get("/api/v1/ai-arena/macro-pulse")
        .then((r) => setMacroLive(r.data))
        .catch(() => {});
    } catch (err) {
  ...
  if (loading)
    return (
      <div className="space-y-4">
        ...
        {[...Array(5)].map((_, i) => (
          <div key={i} className="glass-card rounded-2xl p-6 animate-pulse border border-ink/08">
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-text-muted max-w-md mb-6">
          First report is being generated. Multi-timeframe BTC analysis with DeepSeek R1.
        </p>
        <button
          onClick={fetchData}
          className="px-6 py-3 bg-accent/12 text-accent rounded-xl hover:bg-accent/12 transition-all font-medium"
        >
          Refresh
        </button>
      </div>
  ...
  if (error)
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <button
          onClick={fetchData}
          className="px-5 py-2.5 bg-accent/12 text-accent rounded-xl hover:bg-accent/12 transition-all font-medium"
        >
          Retry
        </button>
      </div>
