        ...
        >
          <div className="relative">
            ...
            >
              ...
              <Combobox.Button className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-text-muted/60 hover:text-text-primary transition-colors">
            ...
            >
              <Combobox.Options
                ...
                style={{
                  ...
                  boxShadow:
                    "0 25px 50px rgb(var(--scrim) / 0.35), 0 0 30px rgb(var(--accent) / 0.05)",
                }}
              >
                {/* Clear option */}
                {country && (
                  ...
                  filteredCountries.map((c) => (
                    <Combobox.Option
                      ...
                      className={({ active, selected }) =>
                        `relative cursor-pointer select-none py-2.5 pl-12 pr-4 transition-colors ${
                          active ? "bg-accent/12 text-text-primary" : "text-text-secondary"
                    ...
                    >
                      {({ selected }) => (
                        <>
                          ...
                          <span
                            className={`block truncate text-[13px] ${selected ? "font-semibold" : "font-normal"}`}
                          >
                            {c.name}
                          </span>
              ...
              >
                {filteredCurrencies.length === 0 ? (
                  ...
                  filteredCurrencies.map((code, idx) => {
                    ...
                    return (
                      <Fragment key={code}>
                        {showDivider && <div className="my-1 mx-3 border-t border-ink/[0.04]" />}
                        <Combobox.Option
                          value={code}
                          className={({ active, selected }) =>
                            `relative cursor-pointer select-none py-2.5 pl-12 pr-4 transition-colors ${
                        ...
                        >
                          {({ selected }) => (
                            <>
                              ...
                              <span
                                className={`block truncate font-mono text-[13px] ${selected ? "font-bold" : ""}`}
                              >
                                {code}
                              </span>
                              {isPopular && (
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-mono uppercase tracking-wider text-text-muted">
                                  Popular
                                </span>
                              )}
                            </>
