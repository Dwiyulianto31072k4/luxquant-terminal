...
const ExpectedValueTab = ({ data, onDrill }) => {
 ...
 const neg = [...data].filter((d) => (d.expected_value ?? 0) < 0).sort((a, b) => a.expected_value - b.expected_value)[0];
 if (neg) {
 out.push({
 kind: "bad",
 ...
 if (!data?.length)
 return <EmptyState title="No EV data available" hint="Need at least 5 signals per pattern in this date range" />;
 ...
 EV = <span className="font-mono text-text-primary/85">(WR × avg_win_peak) + (LR × avg_loss_peak)</span> — the
 expected % return per signal. Positive ={" "}
 <span className="text-profit">edge exists</span>, negative ={" "}
 <span className="text-loss">losing pattern</span>. The bar shows EV magnitude; the dot shows tier
 confidence. A high EV on an <span className="text-loss">unreliable</span> tier is not yet trustworthy.{" "}
 <span className="text-text-muted">Click a row</span> to open the signals behind it.
 </Methodology>
 ...
 <th className="px-3 py-3 text-[10px] tracking-[0.18em] font-mono uppercase text-text-primary/40 font-normal text-left" style={{ minWidth: 160 }}>
 EV magnitude
 </th>
 </tr>
 </thead>
 ...
 <td className="px-3 py-2.5 text-right font-mono tabular-nums text-text-primary/70">{p.count}</td>
 <td className={`px-3 py-2.5 text-right font-mono tabular-nums ${p.win_rate >= 75 ? "text-profit" : p.win_rate >= 50 ? "text-text-primary/75" : "text-loss"}`}>
 ...
 <td className={`px-3 py-2.5 text-right font-mono tabular-nums font-semibold ${ev == null ? "text-text-primary/30" : pos ? "text-profit" : "text-loss"}`}>
 ...
 <span className="text-text-primary/25">| bar centered at 0 · right = +EV, left = −EV · click a row to drill</span>
 </div>
 </Panel>
 </div>
 );
