...
const fmtDate = (iso) => {
 ...
 return isNaN(d) ? String(iso).slice(5, 10)
 : d.toLocaleDateString("en-US", { month: "short", day: "numeric", timeZone: "UTC" });
};

const SECTORS = ["all", "infrastructure", "defi", "ai", "hype", "gamefi", "rwa", "payments", "privacy", "socialfi", "other"];

const wrColorCls = (wr) => (wr >= 85 ? "text-profit" : wr >= 70 ? "text-text-primary/85" : wr >= 50 ? "text-amber-400/90" : "text-loss");
...
const CoinLeaderboardTab = ({ data, onDrill }) => {
 ...
 if (byMedian)
 out.push({ kind: "good", label: "Biggest upside", value: fmtPair(byMedian.pair), sub: `${fmtPeak(byMedian.median_peak)} median peak · ${byMedian.win_rate}% WR · n=${byMedian.count}` });
 const byWr = [...data].filter((d) => d.count >= 15).sort((a, b) => b.win_rate - a.win_rate)[0];
 if (byWr)
 out.push({ kind: "neutral", label: "Highest win rate", value: fmtPair(byWr.pair), sub: `${byWr.win_rate}% WR · ${fmtPeak(byWr.median_peak)} median · n=${byWr.count}` });
 ...
 out.push({ kind: "good", label: "Sweet spot (wins + size)", value: fmtPair(sweet.pair), sub: `${sweet.win_rate}% WR & ${fmtPeak(sweet.median_peak)} median peak` });
 return out;
 }, [data]);

 if (!data?.length)
 return <EmptyState title="No coin data" hint="Need at least 10 resolved signals per coin in this range" />;
 ...
 don't skew the picture. <span className="text-text-muted">Click a coin</span> to open its signals.
 </Methodology>
 ...
 <tbody>
 {filtered.map((c) => {
 const barPct = c.median_peak != null && maxMedian > 0
 ? Math.min(100, Math.max(3, (Math.log10(Math.abs(c.median_peak) + 1) / Math.log10(maxMedian + 1)) * 100))
 : 0;
 return (
 <tr
 key={c.pair}
 ...
 <span className="font-mono tabular-nums text-profit/90 text-[13px]">{fmtPeak(c.median_peak)}</span>
 <div className="h-1.5 w-16 rounded-full bg-ink/[0.05] overflow-hidden shrink-0">
 <div className="h-full rounded-full bg-profit/50" style={{ width: `${barPct}%` }} />
 </div>
 </div>
 </td>
