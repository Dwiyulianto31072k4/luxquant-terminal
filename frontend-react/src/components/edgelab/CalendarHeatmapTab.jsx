  ...
  const { months, insights, monthTotals } = useMemo(() => {
    ...
    if (best)
      ins.push({
        kind: "good",
        label: "Best day",
        value: `${fmtShort(best.date)} · ${best.win_rate.toFixed(0)}%`,
        sub: `${best.wins}/${best.total} resolved`,
      });
    if (worst && best && worst.date !== best.date)
      ins.push({
        kind: "bad",
        label: "Worst day",
        value: `${fmtShort(worst.date)} · ${worst.win_rate.toFixed(0)}%`,
        sub: `${worst.wins}/${worst.total} resolved`,
      });
    if (bestStreak >= 2)
      ins.push({
        kind: "neutral",
        ...
        sub: `≥75% WR, ending ${fmtShort(streakEnd)}`,
      });
  ...
  return (
    <div className="space-y-4">
      ...
      <Methodology title="How to read this">
        Each cell is one day; the big number is that day's win rate and the tint follows the same
        green→red scale as the heatmaps. The small figure is wins/resolved.{" "}
        <span className="text-text-muted">Click any day with signals</span> to open the list of
        trades resolved that day, then click a trade to inspect its full analysis. Faint cells = no
        resolved signals.
      </Methodology>
      ...
      <Panel title="Daily win rate" meta={`${data.length} days`}>
            ...
            return (
              <div key={mo.key} className="flex-1 min-w-[300px]">
                ...
                <div className="flex items-baseline justify-between mb-2.5 px-0.5">
                  <span className="text-sm font-display text-text-primary/90 tracking-tight">
                    {mo.label}
                  </span>
                  {agg?.total > 0 && (
                ...
                <div className="grid grid-cols-7 gap-1 mb-1">
                  {DOW_LABELS.map((d, i) => (
                    <div
                      key={i}
                      className="text-center text-[9px] font-mono uppercase tracking-wider text-text-primary/25"
                    >
                      {d}
                    </div>
                  ))}
                    ...
                    return (
                      ...
                      >
                        ...
                        {has ? (
                          <>
                            <span
                              className="mt-1 font-mono text-[15px] font-bold tabular-nums leading-none text-white"
                              style={{ textShadow: "0 1px 2px rgba(0,0,0,0.5)" }}
                            >
                              {wr?.toFixed(0)}%
                            </span>
                            <span
                              className="mt-1 font-mono text-[9px] tabular-nums leading-none text-white/80"
        ...
        <div className="mt-5 pt-3.5 border-t border-ink/[0.05] flex items-center gap-2 flex-wrap text-[10px] font-mono uppercase tracking-wider text-text-primary/40">
          ...
          <span className="ml-2 text-text-primary/25 normal-case tracking-normal">
            · click a day to drill into its signals
          </span>
        </div>
      </Panel>
    </div>
