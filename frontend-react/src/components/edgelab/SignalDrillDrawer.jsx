...
const SignalRow = ({ s, maxPeak, btcHold, selected, onSelect }) => {
 ...
 <span className={`font-mono tabular-nums text-[12px] shrink-0 ${isWin ? "text-profit" : "text-loss"}`}>
 {peak || "—"}
 </span>
 </div>
...
const DetailPane = ({ s, rank, total, allSignals, btc, btcSeries, opening, onOpenSignal, onBack }) => {
 ...
 <span className={`font-mono tabular-nums text-[12px] ${btc.chg >= 0 ? "text-profit/90" : "text-loss/90"}`}>
 {btc.chg >= 0 ? "+" : ""}{btc.chg}%
 </span>
 </div>
 )}
...
const SignalDrillDrawer = ({ bucket, days, sector, hidden, openingId, onClose, onOpenSignal }) => {
 ...
 {wr != null && (
 <span className={wr >= 60 ? "text-profit" : wr >= 50 ? "text-text-primary/70" : "text-loss"}>
 {wr.toFixed(0)}% WR
 </span>
 )}
 ...
 <div className="font-mono tabular-nums text-base text-profit/90">{fmtPeak(stats.bestPeak) || "—"}</div>
 </div>
 ...
 {error}
 <button onClick={load} className="block mt-2 text-[11px] font-mono uppercase tracking-wider text-loss/70 hover:text-loss">
 ↻ Retry
 </button>
 </div>
 </div>
