  ...
  if (!patterns.length)
    return (
      <EmptyState
        ...
        hint="Need at least 3 signals per (pattern × BTC context)"
      />
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <Methodology title="How to read this">
        ...
        <span className="text-profit">stronger</span>, red ={" "}
        <span className="text-loss">weaker</span>. A big color shift across a row means that pattern
        is regime-dependent — only trade it in the regimes where it's green.{" "}
        <span className="text-text-muted">Click a cell</span> to open its signals.
      </Methodology>

      <Panel title="WR by pattern × BTC context" meta={`${patterns.length} patterns`} pad={false}>
        {/* header row */}
        <div
          className="grid items-center px-5 py-2.5 border-b border-ink/[0.05]"
        ...
        >
          ...
          {BTC_CONTEXTS.map((c) => (
            <div
              key={c}
              className="text-[10px] tracking-[0.15em] font-mono uppercase text-text-primary/40 text-center"
            >
              {c}
            </div>
          ))}
          <div className="text-[10px] tracking-[0.15em] font-mono uppercase text-text-primary/40 text-right">
            N
          </div>
        </div>
        ...
        <div className="px-5 py-2">
          {patterns.map((p) => (
            <div
              ...
              style={{ gridTemplateColumns: "minmax(150px,1.4fr) repeat(4,1fr) 56px", gap: "6px" }}
            >
              <div className="flex items-center font-mono text-xs text-text-primary/85 truncate pr-2">
                {p.pattern}
              </div>
              {BTC_CONTEXTS.map((ctx) => {
                ...
                if (!cell)
                  return (
                    <div
                      key={ctx}
                      className="flex items-center justify-center rounded-md min-h-[40px] bg-ink/[0.015]"
                    >
                ...
                return (
                  <button
                    ...
                    style={{
                      background: wrColor(cell.win_rate, cell.count),
                      opacity: dim ? 0.5 : 1,
                  ...
                  >
                    <span
                      className="font-mono text-[13px] font-bold tabular-nums leading-none text-white"
                      style={{ textShadow: "0 1px 2px rgba(0,0,0,0.5)" }}
                    >
                      {cell.win_rate?.toFixed(0)}%
                    </span>
                    <span
                      className="mt-0.5 font-mono text-[9px] tabular-nums text-white/80"
        ...
        <div className="px-5 py-3 border-t border-ink/[0.05] flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap text-[10px] font-mono uppercase tracking-wider text-text-primary/40">
            {WR_LEGEND.map((s, i) => (
              <span key={i} className="inline-flex items-center gap-1">
                <span
                  className="w-4 h-3 rounded-sm border border-ink/10"
          ...
          <span className="text-[10px] font-mono text-text-primary/25">
            faint cells = n &lt; 5 · click to drill
          </span>
        </div>
      </Panel>
    </div>
