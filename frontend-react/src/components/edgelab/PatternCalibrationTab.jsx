...
const CalibrationRow = ({ row, onDrill }) => {
  ...
  return (
    <button
      ...
      className="group block w-full text-left py-2.5 border-b border-ink/[0.04] last:border-b-0 hover:bg-ink/[0.02] hover:shadow-[inset_2px_0_0_0_rgb(var(--accent) / 0.5)] -mx-5 px-5 cursor-pointer transition"
    >
      <div className="flex items-baseline justify-between gap-3 mb-2 flex-wrap">
        ...
        <div className="flex items-baseline gap-3 font-mono tabular-nums flex-shrink-0">
          ...
          <span className="text-[10px] text-text-primary/35">
            ±{row.win_rate_ci_half_width?.toFixed(1)}pp
          </span>
      ...
      <div className="relative h-4">
        ...
        <div
          className="absolute top-1/2 -translate-y-1/2 w-px h-3.5 bg-ink/12"
          style={{ left: "50%" }}
        />
        <div
        ...
        <div
          ...
          style={{ left: `${ciLo}%`, background: `${color}99` }}
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 w-px h-3"
          style={{ left: `${ciHi}%`, background: `${color}99` }}
        />
        <div
          ...
          style={{
            ...
            height: 8,
            background: color,
            boxShadow: "0 0 0 2px rgb(var(--surface-raised))",
          }}
        />
      </div>
    </button>
  ...
  return (
    <div className="space-y-4">
      ...
      <Methodology title="How calibration works">
        ...
        <span className="text-text-primary/85 font-mono">Wilson 95% confidence interval</span>{" "}
        around each pattern's WR — the band where the true WR most likely sits given sample size.
        Narrow band + large n = <span className="text-profit">reliable</span>; wide band ={" "}
        <span className="text-loss">caution</span>. Tiers:{" "}
        {Object.entries(TIER_DESC).map(([t, d], i) => (
          <span key={t}>
            <span style={{ color: TIER_COLORS[t] }} className="font-mono">
              {TIER_LABELS[t]}
            </span>{" "}
            ({d}){i < 2 ? "; " : "."}
          </span>
        ))}{" "}
        <span className="text-text-muted">Click a pattern</span> to open its signals.
      </Methodology>
          ...
          return (
            ...
            >
              <span className="inline-flex items-center gap-1.5">
                ...
                {TIER_LABELS[t]}{" "}
                <span className="text-text-primary/30 ml-0.5">({tierCounts[t] || 0})</span>
              </span>
            </button>
          );
        })}
      ...
      >
        ...
        <div className="px-5 py-3 border-t border-ink/[0.05] flex items-center gap-4 text-[10px] text-text-primary/35 font-mono flex-wrap">
          ...
          <span className="text-text-primary/25">
            | vertical line = 50% breakeven · click a row to drill
          </span>
        </div>
      </Panel>
    </div>
