...
const STATUS_STYLES = {
  open: "border-ink/15 bg-surface-secondary text-text-secondary",
  closed_win: "border-profit/25 bg-profit/10 text-profit",
  closed_loss: "border-loss/25 bg-loss/10 text-loss",
  breakeven: "border-ink/15 bg-ink/[0.05] text-text-secondary",
};
...
const previewPnl = (form) => {
  const en = parseFloat(form.actual_entry),
    ex = parseFloat(form.actual_exit);
  const sz = parseFloat(form.position_size_usd),
    lv = parseFloat(form.leverage) || 1;
...
const JournalPage = () => {
  ...
  const handleSignalSelect = (d) => {
    setForm((prev) => ({
      ...
      planned_tp1: d.planned_tp1 || "",
      planned_tp2: d.planned_tp2 || "",
      planned_tp3: d.planned_tp3 || "",
      planned_tp4: d.planned_tp4 || "",
      planned_sl: d.planned_sl || "",
      actual_entry: d.planned_entry || prev.actual_entry,
      strategy_tags: prev.strategy_tags.includes("LuxQuant Signal")
        ? prev.strategy_tags
        : ["LuxQuant Signal", ...prev.strategy_tags],
    }));
  };
  ...
  const handleEdit = (e) => {
    setForm({
      ...
      planned_tp1: e.planned_tp1 || "",
      planned_tp2: e.planned_tp2 || "",
      planned_tp3: e.planned_tp3 || "",
      planned_tp4: e.planned_tp4 || "",
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
          <div>
            ...
            <div className="flex items-baseline gap-3 flex-wrap">
              <span
                className={`text-2xl sm:text-3xl font-light tabular-nums tracking-tight leading-none ${finalEq >= 0 ? "text-profit" : "text-loss"}`}
              >
                {fmtMoney(finalEq, { sign: true })}
              </span>
              <span className="text-[11px] font-mono tabular-nums text-text-muted/60 uppercase tracking-[0.1em]">
                cumulative
              </span>
            </div>
          </div>
          ...
          {hasData && (
            <div className="flex items-center gap-4 text-[11px] font-mono tabular-nums">
              <div className="flex flex-col items-end">
                <span className="text-[9px] uppercase tracking-[0.18em] text-text-muted/55">
                  Peak
                </span>
              ...
              <div className="flex flex-col items-end">
                ...
                <span className={`mt-0.5 ${drawdown < -1 ? "text-loss" : "text-text-primary/85"}`}>
                  {fmtPct(drawdown, { sign: true })}
                </span>
              </div>
            </div>
          )}
...
const EquitySvg = ({ points }) => {
  ...
  const accent = isProfit ? "#0ECB81" : "#F6465D";
    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-32 sm:h-36" preserveAspectRatio="none">
      <defs>
        ...
        <linearGradient id="eq-area-neg" x1="0" y1="1" x2="0" y2="0">
          <stop offset="0%" stopColor="#F6465D" stopOpacity="0.25" />
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`inline-flex items-center gap-0.5 text-[9px] font-mono font-medium uppercase tracking-[0.12em] ${isLong ? "text-profit" : "text-loss"}`}
              >
                {isLong ? <IconUpTri /> : <IconDownTri />}
                {entry.direction?.toUpperCase()}
              </span>
            ...
            <p className="text-[10px] font-mono tabular-nums text-text-muted/55 mt-0.5">
              {fmtDateShort(entry.entry_at)}
            </p>
          </div>
        </div>
        <span
          className={`inline-flex items-center text-[9px] font-mono font-medium uppercase tracking-[0.12em] px-1.5 py-0.5 rounded-sm border ${STATUS_STYLES[status]}`}
        >
          {STATUS_LABEL[status] || status.toUpperCase()}
        </span>
      </div>
      <div className="grid grid-cols-4 gap-2 text-[10px] font-mono tabular-nums">
        ...
        <MobileStat
          label="Exit"
          value={entry.actual_exit != null ? `$${fmtPrice(entry.actual_exit)}` : "—"}
          dim={entry.actual_exit == null}
        />
        <MobileStat
          label="P&L"
          value={isOpen || entry.pnl_usd == null ? "—" : fmtMoney(entry.pnl_usd, { sign: true })}
          accent={isOpen ? null : pnlPositive ? "emerald" : "red"}
        />
        <MobileStat
          label="P&L %"
          value={isOpen || entry.pnl_pct == null ? "—" : fmtPct(entry.pnl_pct, { sign: true })}
          accent={isOpen ? null : pnlPositive ? "emerald" : "red"}
        />
      </div>
    </button>
...
const MobileStat = ({ label, value, accent, dim }) => {
  ...
  return (
    <div className="flex flex-col gap-0.5 min-w-0">
      <span className="text-[8.5px] uppercase tracking-[0.15em] text-text-muted/45 font-semibold">
        {label}
      </span>
      <span
        className={`truncate font-medium ${dim ? "text-text-muted/40" : colorMap[accent] || "text-text-primary/85"}`}
      >
        {value}
      </span>
    </div>
      ...
      <div className="relative z-10 p-3.5">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            {monthAgg.trades > 0 && (
              <p
                className={`text-[12px] font-light tabular-nums tracking-tight mt-1.5 ${monthAgg.sum >= 0 ? "text-profit" : "text-loss"}`}
              >
                {fmtMoney(monthAgg.sum, { sign: true })}
                <span className="text-text-muted/45 text-[10px] ml-1.5 font-mono">
                  · {monthAgg.trades} trades
                </span>
              </p>
            )}
          </div>
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={goToday}
              className="h-6 px-2 rounded-sm border border-ink/[0.06] bg-ink/[0.02] text-text-muted/60 hover:text-text-primary hover:border-ink/[0.14] transition-colors text-[9px] font-medium uppercase tracking-[0.15em]"
            >
              Today
            </button>
            <button
        ...
        <div className="grid grid-cols-7 gap-1 mb-1">
          {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
            <div
              key={i}
              className="text-center text-[8.5px] font-mono uppercase tracking-[0.15em] text-text-muted/40 py-1"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((day, i) => {
            ...
            let bg = "rgb(var(--ink) / 0.04)",
              textColor = "rgb(var(--fg-muted))",
              border = "rgb(var(--ink) / 0.06)";
            if (data) {
            ...
            return (
              <div
                ...
                title={
                  data
                    ? `${key}\n${fmtMoney(data.pnl, { sign: true })} · ${data.count} trades`
                    : key
                }
              ...
              >
                <span
                  ...
                  className="text-[10px] font-mono tabular-nums font-medium leading-none"
                >
                  {day}
                </span>
                {data && (
                  <span
                    style={{ color: textColor }}
                    className="text-[7.5px] font-mono tabular-nums leading-none opacity-80"
                  >
        ...
        <div className="mt-3 pt-2.5 border-t border-ink/[0.04] flex items-center justify-between text-[9px] font-mono uppercase tracking-[0.15em] text-text-muted/50">
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-sm" style={{ background: "#0ECB81" }} />
              profit
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-sm" style={{ background: "#F6465D" }} />
              loss
            </span>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <ActionBar
        ...
        onCancel={onCancel}
        onDelete={onDelete}
        canSubmit={!!form.pair && !!form.actual_entry}
      />
    </div>
  );
};
      ...
      try {
        ...
        try {
          const { data } = await api.get(
            "/api/v1/signals/?page=1&page_size=50&sort_by=created_at&sort_order=desc"
          );
  ...
  const handleSelect = async (sig) => {
    ...
    try {
      ...
      onSelect({
        ...
        planned_tp1: sig.target1,
        planned_tp2: sig.target2,
        planned_tp3: sig.target3,
        planned_tp4: sig.target4,
    ...
    return (
      <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-raised">
        <div className="relative z-10 p-3 flex items-center gap-3">
          <span className="w-8 h-8 rounded-md border border-ink/12 bg-accent/12 flex items-center justify-center text-accent flex-shrink-0">
  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full flex items-center gap-3 h-11 px-4 rounded-md border border-dashed border-ink/12 bg-surface-secondary text-text-muted/80 hover:border-transparent hover:text-text-primary hover:bg-surface-secondary transition-all"
      >
        <IconLink className="h-4 w-4 text-text-muted flex-shrink-0" />
        <span className="text-[12px] font-medium tracking-tight">Link LuxQuant Signal</span>
        <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-text-muted ml-auto">
          auto-fill 90%
        </span>
      </button>
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-2 rounded-md overflow-hidden border border-ink/[0.08] shadow-2xl shadow-black/60 bg-surface-raised">
          <div className="p-2 border-b border-ink/[0.06]">
            <label className="group flex h-9 items-center gap-2 bg-ink/[0.03] border border-ink/[0.06] rounded-md px-3 transition-colors focus-within:border-ink/12 focus-within:bg-ink/[0.05]">
          ...
          <div className="max-h-72 overflow-y-auto">
            {loading ? (
              <p className="text-center text-[11px] font-mono uppercase tracking-[0.15em] text-text-muted/50 py-6">
                Loading…
              </p>
            ) : filtered.length === 0 ? (
              <p className="text-center text-[11px] font-mono uppercase tracking-[0.15em] text-text-muted/50 py-6">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {sig.risk_level && (
                        <span
                          className={`text-[8.5px] font-mono font-medium uppercase tracking-[0.12em] px-1 py-px rounded-sm border ${
                  ...
                  <span
                    className={`text-[9px] font-mono font-medium uppercase tracking-[0.12em] px-1.5 py-0.5 rounded-sm border ${getStatusStyle(sig.status)}`}
                  >
...
const LivePreviewBanner = ({ form, preview }) => {
  ...
  const derivedStatus = !form.actual_exit
    ? "open"
    : !hasPnl
      ? "open"
      : Math.abs(preview.pct || 0) < 0.1
        ? "breakeven"
        : preview.pnl >= 0
          ? "closed_win"
          : "closed_loss";
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-raised">
      <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-ink/[0.04]">
        <PreviewCell
          label="P&L $"
          value={hasPnl ? fmtMoney(preview.pnl, { sign: true }) : "—"}
          accent={hasPnl ? (isPositive ? "emerald" : "red") : null}
        />
        <PreviewCell
          label="P&L %"
          value={preview.pct != null ? fmtPct(preview.pct, { sign: true }) : "—"}
          accent={preview.pct != null ? (preview.pct >= 0 ? "emerald" : "red") : null}
        />
        <PreviewCell
          label="R:R"
          value={preview.rr != null ? `${preview.rr >= 0 ? "+" : ""}${preview.rr.toFixed(2)}` : "—"}
          accent={
            preview.rr != null
              ? preview.rr >= 1
                ? "emerald"
        ...
        <PreviewCell
          label="Status"
          value={STATUS_LABEL[derivedStatus]}
          isStatus
...
const PreviewCell = ({ label, value, accent, isStatus, statusKey }) => {
  ...
  return (
    <div className="px-3 py-3 flex flex-col gap-1.5 min-w-0">
      ...
      {isStatus ? (
        <span
          className={`inline-flex items-center text-[10px] font-mono font-medium uppercase tracking-[0.12em] px-1.5 py-0.5 rounded-sm border w-fit ${STATUS_STYLES[statusKey] || STATUS_STYLES.open}`}
        >
          {value}
        </span>
      ) : (
        <span
          className={`text-base sm:text-[17px] font-light tabular-nums tracking-tight truncate ${colorMap[accent] || "text-text-primary/85"}`}
        >
          {value}
        </span>
      )}
    ...
    <div className="relative z-10 p-4 sm:p-5">
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-[12px] font-semibold text-text-primary uppercase tracking-[0.18em]">
            {title}
          </h3>
        </div>
        {description && (
          <p className="text-[10.5px] text-text-muted/55 leading-relaxed">{description}</p>
        )}
      </div>
      {children}
    </div>
...
const FieldLabel = ({ label, required, auto, children }) => (
  <label className="flex flex-col gap-1.5">
    <span className="text-[9.5px] font-semibold uppercase tracking-[0.18em] text-text-muted/55 flex items-center gap-1.5">
      ...
      {auto && (
        <span className="text-[8px] font-mono font-medium uppercase tracking-[0.1em] text-profit/70 bg-profit/10 border border-profit/20 px-1 py-px rounded-sm">
          AUTO
        </span>
      )}
    </span>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
      <FieldLabel label="Planned Entry" auto={isLinked}>
        <input
          ...
          className={`form-input font-mono tabular-nums ${isLinked ? "form-input-auto" : ""}`}
        />
      </FieldLabel>
      ...
      {[1, 2, 3, 4].map((n) => (
        <FieldLabel key={n} label={`TP${n}`} auto={isLinked}>
          <input
            type="number"
            step="any"
            value={form[`planned_tp${n}`]}
            onChange={(e) => update(`planned_tp${n}`, e.target.value)}
            className={`form-input font-mono tabular-nums ${isLinked ? "form-input-auto" : ""}`}
          />
        </FieldLabel>
      ))}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
      <FieldLabel label="Stop Loss" auto={isLinked}>
        <input
          ...
          onChange={(e) => update("planned_sl", e.target.value)}
          className={`form-input font-mono tabular-nums ${isLinked ? "form-input-auto" : ""}`}
        />
      </FieldLabel>
    ...
    <div className="grid grid-cols-3 gap-2">
      ...
      <FieldLabel label="Size (USD)">
        <input
          ...
          onChange={(e) => update("position_size_usd", e.target.value)}
          className="form-input font-mono tabular-nums"
        />
      </FieldLabel>
      <FieldLabel label="Fees (USD)">
        <input
          ...
          step="any"
          value={form.fees_usd}
          onChange={(e) => update("fees_usd", e.target.value)}
          className="form-input font-mono tabular-nums"
        />
      </FieldLabel>
    </div>
  ...
  return (
    <button
      ...
      className={`h-9 rounded-md text-[11px] font-medium uppercase tracking-[0.15em] transition-all border flex items-center justify-center gap-1.5 ${
        active
          ? isLong
            ? "bg-profit text-white border-transparent"
            : "bg-loss text-white border-transparent"
    ...
    <div className="space-y-3">
      ...
      <FieldLabel label="TradingView Link">
        <input
          ...
          onChange={(e) => update("tradingview_link", e.target.value)}
...
const PsychologySection = ({ form, updateEmotion }) => (
  <FormCard title="Psychology & Emotions" icon={<IconBrain className="h-3.5 w-3.5" />}>
    <div className="space-y-4">
      <SliderField
        label="Confidence"
        value={form.emotions.confidence}
        onChange={(v) => updateEmotion("confidence", v)}
        leftLabel="Low"
        rightLabel="High"
        color="emerald"
      />
      <SliderField
        label="FOMO Level"
        value={form.emotions.fomo_level}
        onChange={(v) => updateEmotion("fomo_level", v)}
        leftLabel="None"
        rightLabel="Max"
        color="amber"
      />
      <SliderField
        label="Post-Trade Regret"
        value={form.emotions.regret}
        onChange={(v) => updateEmotion("regret", v)}
        leftLabel="None"
        rightLabel="Max"
        color="red"
      />
      ...
      <div className="pt-1">
        <span className="text-[9.5px] font-semibold uppercase tracking-[0.18em] text-text-muted/55 block mb-2">
          Mood
        </span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        <span className="text-[9.5px] font-semibold uppercase tracking-[0.18em] text-text-muted/55">
          {label}
        </span>
        <span className="text-sm font-light tabular-nums text-text-primary tracking-tight">
          {v}
        </span>
      </div>
      <div className="relative h-7 flex items-center">
        ...
        <div
          ...
          style={{
            width: `${pct}%`,
            background: colorVar.fill,
            boxShadow: `0 0 8px ${colorVar.glow}`,
          }}
        />
        <div className="absolute inset-x-0 flex justify-between pointer-events-none">
          {Array.from({ length: 11 }).map((_, i) => (
            <div key={i} className="w-px h-2 bg-ink/[0.05]" />
          ))}
        </div>
        <input
          type="range"
        ...
        <div
          ...
          style={{
            ...
            borderColor: colorVar.fill,
            boxShadow: `0 0 6px ${colorVar.glow}`,
          }}
        />
      </div>
...
const TagSection = ({
  ...
  const accentMap = {
    gold: { active: "bg-accent text-accent-fg border-transparent", count: "text-accent-fg" },
    profit: { active: "bg-profit text-white border-transparent", count: "text-white" },
    red: { active: "bg-loss text-white border-transparent", count: "text-white" },
    loss: { active: "bg-loss text-white border-transparent", count: "text-white" },
  }[accent];
...
const ActionBar = ({ isEdit, saving, onSubmit, onCancel, onDelete, canSubmit }) => (
  <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-raised sticky bottom-3 z-30">
    <div className="relative z-10 p-3 flex items-center justify-between gap-2 flex-wrap">
      <div className="flex items-center gap-2">
        {onDelete && (
          <button
            type="button"
            onClick={onDelete}
            className="flex h-9 items-center gap-1.5 rounded-md border border-loss/25 bg-loss/10 px-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-loss transition-colors hover:bg-loss/15"
          >
            <IconTrash className="h-3 w-3" />
            <span>Delete</span>
          </button>
      ...
      <button
        ...
        className="flex items-center gap-2 h-9 px-5 rounded-md border border-transparent bg-accent text-accent-fg hover:opacity-90 transition-all text-[11px] font-medium uppercase tracking-[0.14em] disabled:opacity-40 disabled:cursor-not-allowed"
      >
...
const HeroCard = ({ label, value, sub, accent }) => {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-secondary p-4 transition-all hover:border-ink/[0.10]">
      <div className="relative z-10 flex flex-col gap-1">
        <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-text-muted/55">
          {label}
        </span>
        <span
          className={`text-2xl sm:text-[28px] font-light tabular-nums tracking-tight leading-none mt-1 ${colorMap[accent] || "text-text-primary"}`}
        >
          {value}
        </span>
        <span className="text-[10px] font-mono uppercase tracking-[0.12em] text-text-muted/45 mt-1.5">
          {sub}
        </span>
      </div>
    </div>
  ...
  <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-raised">
    <div className="relative z-10 p-4 sm:p-5">
      <div className="flex items-center gap-2.5 mb-3">
        <span className="w-7 h-7 rounded-md border border-ink/12 bg-accent/12 flex items-center justify-center text-accent">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            {insights?.source === "gemini" && (
              <span className="text-[8.5px] font-mono uppercase tracking-[0.12em] text-accent bg-accent/10 border border-ink/10 px-1.5 py-px rounded-sm">
                Gemini
              </span>
            )}
          </div>
      ...
      {!insights || !insights.insights || insights.insights.length === 0 ? (
            ...
            {closedCount < 3 ? (
              <>
                ...
                <span className="text-text-primary font-medium">
                  {Math.max(0, 3 - closedCount)}
                </span>{" "}
                more closed trades to generate insights. Keep journaling — patterns emerge with
                data.
              </>
            ) : (
        ...
        <div className="space-y-2">
          {insights.insights.map((text, i) => (
            <div
              key={i}
              className="flex items-start gap-3 px-3 py-2.5 rounded-md border border-ink/10 bg-surface-secondary"
            >
...
const PnlDistributionCard = ({ entries }) => {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-raised p-4 sm:p-5">
      <div className="relative z-10">
            ...
            {bins.counts.map((c, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5 group">
                  <div className="text-[9px] font-mono tabular-nums text-text-muted/55 h-3.5 leading-none">
                    {c > 0 ? c : ""}
                  </div>
              ...
              return (
                <div key={i}>
                  <div className="flex items-center justify-between mb-1 gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-[11px] font-medium text-text-primary truncate tracking-tight">
                        {row.name}
                      </span>
            ...
            {data.map((row) => {
              ...
              return (
                <div key={row.day} className="text-center">
                  <p className="text-[9px] font-mono uppercase tracking-[0.15em] text-text-muted/45 mb-1.5">
                    {row.day}
                  </p>
                  <div
                    className="rounded-md border py-3.5 flex flex-col items-center justify-center"
                    style={{ background: bg, borderColor: border }}
                  >
                    <span
                      className={`text-[11px] font-mono tabular-nums font-medium ${isZero ? "text-text-muted/35" : isPos ? "text-profit" : "text-loss"}`}
                    >
    ...
    <StreakCard
      ...
      value={
        stats.best_trade_pnl != null
          ? fmtMoney(stats.best_trade_pnl, { sign: true, decimals: 0 })
          : "—"
      }
    ...
    <StreakCard
      ...
      value={
        stats.worst_trade_pnl != null
          ? fmtMoney(stats.worst_trade_pnl, { sign: true, decimals: 0 })
          : "—"
      }
...
const StreakCard = ({ label, value, sub, accent, icon }) => {
  const colorMap = { emerald: "text-profit", profit: "text-profit", red: "text-loss" };
  return (
    <div className="overflow-hidden rounded-lg border border-ink/[0.08] bg-surface-secondary p-4">
      <div className="relative z-10">
        ...
        <p
          className={`text-2xl sm:text-[28px] font-light tabular-nums tracking-tight leading-none ${colorMap[accent] || "text-text-primary"}`}
        >
          {value}
        </p>
        {sub && (
          <p className="text-[10px] font-mono uppercase tracking-[0.12em] text-text-muted/45 mt-1.5">
            {sub}
          </p>
        )}
      </div>
