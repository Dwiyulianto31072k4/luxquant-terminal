...
const StatCard = ({ label, value, sublabel, isGold }) => (
  <div className="rounded-lg border border-ink/[0.08] bg-surface-raised p-4">
    <div className="mb-2 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-text-muted">
      {label}
    </div>
    <div
      className={`mb-1.5 font-mono text-xl font-semibold tabular-nums tracking-tight sm:text-2xl ${isGold ? "text-accent" : "text-text-primary"}`}
    >
      {value}
    </div>
    ...
    <div className="max-w-[1400px] mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <StatCard
          label="Called"
          value={counts.called}
          sublabel="Got a signal"
          isGold={counts.called > 0}
        />
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className="text-text-primary text-sm font-semibold font-mono">
                {coinName(previewSymbol)}
              </span>
              <span className="text-text-muted/60 text-[10px] font-mono uppercase tracking-wider ml-1.5">
                USDT
              </span>
            </div>
...
const CoinRow = ({ item, price, onOpen, onRemove }) => {
  ...
  return (
    <div
      className={`group flex items-center gap-3 px-4 py-3.5 transition-colors ${isCalled ? "bg-profit/[0.06]" : "hover:bg-ink/[0.02]"}`}
    >
      {/* Coin */}
      <div className="flex items-center gap-2.5 flex-1 min-w-0">
        <CoinLogo pair={item.symbol} size={34} />
        <div className="min-w-0">
          <p className="text-text-primary text-sm font-semibold font-mono group-hover:text-text-primary transition-colors">
...
const ListSkeleton = () => (
  <div className="space-y-1.5">
    {[...Array(4)].map((_, i) => (
      <div
        key={i}
        className="rounded-lg border border-ink/[0.08] bg-surface-raised p-3.5 flex items-center gap-3"
      >
