        ...
        {points.map((p, i) => (
          <g key={i}>
            <circle
              ...
              stroke={FC[p.marketCond]?.border || FC.neutral.border}
              strokeWidth="1.5"
            />
            <text x={p.x} y={H + 15} fill="#6b5c52" fontSize="7" textAnchor="middle">
              {fmtDate(p.date)}
            </text>
            <text
              x={p.x}
              y={p.y - 7}
              fill={FC[p.marketCond]?.text || FC.neutral.text}
              fontSize="8"
              fontWeight="700"
  ...
  return (
    ...
    >
      <span
        ...
        style={{ color: rank <= 3 ? "rgb(var(--accent))" : "#6b5c52" }}
      >
        {rank}
      </span>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="font-mono font-bold text-[13px] text-text-primary tracking-wide">
            {coin.pair.replace("USDT", "")}
          </span>
          {coin.anomaly_flags?.slice(0, 2).map((f, i) => (
            <span
              key={i}
              className="text-[6px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-wider"
              style={{
      ...
      <div className="text-right flex-shrink-0 w-16">
        ...
        {coin.current_streak?.length > 0 ? (
          <p
            ...
            style={{ color: coin.current_streak.type === "win" ? "#22c55e" : "#ef4444" }}
          >
            {coin.current_streak.length}
      ...
      <svg
        ...
        viewBox="0 0 6 10"
        fill="none"
        className="opacity-0 group-hover:opacity-40 transition-opacity ml-2"
      >
        <path
          d="M1 1L5 5L1 9"
          stroke="#8a8577"
  ...
  if (loading)
    return (
      <div className="glass-card rounded-xl p-4 border border-ink/08 mb-4 animate-pulse">
        <div className="h-4 bg-accent/12 rounded w-40 mb-2" />
        <div className="h-3 bg-accent/10 rounded w-56" />
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <h3 className="text-text-primary text-base font-bold tracking-wide group-hover:text-accent transition-colors">
              Coin Intelligence
            </h3>
            <span className="text-[9px] font-mono text-accent bg-accent/10 px-2.5 py-1 rounded-md border border-ink/10 shadow-inner">
              {dateLabel}
            </span>
            ...
            <svg
              ...
              viewBox="0 0 10 10"
              fill="none"
              className={`transition-transform duration-300 opacity-50 group-hover:opacity-100 ${collapsed ? "-rotate-90" : ""}`}
            >
              <path
              ...
              <div className="flex gap-4 text-[9px] text-text-muted font-medium">
                <div className="flex items-center gap-1.5">
                  <div
                    ...
                    style={{ background: FC.good.border }}
                  />
                  Good Market (≥70%)
                </div>
                <div className="flex items-center gap-1.5">
                  <div
                    className="w-2 h-2 rounded-full shadow-sm"
                    style={{ background: FC.neutral.border }}
                  />
                  Neutral Market (50-70%)
                </div>
                <div className="flex items-center gap-1.5">
                  <div
                    className="w-2 h-2 rounded-full shadow-sm"
                    style={{ background: FC.bad.border }}
                  />
                  Bad Market (&lt;50%)
                </div>
              </div>
