...
const StarButton = ({ signalId, isStarred: initialStarred, onToggle }) => {
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`p-1.5 rounded-lg transition-all duration-200 ${
          loading
            ...
            : "hover:bg-accent/12 hover:scale-110 active:scale-95"
        }`}
        title={isStarred ? "Remove from watchlist" : "Add to watchlist"}
      >
        {loading ? (
          // Loading spinner
          <svg className="w-5 h-5 animate-spin text-accent" viewBox="0 0 24 24">
            <circle
      ...
      {showTooltip && (
        <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-1.5 bg-bg-card border border-ink/12 rounded-lg shadow-xl z-50 whitespace-nowrap animate-fadeIn">
          <p className="text-xs text-text-primary">Login to add to watchlist</p>
          <div className="absolute left-1/2 -translate-x-1/2 top-full w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-accent/30" />
        </div>
      )}
    </div>
  );
