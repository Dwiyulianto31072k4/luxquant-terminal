...
// ════════════════════════════════════════
// Ticker Bar - Scrolling crypto prices
// ════════════════════════════════════════
const TICKER_COINS = [
...
const FeatureCard = ({ icon, title, desc, delay = 0 }) => (
  <div
    className="glass-card rounded-xl p-6 lg:p-8 border border-gold-primary/10 hover:border-gold-primary/30 transition-all duration-300 group"
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-white relative">
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <div className="flex items-center gap-2 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              <div className="w-10 h-10 relative">
                <img src="/logo.png" alt="LuxQuant" className="w-full h-full object-cover rounded-xl group-hover:scale-105 transition-transform" />
              </div>
              <h1 className="font-display text-lg font-semibold text-white tracking-wide group-hover:text-gold-primary transition-colors">
                LuxQuant
              </h1>
            </div>
            ...
            <nav className="hidden lg:flex items-center gap-6">
              {[
                ...
                <button key={id} onClick={() => scrollTo(id)} className="text-text-secondary hover:text-gold-primary text-sm font-medium transition-colors">
                  {label}
                </button>
              ))}
            </nav>
            ...
            <div className="hidden lg:flex items-center gap-3">
              {isAuthenticated ? (
                <button onClick={goTerminal} className="px-6 py-2.5 rounded-xl font-semibold text-sm transition-all" style={{ background: 'linear-gradient(to right, #d4a853, #8b6914)', color: '#0a0506', boxShadow: '0 0 20px rgba(212, 168, 83, 0.3)' }}>
                  Open Terminal
                </button>
              ) : (
                <>
                  <button onClick={goLogin} className="text-text-secondary hover:text-white text-sm font-medium transition-colors">
                    Login
                  </button>
                  <button onClick={goRegister} className="px-6 py-2.5 rounded-xl font-semibold text-sm transition-all" style={{ background: 'linear-gradient(to right, #d4a853, #8b6914)', color: '#0a0506', boxShadow: '0 0 20px rgba(212, 168, 83, 0.3)' }}>
                    Get Started
                  </button>
                </>
              )}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-bg-primary/98 backdrop-blur-xl border-t border-gold-primary/10 px-4 py-6 space-y-4">
            {[['Features', 'features'], ['Performance', 'performance'], ['Testimonials', 'testimonials']].map(([label, id]) => (
              <button key={id} onClick={() => scrollTo(id)} className="block w-full text-left text-text-secondary hover:text-gold-primary text-sm font-medium py-2">
                {label}
              </button>
            ))}
            <div className="pt-4 border-t border-gold-primary/10 flex flex-col gap-3">
              {isAuthenticated ? (
                <button onClick={goTerminal} className="w-full py-3 rounded-xl font-semibold text-sm" style={{ background: 'linear-gradient(to right, #d4a853, #8b6914)', color: '#0a0506' }}>
                  Open Terminal
                </button>
              ) : (
                <>
                  <button onClick={goLogin} className="w-full py-3 rounded-xl font-semibold text-sm border border-gold-primary/30 text-gold-primary">
                    Login
                  </button>
                  <button onClick={goRegister} className="w-full py-3 rounded-xl font-semibold text-sm" style={{ background: 'linear-gradient(to right, #d4a853, #8b6914)', color: '#0a0506' }}>
                    Get Started
                  </button>
                </>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div>
            ...
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-tight mb-6">
              AI-Powered
              <span className="block bg-gradient-to-r from-gold-light via-gold-primary to-gold-dark bg-clip-text text-transparent">
                Crypto Trading
              </span>
              Intelligence
            </h1>
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                style={{ background: 'linear-gradient(to right, #d4a853, #8b6914)', color: '#0a0506', boxShadow: '0 0 30px rgba(212, 168, 83, 0.4)' }}
              >
                {isAuthenticated ? 'Open Terminal' : 'Start Free Trial'}
              </button>
              <button
                onClick={() => scrollTo('features')}
                className="px-8 py-3.5 rounded-xl font-bold text-sm border border-gold-primary/30 text-gold-primary hover:bg-gold-primary/10 transition-all"
              >
                Explore Features
              </button>
            </div>
          ...
          <div className="hidden lg:flex items-center justify-center">
            <div className="w-full max-w-[500px] aspect-square relative">
              <div className="absolute inset-0 bg-gold-primary/5 rounded-full blur-3xl" />
              <LandingGlobe />
            </div>
          </div>
        </div>
      ...
      {/* ═══════════════════════════════════
          TOP PERFORMERS (same as OverviewPage)
      ═══════════════════════════════════ */}
      ...
      <section id="features" className="relative z-10 max-w-7xl mx-auto px-4 lg:px-8 pb-16 lg:pb-24">
        <div className="text-center mb-12">
          ...
          <p className="text-text-secondary text-lg max-w-2xl mx-auto">
            Everything you need for professional crypto trading, in one terminal.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
          <FeatureCard icon="📡" title="Real-Time Signals" desc="AI-generated trading signals with entry, TP targets, and stop-loss levels. Delivered instantly via Telegram and terminal." delay={0} />
          <FeatureCard icon="🤖" title="AI Arena" desc="Multiple AI models competing to predict market direction. See which model performs best in real-time." delay={100} />
          <FeatureCard icon="📊" title="Market Overview" desc="Total market cap, BTC dominance, Fear & Greed index, sector performance — all in one dashboard." delay={200} />
          <FeatureCard icon="🐋" title="Whale Alerts" desc="Track large transactions and smart money movements. Know when whales are buying or selling." delay={300} />
          <FeatureCard icon="📅" title="Macro Calendar" desc="Economic events that impact crypto markets. CPI, FOMC, earnings — never miss a catalyst." delay={400} />
          <FeatureCard icon="📈" title="Performance Analytics" desc="Track your signal performance, win rates, average gains, and portfolio analytics over time." delay={500} />
        </div>
      </section>
        ...
        <div className="glass-card rounded-2xl p-8 lg:p-16 border border-gold-primary/20 text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="font-display text-3xl lg:text-5xl font-bold text-white mb-4">
              Ready to Trade <span className="text-gold-primary">Smarter</span>?
            </h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={isAuthenticated ? goTerminal : goRegister}
                className="px-10 py-4 rounded-xl font-bold text-sm transition-all hover:scale-105"
                style={{ background: 'linear-gradient(to right, #d4a853, #8b6914)', color: '#0a0506', boxShadow: '0 0 30px rgba(212, 168, 83, 0.4)' }}
              >
              ...
              <a
                ...
                className="px-10 py-4 rounded-xl font-bold text-sm border border-gold-primary/30 text-gold-primary hover:bg-gold-primary/10 transition-all inline-flex items-center justify-center gap-2"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-6">
              <a href="https://t.me/luxquant" target="_blank" rel="noopener noreferrer" className="text-text-muted hover:text-gold-primary transition-colors text-sm">
                Telegram
              </a>
              <a href="https://x.com/luxquant" target="_blank" rel="noopener noreferrer" className="text-text-muted hover:text-gold-primary transition-colors text-sm">
                Twitter
              </a>
            </div>
          </div>
          ...
          <p className="text-text-muted text-xs text-center mt-8 max-w-2xl mx-auto leading-relaxed">
            Disclaimer: Trading cryptocurrency involves significant risk. Past performance does not guarantee future results. LuxQuant provides tools and signals for informational purposes only. Always do your own research and manage your risk.
          </p>
        </div>
      </footer>
    </div>
