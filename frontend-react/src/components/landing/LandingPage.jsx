...
const FAQItem = ({ q, a, isOpen, onClick }) => (
 ...
 isOpen ? "text-accent rotate-180" : "text-text-primary/40 group-hover:text-text-primary/70"
 }`}
 fill="none"
 viewBox="0 0 24 24"
 stroke="currentColor"
...
const TelegramPromo = ({ gainers }) => {
 ...
 style={{
 background: "linear-gradient(135deg, rgb(var(--accent)) 0%, rgb(var(--accent)) 50%, rgb(var(--accent)) 100%)",
...
const RuntimeCounter = () => {
 ...
 style={{
 background: 'radial-gradient(circle at 30% 30%, #fde6a8, rgb(var(--accent)) 60%, rgb(var(--accent)))',
...
const PhoneFlyingCoins = ({ gainers }) => {
 ...
 WebkitBackdropFilter: "blur(14px)",
 boxShadow:
 "0 8px 32px rgb(var(--scrim) / 0.35), 0 0 15px rgb(var(--accent) / 0.15)",
 }}
 >
 <div className="flex items-center gap-2">
 <CoinLogo pair={item.pair} size={20} />
...
const LandingPage = () => {
 ...
 <span>
 <span className="text-accent font-medium">Tip:</span> Click
 ...
 // RAW_DATA <span className="text-accent mx-1">→</span>{" "}
 SANITIZATION <span className="text-accent mx-1">→</span>{" "}
 ALPHA_MODEL <span className="text-accent mx-1">→</span>{" "}
 API_GATEWAY
 </p>
 </div>
 ...
 <span className="text-text-muted font-mono text-[9px] mb-1 block">
 {node.id}
 </span>
 ...
 >
 <span className="text-text-muted font-mono text-[7px] mb-1 block">
 {node.id}
 </span>
 ...
 <svg className="w-3.5 h-3.5 text-text-primary/30 group-hover:text-text-primary group-hover:translate-x-0.5 transition-all flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
