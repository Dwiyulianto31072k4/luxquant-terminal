...
export default function RecentWinnersMarquee({ gainers = [], blendWithHero = true }) {
              ...
              Array.from({ length: 4 }).map((_, i) => (
                <div key={`skel-${i}`} className="rwm-card">
                  ...
                  <div className="rwm-meta">
                    ...
                    <div
                      className="rwm-skel rwm-skel-line"
                      style={{ width: "92%", marginTop: 12 }}
                    />
                    <div
                      className="rwm-skel rwm-skel-line"
            ...
            {track.map((w, i) => {
              ...
              return (
                <button
                  key={`${w.signal_id}-${i}`}
                  onClick={() => {
                    if (!dragRef.current.moved) openProof(w);
                  }}
 ...
 radial-gradient(ellipse 46% 40% at 50% 42%, rgb(var(--accent) / 0.045) 0%, transparent 70%);
 }
 ...
 border-radius: 18px;
 overflow: hidden;
 box-shadow: 0 0 0 1px rgb(var(--accent) / 0.12), 0 22px 52px -22px rgb(var(--scrim) / 0.8);
 transition: box-shadow .3s ease, transform .3s ease;
 }
 .rwm-img { width: 100%; height: auto; display: block; }
 .rwm-card:hover .rwm-img-wrap {
 box-shadow: 0 0 0 1px rgb(var(--accent) / 0.38), 0 28px 60px -18px rgba(139,26,26,0.55);
 transform: translateY(-3px);
 }
 /* Dark: neutral hover shadow (no red glow) */
 [data-theme="dark"] .rwm-card:hover .rwm-img-wrap {
 box-shadow: 0 0 0 1px rgb(var(--accent) / 0.38), 0 28px 60px -18px rgb(var(--scrim) / 0.75);
 }
 ...
 .rwm-card:hover .rwm-proof { color: rgb(var(--accent)); }
