...
const C = {
  gold: "#e7c373",
  goldL: "rgb(var(--accent))",
  gold2: "rgb(var(--accent))",
  gold3: "#b8893c",
  gold4: "rgb(var(--accent))",
  loss: "#f87171",
  win: "#4ade80",
  btc: "#f7931a",
  amber: "#fbbf24",
...
const EVENTS = [
  {
    ...
    note: "Spot BTC ETF approval rally into the first 2024 ATH (~$73k).",
  },
  {
    id: "yen",
    label: "Yen-Carry Crash",
    cat: "event",
    start: "2024-07-25",
    end: "2024-08-20",
    note: "BoJ hike triggered a global carry unwind — BTC -20% to ~$49k in a day.",
  },
  {
    id: "election",
    label: "Election Bull",
    ...
    note: "Trump win → first $100k BTC, peak near the Jan inauguration.",
  },
  {
    id: "tariffQ1",
    label: "Tariff Shock '25",
    cat: "bear",
    start: "2025-02-15",
    end: "2025-04-10",
    note: "Bybit $1.5B hack + 'Liberation Day' tariffs — BTC bottomed ~$74k.",
  },
  {
    id: "meltup",
    label: "Melt-Up to ATH",
    cat: "bull",
    start: "2025-04-10",
    end: "2025-10-06",
    note: "Recovery rally to the cycle ATH ~$126k (Oct 6, 2025).",
  },
  {
    id: "crash1010",
    label: "10.10 Crash",
    ...
    note: "Trump 100% China tariff → record ~$19B liquidations in a single day.",
  },
  {
    id: "postath",
    label: "Post-ATH Bear",
    ...
    note: "~50% drawdown from the ATH into early 2026.",
  },
  {
    id: "bearcycle",
    label: "Bear Market (2025→now)",
    cat: "bear",
    start: "2025-10-06",
    end: "2099-12-31",
    ongoing: true,
    note: "BTC topped at ~$126,198 on Oct 6, 2025 — the cycle's ongoing bear market since. Does the edge survive it?",
  },
  {
    id: "iran",
    ...
    note: "Mideast strikes spiked volatility; relief rally on the Apr 7 ceasefire.",
  },
];
              ...
              >
                ...
                <optgroup label="Bull" className="bg-surface-secondary">
                  {EVENTS.filter((e) => e.cat === "bull").map((e) => (
                    <option key={e.id} value={e.id} className="bg-surface-secondary">
                      {e.label}
                    </option>
                  ))}
                </optgroup>
  ...
  return (
    <span className="relative inline-flex align-middle">
      <button
        ...
        className="flex h-[15px] w-[15px] items-center justify-center rounded-full border border-ink/25 font-mono text-[9px] font-bold leading-none text-text-muted transition-colors hover:border-ink/70 hover:text-text-primary"
      >
        i
      </button>
      {open && (
...
function Spinner() {
  return (
    <div className="flex h-full min-h-[140px] items-center justify-center">
      <div
        className="h-6 w-6 animate-spin rounded-full border-2 border-ink/10"
...
export default function Performance({ data }) {
  ...
  const openDay = (date) => {
    ...
    fetch(
      `/api/v1/analytics/edge-lab/drill?dimension=created_day&key=${encodeURIComponent(date)}&days=90&limit=200`
    )
  ...
  const wrDonut = [
    ...
    { name: "loss", value: Math.max(100 - winRate, 0) },
  ];

  // avg realized P/L per bucket (from the all-pairs journey aggregate)
  const pnlMap = {};
  (timing?.hit_rate_per_tp || []).forEach((h) => {
    pnlMap[h.tp] = h.avg_exit_gain_pct;
  });
  const outcome = stats
    ? [
  ...
  const { trendData, wrAvg, corr, upWR, downWR, btcDelta, btcMin, btcMax } = useMemo(() => {
    ...
    const first = withBtc[0]?.btc_close,
      last = withBtc[withBtc.length - 1]?.btc_close;
    return {
      trendData: td,
            ...
            <Card className="!overflow-visible">
                ...
                right={
                  <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
                    <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">
                      <Seg
                        items={["30D", "90D", "1Y", "ALL"]}
                        value={eventId || customOn ? "" : rangeId}
                        onChange={(v) => {
                          setRangeId(v);
                          setEventId(null);
                          setCustomStart("");
                      ...
                      />
                      <Seg
                        items={["Win rate", "vs BTC"]}
                        value={showBtc ? "vs BTC" : "Win rate"}
                        onChange={(v) => setShowBtc(v === "vs BTC")}
                      />
                    </div>
                    <PeriodFilterPopover
              ...
              {customOn && (
                <p className="mb-3 text-[11px] leading-snug text-text-primary/45">
                  <span className="font-semibold text-accent">Custom range</span> ·{" "}
                  {trendData.length} day{trendData.length === 1 ? "" : "s"} of signals in this
                  window.
                </p>
              )}
              ...
              {activeEvent && (
                <p className="mb-3 text-[11px] leading-snug text-text-primary/45">
                  <span className="font-semibold" style={{ color: EV_COLOR[activeEvent.cat] }}>
                    {new Date(activeEvent.start).toLocaleDateString("en", {
                      month: "short",
                      day: "numeric",
                      year: "2-digit",
                    })}{" "}
                    ...
                    {activeEvent.ongoing
                      ? "now"
                      : new Date(activeEvent.end).toLocaleDateString("en", {
                          month: "short",
                          day: "numeric",
                          year: "2-digit",
                        })}
              ...
              <div className="mb-3 flex flex-wrap gap-x-6 gap-y-1.5 font-mono text-[11px]">
                ...
                {showBtc && btcDelta != null && (
                  <span className="text-text-muted">
                    BTC <b style={{ color: btcDelta >= 0 ? C.win : C.loss }}>{signed(btcDelta)}</b>{" "}
                    over range
                  </span>
                )}
                {showBtc && corr != null && (
                        ...
                        content={({ active, payload }) => {
                          ...
                          return (
                            <div className="rounded-lg border border-ink/15 bg-surface-secondary/95 p-2.5 backdrop-blur-md">
                              ...
                              <p className="mt-1 text-[12px]" style={{ color: C.gold }}>
                                {pct(d.wr)} win rate{" "}
                                <span className="text-text-muted">· {d.closed} closed</span>
                              </p>
                              {showBtc && d.c != null && (
                                <>
                                  <p className="mt-0.5 text-[11px]">
                                    <span style={{ color: C.btc }}>
                                      BTC ${Math.round(d.c).toLocaleString()}
                                    </span>{" "}
                                    <span style={{ color: d.chg >= 0 ? C.win : C.loss }}>
                                      {signed(d.chg)}
                                    </span>
                                  </p>
                                  <p className="font-mono text-[9px] text-text-muted">
                                    O {Math.round(d.o).toLocaleString()} · H{" "}
                                    {Math.round(d.h).toLocaleString()} · L{" "}
                                    {Math.round(d.l).toLocaleString()}
                                  </p>
                                </>
                              )}
                            </div>
