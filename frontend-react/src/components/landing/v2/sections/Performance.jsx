...
function Card({ className = "", children }) {
 return (
 <div className={`group relative overflow-hidden rounded-2xl border border-ink/[0.07] bg-surface-raised p-5 transition-all duration-300 hover:border-ink/12 hover:shadow-[0_14px_34px_rgb(var(--scrim) / 0.35)] ${className}`}>
...
export default function Performance({ data }) {
 ...
 An edge you can{" "}
 <span className="bg-gradient-to-r from-accent via-ink to-accent-dark bg-clip-text text-transparent">audit.</span>
 </h2>
 ...
 >
 <TabIcon id={tb.id} className={`h-[15px] w-[15px] flex-shrink-0 ${on ? "text-accent" : ""}`} />
 ...
 {coins.map((c) => (
 <div key={c.pair} className="flex items-center gap-3 rounded-xl border border-ink/[0.06] bg-ink/[0.015] px-3.5 py-2.5 transition-colors hover:border-ink/12">
 ...
 style={{ background: "linear-gradient(135deg, rgb(var(--accent)) 0%, rgb(var(--accent)) 50%, rgb(var(--accent)) 100%)", color: "rgb(var(--surface))" }}
 >
 See Full Analytics
