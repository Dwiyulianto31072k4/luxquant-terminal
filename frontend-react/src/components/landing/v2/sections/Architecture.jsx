...
function InputCard({ item }) {
 ...
 <span data-wire="in" aria-hidden="true" className="absolute -right-[5px] top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-accent/85" />
 </div>
 );
}
...
function StageNode({ icon, title, subtitle, dataWire, accent = false }) {
 return (
 <div data-wire={dataWire} className={`relative flex min-h-[220px] w-full flex-col items-center justify-center px-5 text-center ${CARD} ${accent ? "border-ink/12" : ""}`}>
...
function Step({ n, accent = false, line = true, children }) {
 ...
 <span className={`relative z-10 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border bg-surface-raised font-mono text-[13px] font-medium ${accent ? "border-ink/15 text-accent" : "border-ink/15 text-text-primary/55"}`}>{n}</span>
...
export default function Architecture() {
 ...
 <div aria-hidden="true" className="pointer-events-none absolute left-1/2 top-1/2 -z-10 h-[460px] w-full max-w-4xl -translate-x-1/2 -translate-y-1/2 rounded-[100%] bg-accent/[0.035] blur-[120px]" />
 ...
 <p className="mb-1 ml-1 font-mono text-[10px] uppercase tracking-[0.25em] text-text-muted">Live Market Data</p>
 ...
 <p className="mt-3.5 border-t border-ink/[0.06] pt-3 text-[11px] font-medium text-accent/85">+ Markets, Portfolio, Journal &amp; more</p>
 </div>
 </Step>
 </ol>
