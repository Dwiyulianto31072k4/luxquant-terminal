...
export default function CoinSpotlight() {
  ...
  useEffect(() => {
    ...
    fetch("/api/v1/analytics/edge-lab?days=90&sector=all")
      ...
      .then((j) => {
        ...
        const majors = TOP_MCAP.map(
          (s) => lbMap.get(s) || { pair: `${s}USDT`, win_rate: null, median_peak: null }
        );
  ...
  const avgPnl = (() => {
    let s = 0,
      n = 0;
    hit.forEach((h) => {
      if (h.avg_exit_gain_pct != null && h.hit_count) {
        s += h.avg_exit_gain_pct * h.hit_count;
        n += h.hit_count;
      }
    });
    return n ? s / n : null;
              ...
              {coins.slice(0, 40).map((c) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[12px] font-semibold ${on ? "text-text-primary" : "text-text-muted"}`}
                    >
                      {sym(c.pair)}
                    </span>
                  </button>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="mb-5 flex flex-wrap items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="font-mono text-[10px] text-text-muted">
                    {detail.sample_size} signals on record
                  </p>
                </div>
                {winRate != null && (
                  <span
                    className="rounded-lg px-2.5 py-1 text-[12px] font-bold"
                    style={{ color: C.win, background: "rgba(74,222,128,0.12)" }}
                  >
                    {pct(winRate)} WR
                  </span>
                )}
              ...
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
                ...
                <StatTile
                  label="Avg PnL / trade"
                  value={signedPct(avgPnl)}
                  accent={avgPnl != null && avgPnl < 0 ? C.loss : C.win}
                />
                <StatTile
                  label="Avg Peak"
              ...
              <div className="mt-3 grid gap-3 lg:grid-cols-2">
                ...
                <div className="group rounded-xl border border-ink/[0.06] bg-surface-raised p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-ink/10 hover:shadow-[0_12px_30px_rgb(var(--scrim) / 0.35)]">
                    ...
                    <div className="flex-1 space-y-1.5">
                      ...
                      {outcome.map((o) => (
                        <div key={o.label} className="flex items-center gap-2">
                          ...
                          />
                          <span
                            className="w-7 font-mono text-[11px] font-semibold"
                          ...
                          <span className="w-8 text-right font-mono text-[9px] text-text-muted">
                            {totalClosed ? ((o.count / totalClosed) * 100).toFixed(0) : 0}%
                          </span>
                        </div>
                      ))}
                    </div>
                  ...
                  <p className="mt-3 border-t border-ink/[0.05] pt-2.5 font-mono text-[8.5px] leading-relaxed text-text-muted">
                    ...
                    <span className="text-text-secondary">TP4+ = avg peak</span> (TP4 is the final
                    target — winners usually run beyond it) · SL = avg loss.
                  </p>
                </div>
                ...
                <div className="group rounded-xl border border-ink/[0.06] bg-surface-raised p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-ink/10 hover:shadow-[0_12px_30px_rgb(var(--scrim) / 0.35)]">
                  ...
                  <div className="space-y-2.5">
                    {ttp.map((t) => (
                      <div key={t.tp}>
                        ...
                        <div
                          className="h-2 overflow-hidden rounded-full bg-scrim/40"
                        ...
                        >
                          <div
                            ...
                            style={{
                              ...
                              background:
                                "linear-gradient(180deg, #f6e0a0 0%, #e7c373 40%, #a8842f 100%)",
                              boxShadow:
                                "inset 0 1px 0 rgb(var(--ink) / 0.5), 0 0 6px rgb(var(--accent) / 0.35)",
                            }}
                          />
                        </div>
                      </div>
                  ...
                  <p className="mt-3 border-t border-ink/[0.06] pt-2.5 font-mono text-[10px] text-text-muted">
                    smooth entry{" "}
                    <span className="text-text-primary">{pct(entry.smooth_entry_rate_pct)}</span> ·
                    time in profit{" "}
                    <span className="text-text-primary">{pct(risk.avg_time_in_profit_pct)}</span>
                  </p>
                </div>
              </div>
            </>
