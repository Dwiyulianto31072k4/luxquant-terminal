...
export default function PhoneMockup({
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="rounded-[2.3rem] bg-gradient-to-b from-text-primary via-text-secondary to-text-primary p-[1px] shadow-[0_24px_56px_rgb(var(--scrim) / 0.85),0_0_34px_rgb(var(--accent) / 0.12)] sm:rounded-[2.6rem] lg:rounded-[2.9rem] lg:p-[1.5px]">
        ...
        <div className="overflow-hidden rounded-[2.2rem] bg-black p-[1.5px] sm:rounded-[2.5rem] lg:rounded-[2.8rem] lg:p-[2px]">
          ...
          <div
            className={`relative overflow-hidden rounded-[2.1rem] bg-bg-primary sm:rounded-[2.4rem] lg:rounded-[2.7rem] ${screenClassName}`}
          >
            <img
              src={src}
              alt={alt}
 ...
 height; lower ones may fall into the card clip — intended) */}
      ...
      <span
        aria-hidden="true"
        className="absolute left-[-1.5px] top-[21%] h-[5%] w-[2px] rounded-l bg-gradient-to-b from-[#d2d4d7] to-text-secondary"
      />
      <span
        aria-hidden="true"
        className="absolute left-[-1.5px] top-[29%] h-[7%] w-[2px] rounded-l bg-gradient-to-b from-[#d2d4d7] to-text-secondary"
      />
      <span
        aria-hidden="true"
        className="absolute left-[-1.5px] top-[38%] h-[7%] w-[2px] rounded-l bg-gradient-to-b from-[#d2d4d7] to-text-secondary"
      />
      {/* right: side/power button (longer) */}
      <span
        aria-hidden="true"
        className="absolute right-[-1.5px] top-[27%] h-[13%] w-[2px] rounded-r bg-gradient-to-b from-[#d2d4d7] to-text-secondary"
      />
