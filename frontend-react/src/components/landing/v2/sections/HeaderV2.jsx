...
export default function HeaderV2({ onNav, activeId = "hero" }) {
 ...
 <span className={["block h-0.5 rounded-full bg-current transition-all duration-300", mobileOpen ? "translate-y-[7px] rotate-45 text-accent" : ""].join(" ")} />
 ...
 active ? "bg-ink/[0.05] text-accent" : "text-text-primary/85 hover:bg-ink/[0.03] hover:text-text-primary",
 ].join(" ")}
 >
 {item.label}
 </button>
 ...
 <span>{grp.group}</span>
 <svg
 className={["h-4 w-4 shrink-0 transition-transform duration-300", open ? "rotate-180 text-accent" : "text-text-primary/40"].join(" ")}
