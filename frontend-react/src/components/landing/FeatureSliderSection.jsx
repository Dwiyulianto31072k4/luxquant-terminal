...
const FEATURES = [
  { 
    ...
    // ICON: Jaringan simpul algoritma/kuantum (Nodes Network)
  ...
  { 
    ...
    // ICON: Target Kamera/Scanner Fokus (Visual Capture)
  ...
  { 
    ...
    // ICON: Kubus Isometrik 3D (Blockchain / Deep Data Structure)
  ...
  { 
    ...
    // ICON: Microchip / AI Core Processor
  ...
  { 
    ...
    // ICON: Radar Crosshair Konsentris (Surveillance Tracking)
  ...
  { 
    ...
    // ICON: Dinding Likuiditas (Bid & Ask Depth Chart)
  ...
  { 
    ...
    // ICON: Masonry Grid Layout (Terminal Command Center)
...
const FeatureSliderSection = () => {
  ...
  useEffect(() => {
    const iv = setInterval(() => setActiveIdx((prev) => (prev + 1) % FEATURES.length), 7000);
  ...
  return (
    <section className="relative z-10 w-full pt-20 lg:pt-28 pb-16 lg:pb-24 overflow-hidden">
      ...
      {/* 1. HORIZONTAL NAVIGATION TABS (Swipeable) */}
      <div className="w-full relative mb-8">
        <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-bg-primary to-transparent z-10 pointer-events-none lg:hidden" />
        <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-bg-primary to-transparent z-10 pointer-events-none lg:hidden" />
        
        <div className="flex overflow-x-auto gap-3 px-4 lg:px-8 max-w-7xl mx-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      <div className="max-w-3xl mx-auto px-4 text-center mb-12 h-[80px] sm:h-[60px] flex items-center justify-center">
      ...
      <div className="relative max-w-5xl mx-auto px-4 lg:px-8 flex justify-center items-center mt-4">
        ...
        {/* Mac Mockup */}
        <div className="relative w-full max-w-[850px] aspect-[16/10] bg-[#0a0805] rounded-2xl lg:rounded-3xl border border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.8)] overflow-hidden z-10 transition-all duration-500">
          ...
          <div className="h-6 lg:h-8 bg-[#1a1a1a] flex items-center px-4 gap-2 border-b border-white/5 w-full absolute top-0 z-20">
            ...
            {/* DOMAIN DIUBAH MENJADI luxquant.tw */}
            ...
            {FEATURES.map((feat, idx) => (
              <img 
                ...
                onError={(e) => { 
                  ...
                  e.target.parentElement.innerHTML += `<div class="absolute inset-0 flex items-center justify-center text-white/20 text-xs font-mono opacity-${activeIdx === idx ? '100' : '0'}">Screenshot Desktop ${feat.title}<br/>(${feat.macImg})</div>`;
                }}
              />
            ))}
          </div>
        ...
        <div className="absolute -bottom-6 -right-2 sm:-right-8 lg:-right-12 w-[110px] sm:w-[150px] lg:w-[190px] xl:w-[220px] aspect-[9/19.5] bg-[#050302] border-[4px] lg:border-[6px] border-[#2a2a2a] rounded-2xl lg:rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.9),0_0_20px_rgba(212,168,83,0.2)] overflow-hidden z-20 transition-all duration-500">
          <div className="absolute top-0 inset-x-0 h-3 lg:h-5 bg-[#2a2a2a] rounded-b-lg lg:rounded-b-xl w-[40%] mx-auto z-30" />
          ...
          <div className="relative w-full h-full bg-[#0a0805]">
             {FEATURES.map((feat, idx) => (
               <img 
                ...
                onError={(e) => { 
                  e.target.style.display = 'none';
                  e.target.parentElement.innerHTML += `<div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white/20 text-[6px] lg:text-[8px] font-mono px-2 opacity-${activeIdx === idx ? '100' : '0'}">Screenshot HP<br/>(${feat.phoneImg})</div>`;
                }}
              />
             ))}
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 mt-20 lg:mt-24 relative z-20">
