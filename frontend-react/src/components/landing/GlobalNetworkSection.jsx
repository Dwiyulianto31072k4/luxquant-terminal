...
function makeEarthCanvas(w, h, polys) {
  ...
  x.fillStyle = "rgb(16,12,7)";
  x.fillRect(0, 0, w, h);
  x.fillStyle = "rgb(155,125,58)";
  x.strokeStyle = "rgb(175,145,70)";
  x.lineWidth = 0.6;
  x.lineJoin = "round";
  polys.forEach((p) => {
    x.beginPath();
...
function decodeTopo(t) {
  ...
  const da = (i) => {
    ...
    ar.forEach(([dx, dy]) => {
      x += dx;
      y += dy;
      c.push([f ? x * f.scale[0] + f.translate[0] : x, f ? y * f.scale[1] + f.translate[1] : y]);
    });
  ...
  const proc = (g) => {
    if (g.type === "Polygon") dr(g.arcs).forEach((c) => ps.push(c));
    else if (g.type === "MultiPolygon") g.arcs.forEach((p) => dr(p).forEach((c) => ps.push(c)));
    else if (g.type === "GeometryCollection") g.geometries.forEach(proc);
  };
  proc(t.objects.land);
  return ps;
}
...
const GlobeRenderer = ({ containerWidth, earthCanvas }) => {
  ...
  useEffect(() => {
    const c = ref.current;
    if (!c || !window.THREE || !earthCanvas) return;
    ...
    const sc = new T.Scene(),
      cam = new T.PerspectiveCamera(28, W / H, 0.1, 1000);
    cam.position.set(0, 2.2, 5.8);
    cam.lookAt(0, 0.1, 0);
    ...
    // Membuat bola bumi
    const tx = new T.CanvasTexture(earthCanvas);
    tx.anisotropy = rn.capabilities.getMaxAnisotropy();
    ...
    for (let lat = -60; lat <= 80; lat += 30) {
      ...
      for (let lo = 0; lo <= 360; lo += 3)
        p.push(new T.Vector3(...ll2xyz(lat, lo - 180, R * 1.002)));
      g.add(new T.Line(new T.BufferGeometry().setFromPoints(p), gM));
    }
    for (let lo = -180; lo < 180; lo += 30) {
      const p = [];
      for (let la = -90; la <= 90; la += 3) p.push(new T.Vector3(...ll2xyz(la, lo, R * 1.002)));
      g.add(new T.Line(new T.BufferGeometry().setFromPoints(p), gM));
    }
    ...
    for (let i = 0; i < 300; i++) {
      ...
      starPts.push(
        rStar * Math.sin(phi) * Math.cos(theta),
        rStar * Math.cos(phi),
        rStar * Math.sin(phi) * Math.sin(theta)
      );
    }
    ...
    const rM1 = new T.MeshBasicMaterial({
      color: 0xf0d890,
      transparent: true,
      opacity: 0.7,
      side: T.DoubleSide,
    });
    const rM2 = new T.MeshBasicMaterial({
      color: 0xf0d890,
      transparent: true,
      opacity: 0.4,
      side: T.DoubleSide,
    });
    const r1 = new T.Mesh(rG, rM1),
      r2 = new T.Mesh(rG.clone(), rM2);
    [r1, r2].forEach((r) => {
      r.position.set(...tw);
      r.lookAt(0, 0, 0);
      g.add(r);
    ...
    const cd = new T.Mesh(
      new T.CircleGeometry(0.025, 16),
      new T.MeshBasicMaterial({ color: 0xf0d890, side: T.DoubleSide })
    );
    cd.position.set(...tw);
    cd.lookAt(0, 0, 0);
    g.add(cd);
    ...
    const anim = () => {
      ...
      const p1 = 1 + ((t * 2) % 1) * 1.8;
      r1.scale.set(p1, p1, 1);
      rM1.opacity = 0.7 * (1 - ((t * 2) % 1));
      const p2 = 1 + ((t * 2 + 0.5) % 1) * 1.8;
      r2.scale.set(p2, p2, 1);
      rM2.opacity = 0.5 * (1 - ((t * 2 + 0.5) % 1));
  ...
  return (
    <canvas
      ref={ref}
      style={{ width: "100%", height: Math.round(containerWidth * 0.65), display: "block" }}
    />
  );
};
...
export default function GlobalNetworkSection() {
  ...
  useEffect(() => {
    const m = () => {
      if (ref.current) setCw(Math.min(ref.current.offsetWidth, 1200));
    };
    m();
  ...
  useEffect(() => {
    ...
    fetch("https://cdn.jsdelivr.net/npm/world-atlas@2/land-110m.json")
      ...
      .then((t) => {
        try {
          setEc(makeEarthCanvas(W, H, decodeTopo(t)));
        } catch {
          setEc(makeEarthCanvas(W, H, FALLBACK));
        }
      })
      .catch(() => setEc(makeEarthCanvas(W, H, FALLBACK)));
  }, []);
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          >
            ...
            <span
              style={{
                background:
                  "linear-gradient(to right,rgb(var(--accent)),rgb(var(--accent)),rgb(var(--accent)))",
          ...
          >
            Built and operated from Taiwan, our quantitative engine delivers real-time algorithmic
            signals to active users worldwide.
          </p>
        </div>
            ...
            >
              ...
              <div
                style={{
                  ...
                  height: 60,
                  background:
                    "linear-gradient(to bottom,rgb(var(--accent) / 0),rgb(var(--accent)))",
                }}
              />
              {/* 2. Dot */}
              <div
              ...
              <span
                style={{
                  ...
                  fontWeight: 700,
                  color: i === 0 ? "rgb(var(--accent))" : "rgb(var(--accent))",
                  lineHeight: 1.1,
                  textShadow: i === 0 ? "0 0 20px rgb(var(--accent) / 0.3)" : "none",
                }}
              >
                {s.value}
              </span>
        ...
        <div
          style={{
            ...
            overflow: "hidden",
            maskImage:
              "linear-gradient(to bottom, transparent 0%, black 8%, black 72%, transparent 95%)",
            WebkitMaskImage:
              "linear-gradient(to bottom, transparent 0%, black 8%, black 72%, transparent 95%)",
          }}
        >
            ...
            >
              <div
                style={{
                  width: 32,
                  height: 32,
                  border: "2px solid rgb(var(--line) / 0.2)",
