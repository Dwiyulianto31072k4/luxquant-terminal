...
const IndicatorGuideModal = ({ isOpen, onClose }) => {
 ...
 <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-text-muted hover:text-text-primary bg-surface-raised hover:bg-red-500/20 border border-ink/10 hover:border-red-500/50 rounded-lg transition-all flex-shrink-0">
 {I.close()}
 </button>
 </div>
 ...
 <Tag tone="red">40–60%</Tag> of the time. The edge comes from <span className="text-accent font-semibold">confluence</span>:
 ...
 {indicators.map((ind) => (
 <div key={ind.name} className="flex flex-col bg-surface-raised border border-ink/[0.06] rounded-xl p-4 hover:border-ink/10 transition-colors">
 ...
 <span className="text-[9px] font-mono text-text-muted bg-accent/12 px-1.5 py-0.5 rounded-[3px] border border-ink/10 whitespace-nowrap flex-shrink-0">{ind.setting}</span>
 </div>
 ...
 <span className="text-text-muted flex-shrink-0 mt-[3px]">{I.arrow("w-2.5 h-2.5")}</span>
 <span>{r}</span>
 </li>
 ))}
 </ul>
 ...
 {steps.map((s, i) => (
 <div key={i} className="flex gap-3 bg-surface/60 border border-ink/08 rounded-lg p-3">
 <span className="flex-shrink-0 w-6 h-6 rounded-full bg-accent border border-ink/12 text-accent text-[11px] font-bold flex items-center justify-center">{i + 1}</span>
 <div>
 ...
 <button onClick={onClose} className="px-4 py-1.5 rounded-lg bg-accent text-accent-fg border border-ink/12 hover:bg-accent/25 text-xs font-bold transition-all">Got it</button>
 </div>
 </div>
