...
const inputCls =
  "w-full bg-ink/[0.03] border border-ink/[0.08] rounded-md px-3 py-2 text-[13px] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-ink/15";
const btnCls =
...
export default function StatusAdminPage() {
  ...
  const postUpdate = async (id, newStatus, body) => {
    setBusy(true);
    try {
      await api.post(`/status/admin/incidents/${id}/updates`, {
        status: newStatus,
        message: body || "",
      });
      await load();
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={load}
          className={`${btnCls} bg-ink/[0.05] text-text-secondary hover:text-text-primary`}
        >
          Refresh
        </button>
      </div>
      ...
      {err && (
        <div className="mb-4 rounded-md border border-loss/30 bg-loss/10 px-3 py-2 text-[13px] text-loss">
          {err}
        </div>
      )}
      ...
      <section className="rounded-lg border border-ink/[0.08] bg-ink/[0.02] p-4 sm:p-5 mb-8">
        ...
        <label className="block text-[11px] text-text-muted mb-1 mt-3">
        ...
        <div className="mt-4">
          <button
            onClick={create}
            disabled={busy}
            className={`${btnCls} bg-accent/12 text-accent border border-ink/12 hover:bg-accent/12`}
          >
            Publish incident
          </button>
        </div>
...
function IncidentAdminCard({ inc, busy, onUpdate, onDelete }) {
  ...
  return (
    <div
      ...
      style={{ borderColor: closed ? "rgb(var(--ink) / 0.08)" : `${accent}40` }}
    >
      <div
        className="px-4 py-3 flex items-center justify-between gap-3"
        style={{ background: closed ? "transparent" : `${accent}0d` }}
      >
        ...
        <span
          ...
          style={{
            color: closed ? "#22c55e" : accent,
            background: `${closed ? "#22c55e" : accent}1a`,
          }}
        >
      ...
      <div className="px-4 py-3 border-t border-ink/[0.06]">
          ...
          <>
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <button
                ...
                disabled={busy}
                className={`${btnCls} bg-accent/12 text-accent border border-ink/12 hover:bg-accent/12 whitespace-nowrap`}
              >
                Post update
              </button>
            </div>
            <p className="text-[11px] text-text-muted mt-2">
              Tip: post <b>Resolved</b> (or <b>Completed</b> for maintenance) to close it and move
              it to Past Incidents.
            </p>
          </>
        ) : (
        ...
        <div className="mt-3">
          <button
            ...
            disabled={busy}
            className="font-mono text-[10px] uppercase tracking-wider text-loss/70 hover:text-loss"
          >
            Delete
          </button>
        </div>
