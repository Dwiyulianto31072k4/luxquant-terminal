...
export default function StatusAdminPage() {
 ...
 {err && (
 <div className="mb-4 rounded-md border border-loss/30 bg-loss/10 px-3 py-2 text-[13px] text-loss">{err}</div>
 )}
 ...
 <div className="mt-4">
 <button onClick={create} disabled={busy} className={`${btnCls} bg-accent/12 text-accent border border-ink/12 hover:bg-accent/12`}>
 Publish incident
 </button>
 </div>
 </section>
...
function IncidentAdminCard({ inc, busy, onUpdate, onDelete }) {
 ...
 <div className="mt-3">
 <button onClick={() => onDelete(inc.id)} disabled={busy} className="font-mono text-[10px] uppercase tracking-wider text-loss/70 hover:text-loss">
 Delete
 </button>
 </div>
 </div>
