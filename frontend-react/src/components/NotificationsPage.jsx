...
const toneStyle = (tone) => {
  if (tone === "gold") return "bg-accent/12 text-accent border-ink/12";
  if (tone === "danger") return "bg-loss/10 text-loss border-loss/25";
...
const NotificationsPage = () => {
  ...
  const handleNotificationClick = (notif) => {
    ...
    if (
      notif.type === "btcdom_call" ||
      notif.type === "watchlist_update" ||
      notif.type === "coin_called"
    )
      navigate("/signals");
  ...
  const groupByDate = (items) => {
    ...
    items.forEach((item) => {
      ...
      let label;
      if (itemDate.getTime() === today.getTime()) label = t("notifications.today") || "Today";
  ...
  const groupByCategory = (items, t) => {
    const CAT_OF = (type) => {
      ...
      if (["daily_results", "btcdom_call", "watchlist_update", "coin_called"].includes(type))
        return "signals";
      if (["sub_expiry", "admin_broadcast"].includes(type)) return "account";
      return "other";
    };
    const LABELS = {
      autotrade: t("notifications.type_autotrade") || "Agent",
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center gap-1 border-b border-ink/[0.06]">
        {topTabs.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              {active && <span className="absolute bottom-0 inset-x-0 h-px bg-accent" />}
            </button>
          );
        })}
      </div>
      ...
      {/* SETTINGS VIEW */}
      {/* ════════════════════════════════════════ */}
      {view === "settings" && <NotificationSettings t={t} navigate={navigate} />}
              ...
              {["all", "unread"].map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`tabular-nums text-[10px] ${active ? "text-text-muted" : "text-text-muted/50"}`}
                    >
                      {count}
                    </span>
                  </button>
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 pt-0.5">
        <span
          className={`inline-flex items-center text-[9px] font-mono uppercase tracking-[0.1em] px-1.5 py-1 rounded border whitespace-nowrap ${toneStyle(token.tone)}`}
        >
          {token.label}
        </span>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p
            className={`text-sm leading-snug ${isUnread ? "font-semibold text-text-primary" : "font-medium text-text-secondary"}`}
          >
            {notif.title}
          </p>
        ...
        {notif.body && (
          <p
            className={`text-xs leading-relaxed line-clamp-2 mt-0.5 ${isUnread ? "text-text-secondary" : "text-text-muted"}`}
          >
            {notif.body}
          </p>
        )}
        {notif.data && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            ...
            {notif.data.percentage !== undefined && notif.data.percentage !== null && (
              <span
                className={`text-[9px] font-mono uppercase tracking-[0.1em] px-1.5 py-0.5 rounded border tabular-nums ${
            ...
            {notif.data.realized_pnl !== undefined && notif.data.realized_pnl !== null && (
              ...
              >
                {notif.data.realized_pnl >= 0 ? "+" : ""}
                {Number(notif.data.realized_pnl).toFixed(2)}
              </span>
            )}
            {notif.data.tp_level && (
