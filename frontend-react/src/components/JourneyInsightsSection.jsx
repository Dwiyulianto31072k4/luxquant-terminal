...
const StatPill = ({ label, value, valueColor = "text-text-primary", sublabel }) => (
  <div className="flex-1 min-w-[100px] bg-ink/[0.02] border border-ink/5 rounded-lg px-2.5 py-2">
    ...
    <p className={`text-sm sm:text-base font-bold font-mono ${valueColor}`}>{value}</p>
    {sublabel && <p className="text-[9px] text-text-primary/40 mt-0.5">{sublabel}</p>}
  </div>
);
...
const TableHeader = ({ children, className = "" }) => (
  <th
    className={`px-2 py-1.5 text-[9px] text-text-primary/40 uppercase tracking-wider font-medium text-left ${className}`}
  >
    {children}
  </th>
);
...
const JourneyInsightsSection = ({ pair }) => {
  ...
  useEffect(() => {
    ...
    const fetchInsights = async () => {
      ...
      setErr(null);
      try {
        const res = await fetch(`${API_BASE}/api/v1/signals/journey-insights/${pair}`);
      ...
      return (
        <div className="bg-surface-secondary/80 rounded-xl p-3 sm:p-4 border border-ink/5">
          ...
          <p className="text-text-primary/40 text-xs">
            Need at least {data.min_required} signals to surface patterns. Currently{" "}
            {data.sample_size} available.
          </p>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ──────────────────────────────────────── */}
      <div className="bg-surface-secondary/80 rounded-xl p-3 sm:p-4 border border-ink/5">
        <SectionHeader icon="🚪" title="Entry Behavior" sample={sample_size} />
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <StatPill
            ...
            valueColor={
              entry_behavior?.avg_drawdown_before_tp1_pct < -0.5 ? "text-loss" : "text-profit"
            }
            ...
            <tbody>
              {time_to_each_tp?.map((row) => (
                <tr key={row.tp} className="border-b border-ink/[0.03] last:border-0">
                  ...
                  <Cell>
                    <span className="text-[11px] font-mono text-profit/90">
                      {row.fastest_human || "—"}
                    </span>
                  </Cell>
                  <Cell>
            ...
            <tbody>
              {drawdown_before_each_tp?.map((row) => (
                <tr key={row.phase} className="border-b border-ink/[0.03] last:border-0">
                  ...
                  <Cell>
                    <span
                      className={`text-[11px] font-mono ${row.avg_dd_pct < 0 ? "text-loss" : "text-text-primary/40"}`}
                    >
              ...
              {hit_rate_per_tp?.map((row) => {
                ...
                return (
                  <tr key={row.tp} className="border-b border-ink/[0.03] last:border-0">
                    <Cell>
                      <span
                        className={`text-[11px] font-bold ${isSL ? "text-loss" : "text-text-primary"}`}
                      >
                        {row.tp}
                      </span>
                    </Cell>
                    ...
                    <Cell>
                      <span
                        className={`text-[11px] font-mono ${isSL ? "text-loss" : "text-profit"}`}
                      >
      ...
      <div className="bg-surface-secondary/80 rounded-xl p-3 sm:p-4 border border-ink/5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          <StatPill
            ...
            value={
              peak_potential?.best_peak_pct !== null ? formatPct(peak_potential.best_peak_pct) : "—"
            }
            valueColor="text-profit"
