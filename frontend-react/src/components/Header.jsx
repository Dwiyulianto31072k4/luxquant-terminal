...
const Header = ({ activeTab, setActiveTab }) => {
 ...
 >
 <div className="relative w-6 h-6 rounded-sm flex items-center justify-center bg-accent/10 border border-ink/12">
 <span className="font-mono text-[9px] font-bold tracking-wider text-accent">
 LQ
 </span>
 </div>
 <h1 className="text-[15px] font-normal text-text-primary tracking-tight group-hover:text-text-primary transition-colors">
 LuxQuant
 </h1>
 </a>
 ...
 {active && (
 <span className="absolute inset-x-2 -bottom-[7px] h-[2px] rounded-full bg-accent/80" />
 )}
 </button>
 );
 })}
...
function IconHome({ active }) {
 return (
 <svg
 className={`w-4 h-4 transition-colors ${active ? 'text-accent' : 'text-text-muted group-hover:text-text-primary'}`}
