...
function TermPage({ slug }) {
  ...
  return (
    <div className="w-full max-w-3xl px-1 py-4">
      ...
      />
      <Crumbs
        trail={[{ label: "Home", to: "/" }, { label: "Learn", to: "/learn" }, { label: term.term }]}
      />
  ...
  return (
    <div className="w-full px-1 py-4">
      ...
      <div className="mt-7 grid grid-cols-1 sm:grid-cols-2 gap-3">
        {GLOSSARY.map((t) => (
          ...
          >
            ...
            <p className="mt-1.5 text-[13px] text-text-primary/55 leading-relaxed line-clamp-3">
              {t.short}
            </p>
          </Link>
        ))}
      ...
      <div className="mt-8 font-mono text-[12px] text-text-muted">
        Prefer long-form? Read the{" "}
        <Link to="/blog" className="text-accent hover:text-accent">
          LuxQuant blog →
        </Link>
      </div>
    </div>
