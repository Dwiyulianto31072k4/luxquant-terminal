...
const SectionHeader = ({ label }) => (
  <div className="flex items-center gap-3">
    <span className="font-mono uppercase tracking-[0.25em] text-text-muted text-[10px]">
      {label}
    </span>
  </div>
);
...
const Toggle = ({ on, locked, onClick }) => (
  ...
  >
    ...
    {locked && (
      <svg
        className="absolute -right-3.5 w-2.5 h-2.5 text-text-muted/60"
        fill="currentColor"
        viewBox="0 0 20 20"
      >
        <path
...
const NotificationSettings = ({ t, navigate }) => {
  ...
  const handleToggle = async (item, channel) => {
    ...
    // Optimistic update
    const prev = items;
    setItems((arr) => arr.map((x) => (x.type === item.type ? { ...x, ...next } : x)));
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {!telegramLinked ? (
        <div className="relative overflow-hidden rounded-md border border-ink/10 bg-surface-raised p-4">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-2 px-3 py-2 rounded-md border border-ink/12 text-[10px] font-mono uppercase tracking-[0.2em] text-accent hover:bg-accent/10 hover:border-ink/18 transition-all"
            >
              Link Telegram
            </button>
          </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="h-12 rounded-md bg-ink/[0.02] border border-ink/[0.05] animate-pulse"
            />
          ))}
        </div>
      ) : (
