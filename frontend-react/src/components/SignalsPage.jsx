...
const SignalsPage = () => {
  ...
  useEffect(() => {
    setPage(1);
  }, [searchPair, statusFilter, riskFilter, dateFilter, sortBy, sortOrder]);
  ...
  // Generate last 7 day buttons with signal counts
  ...
  // Dynamic order label based on sort field
  const getOrderLabel = () => {
    ...
    if (sortOrder === "desc") {
      ...
      if (isRisk) return "↓ High First";
      if (isStatus) return "↓ Latest Stage";
      return "↓ Highest";
    } else {
      if (isTime) return "↑ Oldest";
      if (isAlpha) return "↑ A-Z";
      if (isRisk) return "↑ Low First";
      if (isStatus) return "↑ Early Stage";
      return "↑ Lowest";
    }
  };
  ...
  const { signals, totalPages, totalSignals } = useMemo(() => {
    ...
    if (dateFilter !== "all") {
      filtered = filtered.filter((s) => s.created_at && s.created_at.slice(0, 10) === dateFilter);
    }
  ...
  }, [allSignals, searchPair, statusFilter, riskFilter, dateFilter, sortBy, sortOrder, page, pageSize, priceVersion]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4">
          <span className="text-text-muted text-sm">
            ...
            {updatedCount > 0 && (
              <span className="ml-2 text-gold-primary">· <span className="font-semibold">{updatedCount}</span> {t("signals.updated")}</span>
            )}
          </span>
          <div className="flex items-center gap-2 text-text-muted text-sm">
            <span className={`w-2 h-2 rounded-full ${loading ? "bg-yellow-400 animate-pulse" : "bg-green-400"}`} />
            <span>{loading ? "Loading..." : lastUpdated ? `${lastUpdated.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}` : "Ready"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card rounded-xl p-4 border border-gold-primary/10">
          ...
          <p className="text-text-muted text-xs mt-1"><span className="text-green-400">{todayStats.open} {t("signals.open")}</span> · {todayStats.wins}W / {todayStats.losses}L</p>
        </div>
        <div className="glass-card rounded-xl p-4 border border-gold-primary/10">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="BTC, ETH, SOL..." value={searchPair} onChange={(e) => setSearchPair(e.target.value)}
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <button onClick={() => setSortOrder(sortOrder === "desc" ? "asc" : "desc")}
                className="px-4 py-2.5 bg-bg-primary border border-gold-primary/20 rounded-xl text-white text-sm hover:border-gold-primary/40 transition-colors whitespace-nowrap">
                {getOrderLabel()}
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {dateOptions.map((opt) => (
              <button key={opt.value} onClick={() => setDateFilter(opt.value)}
                ...
                <span>{opt.label}</span>
                {opt.count != null && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${dateFilter === opt.value ? "bg-black/20 text-white" : "bg-gold-primary/10 text-gold-primary"}`}>{opt.count}</span>
                )}
              </button>
            ))}
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {statusOptions.map((opt) => (
                <button key={opt.value}
                  onClick={() => { setStatusFilter(opt.value); if (opt.value === "updated" && sortBy === "created_at") setSortBy("last_update"); }}
                  className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-medium transition-all ${
                    statusFilter === opt.value
                      ? opt.value === "updated" ? "bg-gradient-to-r from-amber-600 to-amber-500 text-white shadow-lg shadow-amber-500/20" : "bg-gradient-to-r from-gold-dark to-gold-primary text-bg-primary shadow-gold-glow"
                      : "bg-bg-primary border border-gold-primary/20 text-text-secondary hover:text-white hover:border-gold-primary/40"
                  }`}>
                  <span>{opt.icon}</span>
                  <span>{opt.label}</span>
      ...
      {error && (
        <div className="glass-card rounded-xl p-6 border border-red-500/30 text-center">
          ...
          <button onClick={() => fetchBulkSignals(true)} className="px-4 py-2 bg-gold-primary/20 text-gold-primary rounded-lg hover:bg-gold-primary/30 transition-colors">Retry</button>
        </div>
      )}

      {!error && (
        <SignalsTable signals={signals} loading={loading} onRowClick={setSelectedSignal} sortBy={sortBy} sortOrder={sortOrder} onSort={handleSort}
          page={page} totalPages={totalPages} onPageChange={setPage} onPricesUpdate={handlePricesUpdate} />
      )}

      {selectedSignal && (
        <SignalModal key={selectedSignal.signal_id} signal={selectedSignal} isOpen={!!selectedSignal} onClose={() => setSelectedSignal(null)}
          onSwitchSignal={(newSignal) => { setSelectedSignal(null); setTimeout(() => setSelectedSignal(newSignal), 100); }} />
      )}
    </div>
  );
};
