...
const SignalDetailModal = ({ item, detail, loading, signalIds, currentIndex, onNavigate, onClose, cleanPair, t, onOpenHistory }) => {
  ...
  const events = [];
  events.push({ label: t('top.called_sig'), time: 'T+0', sub: fmtDt(created), detail: `${t('top.entry')} @ $${formatPrice(detail?.entry)}`, colors: themeColors.gold, isSL: false });
  if (detail?.updates) { detail.updates.forEach(u => { const isSL = u.update_type === 'sl' || u.update_type === 'sl1' || u.update_type === 'sl2'; events.push({ label: isSL ? t('top.sl_hit') : `${u.update_type?.toUpperCase().replace('TP','TP ')} ${t('top.hit')}`, time: `+${fmtDiff(created, u.update_at)}`, sub: fmtDt(u.update_at), detail: u.price > 0 ? `$${formatPrice(u.price)}${!isSL && detail.entry > 0 ? ` (+${((Math.abs(u.price - detail.entry) / detail.entry) * 100).toFixed(2)}%)` : ''}` : null, colors: isSL ? themeColors.red : themeColors.green, isSL }); }); }
                ...
                {!hasAnyImg ? (<div className="w-full h-[350px] sm:h-[450px] md:h-[500px] bg-[#0d0d0d] rounded-md border border-gold-primary/20 overflow-hidden relative shadow-lg"><div id="tv_chart_modal_topperf" className="absolute inset-0 w-full h-full" /></div>
                  ...
                  <div className="flex flex-col md:flex-row items-stretch gap-4 sm:gap-5 w-full">
                    ...
                    <div className="flex-1 w-full min-w-0 flex flex-col">
                      ...
                      {showInteractiveRight ? (<div className="relative rounded-md overflow-hidden border border-white/10 bg-[#0d0d0d] h-[250px] sm:h-[350px] md:h-[400px] w-full shadow-md"><div id="tv_chart_modal_topperf" className="absolute inset-0 w-full h-full" /></div>) : (<div className={`relative group rounded-md overflow-hidden border bg-[#0d0d0d] h-[250px] sm:h-[350px] md:h-[400px] w-full shadow-md ${isStopped ? 'border-loss/20' : 'border-white/10'}`}><img src={afterImg} alt="Latest" className="absolute inset-0 w-full h-full object-contain" loading="lazy" onError={(e) => { if (rawAfterImg && e.target.src !== rawAfterImg) { e.target.onerror = null; e.target.src = rawAfterImg; } }} /><div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3 backdrop-blur-sm z-10"><button onClick={() => setShowTV(true)} className="px-4 py-2 bg-white/10 text-white hover:bg-white/20 rounded font-bold text-xs border border-white/20 flex items-center gap-2"><span>{t('top.interactive')}</span><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg></button><button onClick={() => setLightboxImg(afterImg)} className="text-white/60 hover:text-white text-[10px] underline">{t('top.view_full')}</button></div></div>)}
                    </div>
                  </div>
                )}
