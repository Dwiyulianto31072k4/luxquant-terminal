...
// ════════════════════════════════════════════
// REVISED: SignalDetailModal (FIXED LAYOUT: items-start + gap atas)
// ════════════════════════════════════════════
const SignalDetailModal = ({ item, detail, loading, signalIds, currentIndex, onNavigate, onClose, cleanPair }) => {
  ...
  // TEMA ELEGAN: Gold untuk Entry, Green untuk Profit, Red untuk Loss
  const themeColors = {
  ...
  // Susun Array Event Timeline
  const events = [];
  events.push({ 
    label: 'Signal Called', time: 'T+0', sub: fmtDt(created), 
    detail: `Entry @ $${formatPrice(detail?.entry)}`, 
  ...
  if (detail?.updates) {
    detail.updates.forEach((u) => {
      ...
      events.push({
        label: isSL ? 'Stop Loss Hit' : `${u.update_type?.toUpperCase().replace('TP','TP ')} Hit`,
  ...
  // === RENDER KONTEN MODAL ===
  const modalContent = (
    // PERUBAHAN UTAMA: Gunakan items-start dan pt-[90px] agar modal selalu didorong ke bawah sejauh 90px (tidak menutupi header web)
    <div className={`fixed inset-0 z-[100000] flex items-start justify-center px-3 py-4 sm:px-6 md:px-8 pt-[80px] sm:pt-[100px] pb-6 isolation-isolate ${isClosing ? 'animate-[smBO_.2s_ease-in_forwards]' : 'animate-[smBI_.25s_ease-out]'}`}>
      ...
      {/* Container ini sekarang dilimit height-nya berdasarkan tinggi layar dikurangi padding atas-bawah */}
        ...
        <div className="flex-shrink-0 bg-[#0a0a0a] border-b border-gold-primary/30 px-4 py-3 z-10">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <p className="text-text-muted text-xs mt-0.5 truncate">Signal Called: {fmtDt(created)}</p>
              </div>
            </div>
          ...
          {multi && (
            <div className="flex items-center justify-center gap-4 mt-3 pt-3 border-t border-gold-primary/10">
              <button onClick={() => onNavigate(currentIndex - 1)} disabled={currentIndex <= 0} className="px-3 py-1 rounded border border-gold-primary/20 text-gold-primary hover:bg-gold-primary/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-[10px] sm:text-xs font-bold">← Prev</button>
              <div className="flex items-center gap-2">
                <span className="text-text-muted text-[10px] sm:text-xs hidden sm:inline">Signal</span>
              ...
              <button onClick={() => onNavigate(currentIndex + 1)} disabled={currentIndex >= total - 1} className="px-3 py-1 rounded border border-gold-primary/20 text-gold-primary hover:bg-gold-primary/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-[10px] sm:text-xs font-bold">Next →</button>
            </div>
          )}
        </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <div className="text-center">
                ...
                <p className="text-gold-primary font-mono text-sm">Loading Signal Data...</p>
              </div>
            </div>
          ) : detail ? (
            <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-4">
              ...
              <div className="w-full">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-gold-primary text-xs sm:text-sm font-semibold flex items-center gap-2">📸 Trade Proof</span>
                  {detail.message_link && (
                    ...
                    <div className="flex-1 w-full min-w-0 flex flex-col">
                      <div className="flex items-center justify-between mb-2 px-1">
                        <span className="text-blue-400 text-[10px] sm:text-xs font-bold tracking-wide uppercase flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span> Before (Entry)
                        </span>
                      </div>
                      {entryImg ? (
                        <div className="relative group rounded-xl overflow-hidden border border-blue-500/20 bg-[#0d0d0d] h-[250px] sm:h-[350px] md:h-[400px] w-full cursor-zoom-in shadow-md" onClick={() => setLightboxImg(entryImg)}>
                          ...
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center pointer-events-none">
                            <span className="opacity-0 group-hover:opacity-100 bg-black/80 text-white text-[10px] sm:text-xs px-3 py-1.5 rounded font-medium backdrop-blur-sm transition-all shadow-xl">🔍 Fullscreen</span>
                          </div>
                        </div>
                      ) : (
                        <div className="rounded-xl border border-dashed border-gray-700 bg-gray-800/20 flex flex-col items-center justify-center h-[250px] sm:h-[350px] md:h-[400px] w-full text-gray-500">
                          ...
                          <p className="text-xs">Waiting for entry screenshot...</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex-1 w-full min-w-0 flex flex-col">
                      <div className="flex items-center justify-between mb-2 px-1 min-h-[20px]">
                        <span className={`text-[10px] sm:text-xs font-bold tracking-wide uppercase flex items-center gap-1.5 ${isStopped ? 'text-red-400' : 'text-green-400'}`}>
                          ...
                          After ({status === 'open' ? 'LATEST' : sLabel(status)})
                        </span>
                        
                        {/* Tombol Back ke Gambar */}
              ...
              {/* SECTION BAWAH: TIMELINE HORIZONTAL (FIT & ELEGAN) */}
              <div className="space-y-6">
                      ...
                      {events.map((ev, i) => {
                        ...
                        return (
                          <div key={i} className="relative flex flex-col items-center flex-1 w-0 group z-10">
                            ...
                            {/* Text Info */}
                            {/* Menggunakan shrink, max-w-full dan break-words agar text tidak bertumpuk */}
                            <div className="mt-2.5 text-center flex flex-col items-center px-0.5 sm:px-1 w-full max-w-full">
                              ...
                              {/* Subtext disembunyikan jika layar terlalu kecil atau diganti truncate */}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                    <StatBlock label="Duration" value={detail.updates?.length > 0 ? fmtDiff(created, detail.updates[detail.updates.length - 1].update_at) : 'Active'} />
                    <StatBlock label="Volume Rank" value={detail.volume_rank_num && detail.volume_rank_den ? `#${detail.volume_rank_num} / ${detail.volume_rank_den}` : 'N/A'} />
                    <StatBlock label="Risk Level" value={detail.risk_level || 'N/A'} valueClass={detail.risk_level === 'High' ? 'text-red-400' : detail.risk_level === 'Medium' ? 'text-yellow-400' : 'text-green-400'} />
                    <StatBlock label="Signal ID" value={detail.signal_id ? `${detail.signal_id.slice(0, 8)}...` : 'N/A'} valueClass="text-text-muted" />
                  </div>
                </div>
             ...
             <div className="flex items-center justify-center py-20"><p className="text-text-muted text-sm">Failed to load signal details</p></div>
          )}
        </div>
      </div>
