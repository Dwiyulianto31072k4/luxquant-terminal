...
const IC = {
  ...
  tool: (
    <>
      <path d="M3 21l5-5" />
      <path d="M14 4a4 4 0 0 0 5 5l-9 9-5-5 9-9z" />
    </>
  ),
  swap: (
    <>
  ...
  dollar: (
    <>
      <path d="M12 3v18" />
      <path d="M16 7.5C16 6 14.5 5 12 5S8 6 8 7.5 9.5 10 12 10s4 1 4 2.5S14.5 15 12 15s-4-1-4-2.5" />
    </>
  ),
  store: (
    <>
  ...
  help: (
    <>
      <circle cx="12" cy="12" r="9" />
      <path d="M9.5 9.5a2.5 2.5 0 0 1 5 0c0 2-2.5 2-2.5 3.5" />
      <path d="M12 17h.01" />
    </>
  ),
  bulb: (
...
function IconBadge({ d, color = "rgb(var(--accent))", size = 22 }) {
  return (
    <span
...
function SectionHead({ d, color = "rgb(var(--accent))", children }) {
  return (
    <h3
      className="mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider sm:text-xs"
      style={{ color }}
    >
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-[10px] text-text-muted">
          ...
          {coinData?.market_cap_rank && (
            <span className="ml-2 text-text-muted">Rank #{coinData.market_cap_rank}</span>
          )}
        </p>
      </div>
    </div>
    ...
    >
      <div className="px-3 py-4 sm:px-5 sm:py-5">
        ...
        {coinData && !coinData.is_categorized && (
          <div className="flex flex-col items-center justify-center rounded-lg border border-ink/12 bg-accent/10 p-6 text-center">
            <span className="mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-accent/12 text-accent ring-1 ring-accent/25">
        ...
        {coinData && coinData.is_categorized && (
          <div className="space-y-3 sm:space-y-4">
            {/* HERO */}
            <div className="space-y-3 rounded-xl border border-ink/12 bg-gradient-to-br from-accent/15 to-accent/5 p-3 sm:p-4">
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-lg border border-ink/12 bg-accent/12 px-2.5 py-1 text-xs font-bold text-accent">
                ...
                {coinData.has_utility === true && (
                  <span className="inline-flex items-center gap-1.5 rounded-lg border border-profit/25 bg-profit/15 px-2.5 py-1 text-xs font-bold text-profit">
              ...
              {coinData.summary && (
                <p className="text-xs leading-relaxed text-text-primary/85 sm:text-sm">
                  {coinData.summary}
                </p>
              )}
            </div>
            ...
            {((coinData.use_cases && coinData.use_cases.length > 0) ||
              ...
              <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-2">
                {coinData.use_cases && coinData.use_cases.length > 0 && (
                  <div className="h-full rounded-xl border border-ink/10 bg-surface-raised p-3 sm:p-4">
            ...
            {coinData.utility_details && Object.keys(coinData.utility_details).length > 0 && (
              <div className="rounded-xl border border-ink/10 bg-surface-raised p-3 sm:p-4">
                ...
                <div className="grid grid-cols-1 gap-x-4 gap-y-2 sm:grid-cols-2">
                  {Object.entries(coinData.utility_details).map(([key, value]) => (
                    <div key={key} className="flex flex-col gap-0.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-text-muted sm:text-xs">
                        {key.replace(/_/g, " ")}
                      </span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
              ...
              {coinData.website && (
                <a
                  ...
                  className="flex items-center gap-1.5 text-[10px] text-text-muted transition-colors hover:text-text-primary sm:text-xs"
                >
            ...
            <div className="flex items-start gap-2 rounded-lg border border-ink/5 bg-surface-raised p-3">
              <span className="mt-px text-text-muted">
                <Icon d={IC.bulb} className="h-3.5 w-3.5" />
              </span>
              <p className="text-[9px] leading-relaxed text-text-muted sm:text-[10px]">
                This categorization is for educational purposes only and is automatically generated.
                Each user should make their own decisions based on personal values, risk tolerance,
                and applicable regulations. Categorization is not financial, legal, or religious
                advice.
              </p>
            </div>
          </div>
