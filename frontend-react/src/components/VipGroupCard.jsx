      ...
      <div className="p-4 sm:p-5">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-text-primary text-xs font-medium">
                {t("vip.name", "VIP Signal Group")}
              </p>
              {inGroup ? (
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="w-1 h-1 rounded-full bg-profit flex-shrink-0" />
                  <p className="text-profit/80 text-[10px] font-mono truncate">
                    {t("vip.member", "Member")}
                  </p>
                </div>
              ) : (
          ...
          <div className="flex-shrink-0">
            {!hasAccess ? (
              ...
              <span className="px-2.5 py-1.5 rounded-md font-mono text-[9px] uppercase tracking-wider text-profit/70 border border-profit/15 bg-profit/5">
                {t("vip.joined", "Joined")}
              </span>
            ) : !telegramLinked ? (
        ...
        {hasAccess && !telegramLinked && !inGroup && (
          <p className="text-text-muted/40 text-[10px] mt-3 leading-relaxed">
            {t(
              "vip.link_first_hint",
              "Connect your Telegram account above, then come back to join the VIP group."
            )}
          </p>
        )}
      </div>
