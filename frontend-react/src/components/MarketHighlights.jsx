...
const MarketHighlights = () => {
  ...
  const fetchMarketData = async () => {
    try {
      ...
      if (Array.isArray(coinsData) && globalData.data) {
        ...
        const stablecoinDom =
          (global.market_cap_percentage?.usdt || 0) + (global.market_cap_percentage?.usdc || 0);

        const processed = {
          // Hot - top by market cap
          hot: coinsData.slice(0, 3).map((coin) => ({
          ...
          marketOverview: {
            ...
            altcoinMarketCap:
              (global.total_market_cap?.usd || 0) *
              (1 - (global.market_cap_percentage?.btc || 0) / 100),
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
        <div className="glass-card rounded-xl p-4 border border-ink/08 animate-pulse">
        ...
        <div className="md:col-span-2 glass-card rounded-xl p-4 border border-ink/08 animate-pulse">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
      {/* Hot */}
      <CategoryCard icon="🔥" label="Hot" coins={marketData.hot} formatChange={formatChange} />

      {/* Market Overview - Expanded 2 columns */}
      <div className="md:col-span-2 glass-card rounded-xl border border-ink/08 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-ink/08">
        ...
        <div className="p-4">
          {/* Top Row - Big Numbers */}
          <div className="grid grid-cols-3 gap-4 mb-4 pb-4 border-b border-ink/08">
            {/* Total Market Cap */}
            <div>
              ...
              <p className="text-text-primary font-mono text-lg font-bold">
                {formatLargeNumber(overview.totalMarketCap)}
              </p>
              <p
                className={`text-xs font-semibold ${overview.marketCapChange24h >= 0 ? "text-positive" : "text-negative"}`}
              >
            ...
            <div>
              ...
              <p className="text-text-primary font-mono text-lg font-bold">
                {formatLargeNumber(overview.totalVolume24h)}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`font-mono text-lg font-bold ${getFearGreedColor(overview.fearGreed)}`}
                >
                  {overview.fearGreed}
                </span>
                <span
                  className={`text-[10px] px-1.5 py-0.5 rounded ${getFearGreedBg(overview.fearGreed)} ${getFearGreedColor(overview.fearGreed)}`}
                >
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            {/* Altcoin Cap */}
            <div className="text-center p-2 rounded-lg bg-accent/10 border border-ink/10">
              <p className="text-text-muted text-[10px] uppercase mb-1">Altcoin Cap</p>
              <p className="text-accent font-mono text-sm font-bold">
                {formatLargeNumber(overview.altcoinMarketCap)}
              </p>
            </div>
      ...
      <div className="space-y-3">
        {/* Top Gainer - Compact */}
        <div className="glass-card rounded-xl border border-ink/08 overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-ink/08">
            <div className="flex items-center gap-1.5">
              <span className="text-xs">📈</span>
              <span className="text-text-primary font-semibold text-xs">Top Gainer</span>
            </div>
            <button className="text-text-muted text-[10px] hover:text-text-primary transition-colors">
              More
            </button>
          </div>
                ...
                <div className="flex items-center gap-2">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = `https://ui-avatars.com/api/?name=${coin.symbol}&background=d4a853&color=0a0506&size=20&bold=true`;
                    }}
                  />
          ...
          <div className="divide-y divide-accent/5">
            {marketData.topVolume?.slice(0, 2).map((coin, idx) => (
              ...
              >
                ...
                <span
                  className={`text-xs font-semibold ${coin.change >= 0 ? "text-positive" : "text-negative"}`}
                >
                  {formatChange(coin.change)}
                </span>
              </div>
...
const CategoryCard = ({ icon, label, coins, formatChange }) => (
  <div className="glass-card rounded-xl border border-ink/08 overflow-hidden group hover:border-ink/12 transition-all">
    <div className="flex items-center justify-between px-4 py-3 border-b border-ink/08">
      <div className="flex items-center gap-2">
        <span className="text-sm">{icon}</span>
        <span className="text-text-primary font-semibold text-sm">{label}</span>
      </div>
      <button className="text-text-muted text-xs hover:text-text-primary transition-colors flex items-center gap-1">
        More
    ...
    <div className="divide-y divide-accent/5">
      {coins?.map((coin, idx) => (
        <div
          key={idx}
          className="flex items-center justify-between px-4 py-2.5 hover:bg-accent/10 cursor-pointer transition-colors"
        >
