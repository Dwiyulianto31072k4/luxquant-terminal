      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1.5">
        {data.map((d, i) => (
          <div key={i} className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-mono tabular-nums text-text-primary font-medium">
              {d.value}
            </span>
...
const StatCard = ({ label, value, sub, color = "text-text-primary", icon }) => (
  <div className="rounded-xl border border-ink/[0.07] bg-ink/[0.02] p-3 flex-1 min-w-[100px]">
    ...
    <p className={`text-lg sm:text-xl font-semibold font-mono tabular-nums leading-none ${color}`}>
      {value}
    </p>
    ...
    <div className="flex-1 overflow-y-auto px-3 py-3 sm:px-6 sm:py-4 custom-scrollbar bg-surface">
      <div className="max-w-4xl mx-auto space-y-3 sm:space-y-4 pb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <StatCard
            icon={
              stats.streak_type === "win" ? Ic.flame("w-3.5 h-3.5") : Ic.snowflake("w-3.5 h-3.5")
            }
            label="Streak"
