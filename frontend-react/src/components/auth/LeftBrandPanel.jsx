...
export const TypewriterLine = ({ mobile }) => {
 ...
 {parts.map((p, i) => (<span key={i} style={{ color: p.g ? 'rgb(var(--accent))' : '#8a7d73' }}>{p.text}</span>))}
...
export const DeviceShowcase = ({ compact = false, phone = true }) => {
 ...
 style={{ width: '132%', height: '120%', background: 'radial-gradient(ellipse at center, rgb(var(--accent) / 0.13) 0%, transparent 64%)' }} />

 {/* iMac (static — no float) */}
 <div className="relative">
 <div className="relative overflow-hidden rounded-[12px] bg-black ring-1 ring-ink/[0.07]"
 style={{ boxShadow: '0 36px 80px rgb(var(--scrim) / 0.35), 0 0 56px rgb(var(--accent) / 0.10)' }}>
 ...
 style={{ boxShadow: '0 24px 50px rgb(var(--scrim) / 0.85), 0 0 34px rgb(var(--accent) / 0.18)' }}>
...
const MARKETS = [
 ...
 label: 'Crypto',
 icon: (
 <svg viewBox="0 0 24 24" fill="rgb(var(--accent))" className="h-[18px] w-[18px]" aria-hidden="true">
