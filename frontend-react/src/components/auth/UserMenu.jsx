...
const UserMenu = () => {
    ...
    return (
      <div className="flex items-center gap-1.5 sm:gap-2">
        <button
          ...
          className="px-2.5 sm:px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium text-accent border border-ink/12 hover:bg-accent/12 transition-all whitespace-nowrap"
        >
          {t("userMenu.login")}
        </button>
        <button
          onClick={() => navigate("/register")}
          className="hidden sm:block px-3 py-1.5 rounded-lg text-sm font-bold bg-gradient-to-r from-accent to-accent text-bg-primary hover: transition-all whitespace-nowrap"
        >
          {t("userMenu.register")}
        </button>
      </div>
  ...
  const getSubscriptionInfo = () => {
    ...
    if (role === "premium" || role === "subscriber") {
      ...
      else if (note.toLowerCase().includes("yearly") || note.includes("1 Tahun") || daysLeft > 60)
        planLabel = t("userMenu.plan_yearly");
  ...
  const getPlanBadge = () => {
    switch (subscription.status) {
      case "active":
        return { label: subscription.plan, color: "bg-profit/15 text-profit border-profit/20" };
      case "expired":
        return {
  ...
  const getSubLabel = () => {
    if (subscription.status === "active")
      return `${subscription.plan} ${t("userMenu.subs_plan_suffix")}`;
 ...
 `}</style>
      ...
      <button
        ...
        className={`flex items-center justify-center w-9 h-9 rounded-full transition-all duration-200 ${
          isOpen
            ? "ring-2 ring-accent/40 ring-offset-2 ring-offset-bg-primary"
            : "hover:ring-2 hover:ring-ink/10 hover:ring-offset-1 hover:ring-offset-bg-primary"
        }`}
      >
        {avatarUrl ? (
          <div
            className="w-9 h-9 rounded-full p-[2px] bg-gradient-to-br from-accent via-ink to-accent"
            style={{
              boxShadow: "0 0 10px rgb(var(--accent) / 0.5), 0 0 20px rgb(var(--accent) / 0.2)",
            }}
          >
            <img
              src={avatarUrl}
        ...
        <div
          className="w-9 h-9 rounded-full bg-gradient-to-br from-accent via-ink to-accent items-center justify-center"
          style={{
      ...
      {isOpen && (
        <div
          className={`absolute right-0 mt-2 w-72 rounded-2xl overflow-hidden z-50 shadow-2xl shadow-black/60 ${isClosing ? "user-menu-exit" : "user-menu-enter"}`}
          style={{
        ...
        >
          ...
          <div className="px-4 pt-4 pb-3">
            <div className="flex items-start gap-3">
              {avatarUrl ? (
                <div
                  className="w-11 h-11 rounded-full p-[2px] bg-gradient-to-br from-accent via-ink to-accent flex-shrink-0"
                  style={{
                    boxShadow:
                      "0 0 12px rgb(var(--accent) / 0.5), 0 0 24px rgb(var(--accent) / 0.2)",
                  }}
                >
                  <img
                    src={avatarUrl}
              ...
              <div
                className="w-11 h-11 rounded-full bg-gradient-to-br from-accent via-ink to-accent items-center justify-center flex-shrink-0"
                style={{
                  display: avatarUrl ? "none" : "flex",
                  boxShadow: "0 0 12px rgb(var(--accent) / 0.5), 0 0 24px rgb(var(--accent) / 0.2)",
                }}
              >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded text-[9px] font-bold border ${badge.color} flex-shrink-0`}
                  >
                    {badge.label}
                  </span>
                </div>
                ...
                {subscription.status === "active" &&
                  ...
                  subscription.plan !== t("userMenu.plan_admin") && (
                    <p className="text-profit/70 text-[10px] mt-1">
                      {t("userMenu.lifetime_badge")}
                    </p>
                  )}
                {subscription.status === "active" &&
                  subscription.plan === t("userMenu.plan_admin") && (
                    <p className="text-loss/70 text-[10px] mt-1">{t("userMenu.admin_badge")}</p>
                  )}
                {subscription.status === "expired" && (
                  <p className="text-loss/70 text-[10px] mt-1">{t("userMenu.subs_expired")}</p>
                )}
          ...
          <div className="py-1.5 px-1.5">
            ...
            <MenuItem
              ...
              badge={
                subscription.status === "free"
                  ? { label: t("userMenu.upgrade_badge"), color: "bg-accent text-accent-fg" }
                  : null
              }
            />
            <MenuItem
...
const MenuItem = ({ icon, label, sublabel, onClick, badge }) => (
  ...
  >
    <svg
      className="w-[18px] h-[18px] flex-shrink-0 text-text-muted group-hover:text-text-muted transition-colors"
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
    ...
    <div className="flex-1 text-left min-w-0">
      ...
      {sublabel && (
        <p className="text-[10px] text-text-muted group-hover:text-text-muted/80 truncate mt-0.5">
          {sublabel}
        </p>
      )}
    </div>
    <svg
      className="w-3.5 h-3.5 text-text-muted/30 group-hover:text-text-muted/50 transition-colors flex-shrink-0"
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
