...
const UserMenu = () => {
 ...
 case 'expired':
 return { label: t('userMenu.status_expired'), color: 'bg-red-500/15 text-loss border-red-500/20' };
 default:
 ...
 >
 {avatarUrl ? (
 <div className="w-9 h-9 rounded-full p-[2px] bg-gradient-to-br from-accent via-ink to-accent" style={{ boxShadow: '0 0 10px rgb(var(--accent) / 0.5), 0 0 20px rgb(var(--accent) / 0.2)' }}>
 <img 
 src={avatarUrl} 
 alt={user?.username} 
 ...
 style={{ display: avatarUrl ? 'none' : 'flex', boxShadow: '0 0 10px rgb(var(--accent) / 0.5), 0 0 20px rgb(var(--accent) / 0.2)' }}
 >
 ...
 {avatarUrl ? (
 <div className="w-11 h-11 rounded-full p-[2px] bg-gradient-to-br from-accent via-ink to-accent flex-shrink-0" style={{ boxShadow: '0 0 12px rgb(var(--accent) / 0.5), 0 0 24px rgb(var(--accent) / 0.2)' }}>
 <img 
 src={avatarUrl} 
 alt={user?.username}
 className="w-full h-full rounded-full object-cover"
 ...
 style={{ display: avatarUrl ? 'none' : 'flex', boxShadow: '0 0 12px rgb(var(--accent) / 0.5), 0 0 24px rgb(var(--accent) / 0.2)' }}
 >
 ...
 <p className="text-profit/70 text-[10px] mt-1">{t('userMenu.lifetime_badge')}</p>
 )}
 ...
 <p className="text-text-muted text-[10px] mt-1">
 <span className="text-accent cursor-pointer hover:underline" onClick={() => { handleClose(); navigate('/pricing'); }}>
 ...
 badge={subscription.status === 'free' ? { label: t('userMenu.upgrade_badge'), color: 'bg-accent text-accent-fg' } : null}
 />
 <MenuItem
...
const MenuItem = ({ icon, label, sublabel, onClick, badge }) => (
 ...
 >
 <svg className="w-[18px] h-[18px] flex-shrink-0 text-text-muted group-hover:text-text-muted transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 {icon}
 </svg>
