    ...
    <div className="min-h-screen flex" style={{ background: '#0a0506' }}>
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center relative overflow-y-auto">
        ...
        <div className="relative z-10 w-full max-w-md px-8 py-10">
          ...
          {displayError && (
            <div className="mb-5 p-4 rounded-xl text-sm flex items-center gap-3" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171' }}>
          ...
          <div className="space-y-3 mb-5">
            <SocialBtn icon={<GoogleIcon />} text="Daftar dengan Google" />
            <SocialBtn icon={<TelegramIcon />} text="Daftar dengan Telegram" />
          </div>

          <div className="flex items-center gap-4 mb-5">
            <div className="flex-1 h-px" style={{ background: 'rgba(212,168,83,0.1)' }} />
            <span style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#6b5c52' }}>Atau daftar dengan email</span>
            <div className="flex-1 h-px" style={{ background: 'rgba(212,168,83,0.1)' }} />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs" style={{ color: '#6b5c52' }}>Huruf, angka, underscore. Min 3 karakter</p>
            </div>
            <div>
              ...
              {password && (
                <div className="mt-2 flex items-center gap-2">
                  <div className="flex-1 flex gap-1">
                    {[1,2,3,4].map(l => (
                      <div key={l} className="h-1 flex-1 rounded-full transition-all" style={{ background: l <= strength.level ? strength.color : 'rgba(212,168,83,0.1)' }} />
                    ))}
                  </div>
          ...
          <p className="mt-3 text-center" style={{ color: '#6b5c52', fontSize: 11 }}>
            ...
            <a href="#" className="underline hover:opacity-80" style={{ color: '#b8a89a' }}>Terms & Conditions</a>
            {' '}dan{' '}
            <a href="#" className="underline hover:opacity-80" style={{ color: '#b8a89a' }}>Privacy Policy</a>
          </p>
        </div>
      </div>
    </div>
...
const FormInput = ({ label, type = 'text', value, onChange, placeholder }) => (
  <div>
    <label className="block text-sm font-medium mb-1.5" style={{ color: '#b8a89a' }}>{label}</label>
...
const PasswordField = ({ value, onChange, show, toggle, placeholder }) => (
  <div className="relative">
    <input type={show ? 'text' : 'password'} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} required
      className="w-full px-4 py-3 pr-12 rounded-xl text-white text-sm focus:outline-none transition-all duration-200"
      style={{ background: 'rgba(18,8,9,0.8)', border: '1px solid rgba(212,168,83,0.15)' }}
      onFocus={e => { e.target.style.borderColor = 'rgba(212,168,83,0.5)'; }}
      onBlur={e => { e.target.style.borderColor = 'rgba(212,168,83,0.15)'; }} />
