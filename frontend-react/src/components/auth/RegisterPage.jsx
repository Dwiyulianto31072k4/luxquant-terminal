...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      setLocalError(a("err_password_mismatch"));
      return;
    }
    if (password.length < 8) {
      setLocalError(a("err_password_min"));
      return;
    }
    if (username.length < 3) {
      setLocalError(a("err_username_min"));
      return;
    }
    setLoading(true);
    try {
    ...
    >
      ...
      <div className="w-full lg:w-[45%] flex items-center justify-center relative flex-1 overflow-y-auto p-4 sm:p-6 lg:p-0">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(to bottom, transparent 10%, rgb(var(--accent) / 0.15) 50%, transparent 90%)",
          }}
        />
        ...
        <div
          className="relative z-10 w-full max-w-md px-5 py-5 sm:px-10 sm:py-10 rounded-[2rem] transition-all duration-500 mt-16 sm:mt-20 lg:mt-10 mb-6 sm:mb-8"
          style={{
        ...
        >
          ...
          <div className="mb-1 text-center lg:text-left">
            <h1
              className="font-display text-2xl lg:text-3xl font-semibold text-text-primary tracking-tight mb-1.5"
          ...
          {displayError && (
            <div
              className="mb-4 p-3.5 rounded-xl text-sm flex items-center gap-3 animate-pulse"
              style={{
          ...
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            ...
            <div>
              <FormInput
                ...
                placeholder={a("username_placeholder")}
              />
              <p className="mt-1 text-xs ml-1" style={{ color: "rgb(var(--fg-muted))" }}>
                {a("username_hint")}
              </p>
            </div>
            <div>
              ...
              <PasswordField
                ...
                placeholder={a("password_min")}
              />
              {password && (
                <div className="mt-2 bg-scrim/30 p-2 rounded-xl border border-ink/5 transition-all">
                  <div className="flex items-center gap-3">
                    <span
                      className="text-[11px] font-bold uppercase tracking-wider w-12 text-right transition-colors"
                    ...
                    <div className="flex-1 flex gap-1.5">
                      {[1, 2, 3, 4].map((l) => (
                        <div
                          ...
                          style={{
                            background:
                              l <= strength.level ? strength.color : "rgb(var(--ink) / 0.08)",
                          }}
                        />
                      ))}
                    </div>
            ...
            <div>
              ...
              <PasswordField
                ...
                placeholder={a("confirm_placeholder")}
              />
            ...
            <div className="pt-1 sm:pt-3">
              <GoldButton
                loading={loading}
                text={a("register_button")}
                loadingText={a("register_loading")}
              />
            </div>
          </form>
          ...
          <p
            className="mt-2 sm:mt-4 text-center pb-1"
    ...
    const t = setTimeout(() => {
      ...
      try {
        ...
        const sc = new T.Scene(),
          cam = new T.PerspectiveCamera(50, w / h, 0.1, 2000);
        ...
        ct.enablePan = false;
        ct.autoRotate = true;
        ct.autoRotateSpeed = 0.8;
        sc.add(new T.AmbientLight(0xffffff, 0.6));
        const dl = new T.DirectionalLight(0xfff5e0, 1.0);
        ...
        const LOCS = [
          ...
          { lat: 39.933, lng: 32.86, s: 0.7 },
          { lat: 40.713, lng: -74.006, s: 0.65 },
          { lat: -6.209, lng: 106.846, s: 0.6 },
          { lat: 35.676, lng: 139.65, s: 0.5 },
          { lat: 51.507, lng: -0.128, s: 0.55 },
          { lat: 37.567, lng: 126.978, s: 0.45 },
        ];
        ...
        const hub = LOCS[0];
        g.arcsData(
          LOCS.filter((l) => !l.hub).map((l) => ({
            startLat: hub.lat,
            startLng: hub.lng,
            endLat: l.lat,
            endLng: l.lng,
            color: ["#F0B90B", "rgba(240,185,11,0.25)"],
            stroke: 0.4,
          }))
        )
          .arcColor("color")
        ...
        >
          <div
            style={{
              width: 32,
              height: 32,
              border: "2px solid rgb(var(--line) / 0.15)",
  ...
  <div>
    ...
    <input
      ...
      onFocus={(e) => {
        ...
        e.target.style.boxShadow = "0 0 0 4px rgb(var(--accent) / 0.15)";
        e.target.style.background = "rgb(var(--scrim) / 0.35)";
      }}
      onBlur={(e) => {
        e.target.style.borderColor = "rgb(var(--accent) / 0.15)";
...
const PasswordField = ({ value, onChange, show, toggle, placeholder }) => (
  <div className="relative group">
    ...
    <button
      type="button"
      onClick={toggle}
      className="absolute right-4 top-1/2 p-1 rounded-md transition-all"
      style={{
...
const GoldButton = ({ loading, text, loadingText }) => (
  ...
  >
    <span
      ...
      style={{ background: "linear-gradient(135deg, #fceca1 0%, rgb(var(--accent)) 100%)" }}
    />
...
const Divider = ({ text }) => (
  <div className="flex items-center gap-4 my-4 sm:my-7 opacity-70">
    <div
      ...
      style={{ background: "linear-gradient(to right, transparent, rgb(var(--accent) / 0.4))" }}
    />
    <span
      style={{
        fontSize: 11,
    ...
    <div
      className="flex-1 h-px"
      style={{ background: "linear-gradient(to left, transparent, rgb(var(--accent) / 0.4))" }}
    />
  </div>
);
...
const SocialBtn = ({ icon, text }) => (
  <button
    ...
    onMouseEnter={(e) => {
      ...
      e.currentTarget.style.background = "rgb(var(--accent) / 0.08)";
      e.currentTarget.style.color = "#fff";
    }}
    onMouseLeave={(e) => {
      e.currentTarget.style.borderColor = "rgb(var(--accent) / 0.15)";
      e.currentTarget.style.background = "rgb(var(--ink) / 0.02)";
