...
const RegisterPage = () => {
 ...
 <div className="absolute inset-0 border-2 rounded-full" style={{ borderColor: 'rgb(var(--accent) / 0.2)' }} />
 <div className="absolute inset-0 border-2 border-transparent rounded-full animate-spin" style={{ borderTopColor: 'rgb(var(--accent))' }} />
 </div>
 ...
 <div className="hidden lg:block absolute left-0 top-0 h-full w-px" style={{ background: 'linear-gradient(to bottom, transparent 10%, rgb(var(--accent) / 0.15) 50%, transparent 90%)' }} />
...
const MobileGlobeInner = () => {
 ...
 <div style={{ width: 32, height: 32, border: '2px solid rgb(var(--line) / 0.15)', borderTopColor: 'rgb(var(--accent))', borderRadius: '50%', animation: 'lq-spin 1s linear infinite' }} />
 </div>
 )}
...
const PasswordField = ({ value, onChange, show, toggle, placeholder }) => (
 ...
 onMouseEnter={e => { e.currentTarget.style.color = 'rgb(var(--accent))'; e.currentTarget.style.background = 'rgb(var(--accent) / 0.1)'; }} 
...
const GoldButton = ({ loading, text, loadingText }) => (
 ...
 style={{ background: 'linear-gradient(135deg, rgb(var(--accent)) 0%, rgb(var(--accent)) 100%)', color: 'rgb(var(--surface))', boxShadow: loading ? 'none' : '0 10px 25px -5px rgb(var(--accent) / 0.4)' }}>
 <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: 'linear-gradient(135deg, #fceca1 0%, rgb(var(--accent)) 100%)' }} />
...
const Divider = ({ text }) => (
 ...
 <div className="flex-1 h-px" style={{ background: 'linear-gradient(to right, transparent, rgb(var(--accent) / 0.4))' }} />
 <span style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'rgb(var(--fg-muted))' }}>{text}</span>
 <div className="flex-1 h-px" style={{ background: 'linear-gradient(to left, transparent, rgb(var(--accent) / 0.4))' }} />
 </div>
);
...
const SocialBtn = ({ icon, text }) => (
 ...
 onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgb(var(--accent) / 0.4)'; e.currentTarget.style.background = 'rgb(var(--accent) / 0.08)'; e.currentTarget.style.color = '#fff'; }}
 onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgb(var(--accent) / 0.15)'; e.currentTarget.style.background = 'rgb(var(--ink) / 0.02)'; e.currentTarget.style.color = '#b8a89a'; }}>
 {icon} <span>{text}</span>
 </button>
);
