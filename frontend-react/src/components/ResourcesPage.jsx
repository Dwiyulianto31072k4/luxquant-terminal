...
const TYPE_META = {
  ...
  pdf: { label: "Guide", color: "rgb(var(--neg-text))" },
  video: { label: "Video", color: "#f97316" },
  link: { label: "Link", color: "rgb(var(--pos-text))" },
};
    ...
    >
      ...
      <div className="p-6 lg:p-8 flex flex-col justify-center">
        <div className="flex items-center gap-2 mb-3">
          <span className="px-2 py-0.5 rounded bg-accent text-accent-fg text-[10px] font-bold uppercase tracking-wider">
            Featured
          </span>
        ...
        {resource.excerpt && (
          <p className="text-text-muted text-sm mt-3 line-clamp-3 leading-relaxed">
            {stripMarkdown(resource.excerpt)}
          </p>
        )}
        <div className="flex items-center gap-2 text-[11px] text-text-muted mt-4">
          {resource.author_name && (
            <span className="text-text-secondary font-medium">{resource.author_name}</span>
          )}
    ...
    >
      ...
      >
        {cover ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <svg
              className="w-11 h-11 text-accent/25"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        {isVideo && (
          <div className="absolute inset-0 flex items-center justify-center opacity-90 group-hover:opacity-100 transition">
            <div className="w-12 h-12 rounded-full bg-scrim/55 backdrop-blur flex items-center justify-center border border-ink/20 group-hover:scale-110 transition-transform">
              <svg
                className="w-5 h-5 text-text-primary ml-0.5"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path d="M8 5v14l11-7z" />
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between text-[10px] text-text-muted">
          <span className="truncate">
            {resource.author_name || fmtDate(resource.published_at || resource.created_at)}
          </span>
          {resource.author_name && (
            <span className="shrink-0 ml-2">
              {fmtDate(resource.published_at || resource.created_at)}
            </span>
          )}
        </div>
      </div>
...
const AdminActions = ({ resource, onEdit, onDelete }) => (
  ...
  >
    ...
    <button
      ...
      className="p-1.5 bg-scrim/70 backdrop-blur rounded-lg text-loss hover:text-loss border border-loss/20"
      title="Delete"
    >
      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path
...
const ResourcesPage = () => {
  ...
  useEffect(() => {
    fetchAll();
  }, [activeType, activeCategory]); // eslint-disable-line
  useEffect(() => {
    resourcesApi
      .categories()
      .then(setCategories)
 ...
 `}</style>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        {isAdmin && (
          <button
            onClick={() => setEditing({})}
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-accent to-accent text-bg-primary rounded-xl text-sm font-bold hover: transition-all"
          >
        ...
        {categories.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-1 border-t border-ink/5">
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${activeCategory === "all" ? "bg-accent text-accent-fg" : "text-text-muted hover:text-text-primary"}`}
            >
              All categories
            </button>
            {categories.map((c) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-all ${activeCategory === c ? "bg-accent text-accent-fg" : "text-text-muted hover:text-text-primary"}`}
              >
                {c}
              </button>
            ))}
      ...
      {!error && items.length === 0 && (
        <div className="glass-card rounded-xl p-12 border border-ink/08 text-center">
          ...
          <p className="text-text-secondary text-sm">
            {t("resources.empty", { defaultValue: "No resources yet" })}
          </p>
          {isAdmin && (
            <button
              onClick={() => setEditing({})}
      ...
      {deleteConfirm && (
        ...
        >
          <div
            className="bg-bg-secondary rounded-t-3xl sm:rounded-2xl p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] border-t border-loss/25 sm:border max-w-sm w-full"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <p className="text-text-muted text-sm text-center mb-5">
              "{deleteConfirm.title}" will be hidden from the hub.
            </p>
