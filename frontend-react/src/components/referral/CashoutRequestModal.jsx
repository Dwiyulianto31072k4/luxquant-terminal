...
const CashoutRequestModal = ({ isOpen, onClose, availableBalance = 0, onSuccess }) => {
 ...
 <svg className="h-7 w-7 text-profit" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
 ...
 Admin will DM you at <span className="text-accent">@{success.destination_telegram}</span> to coordinate.
 </p>
 ...
 <span className="text-3xl font-bold tabular-nums text-accent" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{availableBalance.toFixed(2)}</span>
 ...
 <button type="button" onClick={setMaxAmount} className="rounded bg-accent/12 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent transition-colors hover:bg-accent/20">Max</button>
 </div>
