...
const CashoutRequestModal = ({ isOpen, onClose, availableBalance = 0, onSuccess }) => {
  ...
  const validate = () => {
    ...
    if (!/^[a-zA-Z0-9_]{5,32}$/.test(telegram))
      return "Telegram username can only contain letters, numbers, and underscores (5–32 chars).";
    return null;
  };
  ...
  const inputCls =
    "w-full rounded-xl border border-ink/[0.08] bg-surface-raised px-4 py-3.5 text-text-primary outline-none transition-colors focus:border-ink/15 disabled:opacity-50";

  const footer = (
    <div className="flex gap-3">
      <GhostButton onClick={guardedClose} disabled={submitting} className="flex-1">
        Cancel
      </GhostButton>
      <GoldButton
    ...
    >
      {success ? (
        <div className="py-6 text-center">
          ...
          <h4 className="mb-2 text-base font-bold text-profit">Cashout #{success.id} Submitted!</h4>
          <p className="mb-4 text-xs leading-relaxed text-text-secondary">
            ${success.amount_usdt.toFixed(2)} has been reserved.
            <br />
            Admin will DM you at{" "}
        ...
        <>
          {/* Available balance */}
          <div className="mb-5 rounded-xl border border-ink/12 bg-surface-raised p-4">
          ...
          <div className="mb-4">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button
                ...
                className="rounded bg-accent/12 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-accent transition-colors hover:bg-accent/20"
              >
                Max
              </button>
            </div>
            <div className="relative">
              ...
              <input
                ...
                placeholder="0.00"
                disabled={submitting}
                className={`${inputCls} pl-8 pr-16 text-base font-semibold`}
                style={{ fontFamily: "'Space Grotesk', sans-serif" }}
              />
              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-semibold text-text-muted">
                USDT
              </span>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <p className="mt-1.5 text-[10px] text-text-muted">
              Admin will DM you at this username to coordinate.
            </p>
          </div>
          ...
          <div className="mb-5">
            ...
            <textarea
              ...
              placeholder="Preferred network, time zone, etc. (optional)"
              maxLength={500}
              disabled={submitting}
              rows={2}
          ...
          >
            ...
            <p className="text-[11px] leading-relaxed text-text-secondary">
              <span className="font-semibold text-text-primary/90">Hard reserve:</span> your balance
              is reserved immediately on submit. You can cancel while it’s pending, and it’s
              refunded if the admin rejects it.
            </p>
          </div>
