...
const StatsCards = ({ stats, loading }) => {
  if (loading) {
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-8">
        {[...Array(7)].map((_, i) => (
          <div key={i} className="glass-card rounded-xl p-4 border border-ink/08 animate-pulse">
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-bg-primary border border-ink/12 rounded-lg px-3 py-2 shadow-xl">
          <p className="text-accent font-semibold">{data.name}</p>
  ...
  return (
    <div className="mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
        {/* Total Signals Card */}
        <div className="glass-card rounded-xl p-5 border border-ink/08">
...
const StatCard = ({ label, value, color }) => (
  <div className="glass-card rounded-lg p-3 border border-ink/08 text-center">
    <p className="text-text-muted text-[10px] uppercase tracking-wider mb-1">{label}</p>
    <p className={`text-lg font-display font-bold ${color}`}>{value?.toLocaleString() || 0}</p>
  </div>
);
