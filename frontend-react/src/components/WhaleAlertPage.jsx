...
const transferTypeStyle = (type) => {
  if (type === "exchange_inflow") return "bg-loss/10 text-loss border-loss/25";
  if (type === "exchange_outflow") return "bg-profit/10 text-profit border-profit/25";
  if (type === "exchange_to_exchange") return "bg-accent/12 text-accent border-ink/12";
    ...
    <div className="relative overflow-hidden bg-surface-raised border border-ink/[0.06] rounded-md">
      <div className="relative px-4 py-3 flex flex-wrap items-center gap-x-6 gap-y-2">
        ...
        <div className="flex items-center gap-2.5">
          <span
            className={`w-1.5 h-1.5 rounded-full ${sentimentDot} ${isBullish || isBearish ? "animate-pulse" : ""}`}
          />
          <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-text-muted">
            Net Flow
          </span>
          <span
            className={`font-mono text-[11px] uppercase tracking-[0.2em] font-semibold ${sentimentColor}`}
          >
            {sentimentLabel}
          </span>
        </div>
...
const FlowItem = ({ label, value, sublabel, tone, valueColor }) => {
  ...
  return (
    <div className="flex items-center gap-2 font-mono text-[11px]">
      ...
      {sublabel && <span className="text-text-muted/50 lowercase tabular-nums">{sublabel}</span>}
    </div>
  );
};
...
const TransactionRow = ({ tx, isNew, t }) => {
  const fromLabel =
    tx.from_owner && tx.from_owner !== "unknown" ? tx.from_owner : tx.from_address || "—";
  const toLabel = tx.to_owner && tx.to_owner !== "unknown" ? tx.to_owner : tx.to_address || "—";
      ...
      <div className="flex items-center gap-3 px-3 sm:px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span
              className={`font-mono text-sm tabular-nums font-semibold ${isHighlight ? "text-accent" : "text-text-primary"}`}
            >
...
const LoadingSkeleton = () => (
  <div className="space-y-px">
    {[...Array(8)].map((_, i) => (
      <div key={i} className="flex items-center gap-3 px-4 py-3 border-b border-ink/[0.04]">
...
const EmptyState = ({ message, hint }) => (
  <div className="relative overflow-hidden p-12 text-center">
    ...
    {hint && (
      <p className="text-text-muted text-[10px] font-mono uppercase tracking-[0.15em]">{hint}</p>
    )}
  </div>
);
          ...
          {loading && (
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="relative overflow-hidden bg-surface-raised border border-ink/[0.06] rounded-md p-4"
                >
