...
// Assisted payment — bank transfer / other network. Public copy never says "admin".
  ...
  const footer = (
    <div>
      <GoldButton
        ...
        className="flex w-full items-center justify-center gap-2.5 !py-3 !text-sm"
      >
        <TelegramGlyph className="h-4 w-4" />
        <span>{t("pricing.admin_open")}</span>
      </GoldButton>
      <p className="mt-2.5 text-center text-[10px] text-text-muted">{t("pricing.admin_wait")}</p>
    </div>
  );
      ...
      <div className="px-6 py-4 sm:px-7">
        <div className="relative mb-4 overflow-hidden rounded-xl border border-ink/08 bg-surface-raised px-4 py-3">
          <span
            ...
            style={{
              background:
                "linear-gradient(to right, transparent, rgb(var(--accent) / 0.25), transparent)",
            }}
          />
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[10px] uppercase tracking-wider text-text-muted">
                {t("pricing.admin_plan")}
              </p>
        ...
        <div className="mb-2 flex items-center justify-between">
          ...
          >
            {copied ? t("pricing.admin_copied") : t("pricing.admin_copy")}
          </button>
        </div>
        ...
        />
        <p className="mt-2 text-[10px] text-text-muted">{t("pricing.admin_prefill")}</p>
      </div>
    </Modal>
  );
};
