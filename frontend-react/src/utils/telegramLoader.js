...
export function ensureTelegram(timeout = 8000) {
 ...
 if (window.Telegram?.Login?.auth) { clearInterval(poll); resolve(window.Telegram); }
 else if (Date.now() - start > timeout) { clearInterval(poll); telegramPromise = null; reject(new Error('telegram-load-timeout')); }
 }, 100);
 });
