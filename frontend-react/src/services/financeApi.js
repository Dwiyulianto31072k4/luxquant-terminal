...
export const financeApi = {
  ...
  createManualPayment: async (payload) => {
    const response = await api.post("/api/v1/workspace/finance/manual-payment", payload);
    return response.data;
  ...
  cancelPayment: async (paymentId, note = null) => {
    const body = note ? { note } : {};
    const response = await api.post(`/api/v1/workspace/finance/payments/${paymentId}/cancel`, body);
    return response.data;
  },

  refundPayment: async (paymentId, note = null) => {
    const body = note ? { note } : {};
    const response = await api.post(`/api/v1/workspace/finance/payments/${paymentId}/refund`, body);
    return response.data;
  },

  addNote: async (paymentId, note) => {
    const response = await api.post(`/api/v1/workspace/finance/payments/${paymentId}/note`, {
      note,
    });
    return response.data;
  },
  ...
  voidPayment: async (paymentId, note = null) => {
    const body = note ? { note } : {};
    const response = await api.post(`/api/v1/workspace/finance/payments/${paymentId}/void`, body);
    return response.data;
  },
  ...
  deletePayment: async (paymentId) => {
    const response = await api.delete(`/api/v1/workspace/finance/payments/${paymentId}`);
    return response.data;
  },

  bulkCancelStale: async (hours = 24) => {
    const response = await api.post(`/api/v1/workspace/finance/payments/bulk-cancel-stale`, null, {
      params: { hours },
    });
    return response.data;
  },
